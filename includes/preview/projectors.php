...
namespace ElementorMCP\Preview\Projectors;
...
/**
 * Projected after-images for the writes Elementor MCP understands.
 *
 * A projector is a pure function of (before-snapshot, normalized input) that
 * returns an after-snapshot in the exact shape elementor_mcp_snapshot_post() and its
 ...
 * reason, in the same voice elementor_mcp_extension_files_rollback_reason() uses to
 ...
 *
 * Deliberately shaped as the sibling of elementor_mcp_capture_before_image(), which is
...
{
    $projectors = [
        'elementor-mcp/update-post' => __NAMESPACE__ . '\\project_update_post',
        'elementor-mcp/update-media' => __NAMESPACE__ . '\\project_update_post',
        'elementor-mcp/update-site-settings' => __NAMESPACE__ . '\\project_update_settings',
        'elementor-mcp/delete-post' => __NAMESPACE__ . '\\project_delete',
        'elementor-mcp/delete-media' => __NAMESPACE__ . '\\project_delete',
        'elementor-mcp/delete-term' => __NAMESPACE__ . '\\project_delete',
        'elementor-mcp/delete-comment' => __NAMESPACE__ . '\\project_delete',
        'elementor-mcp/delete-menu' => __NAMESPACE__ . '\\project_delete',
        'elementor-mcp/delete-menu-item' => __NAMESPACE__ . '\\project_delete',
    ];

    /** @var mixed $filtered */
    $filtered = apply_filters('elementor_mcp_preview_projectors', $projectors);
        ...
        if (str_contains($name, $needle)) {
            return [
                ...
                'message' => 'Preview is not available for ' . $ability_name . ': it writes files to the server, and '
                    ...
                    . '— the change ledger records it, and elementor-mcp/list-changes reports how to undo it by hand.',
            ];
        }
    }
    ...
    if (str_contains($name, 'create-')) {
        return [
            ...
            'message' => 'Preview is not available for ' . $ability_name . ': a create changes nothing that exists '
                ...
                . 'undo — elementor-mcp/list-changes reports them as reversible.',
        ];
    }

    return [
        'reason' => 'no_projector',
        'message' => 'Preview is not available for ' . $ability_name . '. Elementor MCP only previews abilities it has a '
...
function unpredicted_for(string $ability_name, array $input): array
{
    $notes = [];
    if (
        in_array($ability_name, ['elementor-mcp/update-post', 'elementor-mcp/update-media'], strict: true)
...
function side_effects_for(string $ability_name, array $input): array
{
    ...
    if (in_array($ability_name, ['elementor-mcp/update-post', 'elementor-mcp/update-media'], strict: true)) {
...
/**
 * Project elementor-mcp/update-post and elementor-mcp/update-media.
 *
...
{
    if (($before['type'] ?? '') !== 'post') {
        return new WP_Error('elementor_mcp_preview_unsupported_target', 'The target is not a post-shaped record.');
    }
...
/**
 * Project elementor-mcp/update-site-settings.
 *
...
function project_update_settings(array $before, array $input): array|WP_Error
{
    if (($before['type'] ?? '') !== 'settings') {
        return new WP_Error('elementor_mcp_preview_unsupported_target', 'The target is not a settings record.');
    }

    if (function_exists('ElementorMCP\\Abilities\\WordPress\\wordpress_validate_site_settings')) {
        $invalid = \ElementorMCP\Abilities\WordPress\wordpress_validate_site_settings($input);
