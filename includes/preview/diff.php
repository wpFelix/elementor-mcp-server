...
namespace ElementorMCP\Preview\Diff;
 ...
 *     ELEMENTOR_MCP_VOLATILE_POST_FIELDS that the ledger fingerprint also uses.
...
function walk(array $before, array $after, array $path, array &$entries, array $sensitive_keys): void
{
    ...
    foreach ($keys as $key) {
        ...
        // excluded_meta_keys is by definition one elementor_mcp_change_key_is_sensitive()
...
{
    $volatile = defined('ELEMENTOR_MCP_VOLATILE_POST_FIELDS')
        ? (array) constant('ELEMENTOR_MCP_VOLATILE_POST_FIELDS')
...
{
    if (!function_exists('elementor_mcp_change_key_is_sensitive')) {
        return false;
    }
    foreach ($path as $segment) {
        if (\elementor_mcp_change_key_is_sensitive($segment)) {
            return true;
        }
    }
    return false;
