...
namespace ElementorMCP\Preview\Store;
 ...
 * custom post type a long-lived review queue would use. A preview is one
 * short-lived record with no children. A CPT would
 * have cost a register_post_type block, a retention cron, an entry in
 * elementor_mcp_deactivate_current_site(), and another in elementor_mcp_uninstall_post_types()
 * — for storage that a bounded option already provides. The bound-by-count-then-
 * by-bytes discipline is the one elementor_mcp_store_change() already uses.
 *
...
const OPTION_PREFIX = 'elementor_mcp_preview_';
const LOCK_PREFIX = 'elementor_mcp_preview_lock_';
const INDEX_OPTION = 'elementor_mcp_preview_index';
...
const PAYLOAD_AAD = 'elementor-mcp-preview-v1';
...
function create(array $record): string|\WP_Error
{
    ...
    if (is_string($encoded) && strlen($encoded) > MAX_RECORD_BYTES) {
        ...
        if (!is_string($encoded) || strlen($encoded) > MAX_RECORD_BYTES) {
            return new \WP_Error(
                'elementor_mcp_preview_too_large',
...
function get(string $id): ?array
{
    ...
    if (($record['status'] ?? '') === STATUS_APPLYING && applying_is_stale($record)) {
        ...
        $record['error'] = [
            'code' => 'elementor_mcp_preview_apply_interrupted',
 ...
 * compare-and-set without a transaction. Two administrators clicking Apply on
...
function encode_input(mixed $input): string|\WP_Error
{
    if (!function_exists('openssl_encrypt')) {
        return new \WP_Error(
            'elementor_mcp_preview_crypto_unavailable',
    ...
    if (!is_string($json)) {
        return new \WP_Error('elementor_mcp_preview_encode_failed', 'The preview input could not be encoded.', ['status' => 500]);
    }

    try {
        $iv = random_bytes(12);
    } catch (\Throwable) {
        return new \WP_Error('elementor_mcp_preview_crypto_failed', 'The preview input could not be encrypted.', ['status' => 500]);
    }
    ...
    return is_string($envelope)
        ? $envelope
        : new \WP_Error('elementor_mcp_preview_encode_failed', 'The encrypted preview input could not be encoded.', ['status' => 500]);
}
...
function decode_input(string $payload): mixed
{
    if (!function_exists('openssl_decrypt')) {
        return new \WP_Error('elementor_mcp_preview_crypto_unavailable', 'This server cannot decrypt the preview input.', ['status' => 500]);
    }

    /** @var mixed $envelope */
    $envelope = json_decode($payload, associative: true);
    if (!is_array($envelope) || ($envelope['v'] ?? null) !== 1 || ($envelope['alg'] ?? '') !== 'A256GCM') {
        return new \WP_Error('elementor_mcp_preview_payload_invalid', 'The stored preview input is invalid.', ['status' => 409]);
    }
    ...
    if (!is_string($json)) {
        return new \WP_Error('elementor_mcp_preview_payload_tampered', 'The stored preview input failed authentication.', ['status' => 409]);
    }
    ...
    return json_last_error() === JSON_ERROR_NONE
        ? $decoded
        : new \WP_Error('elementor_mcp_preview_payload_invalid', 'The stored preview input is invalid.', ['status' => 409]);
}
