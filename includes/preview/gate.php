...
namespace ElementorMCP\Preview\Gate;

use WP_Ability;
use WP_Error;
use ElementorMCP\Preview;
use ElementorMCP\Preview\Projectors;
...
const OPTION = 'elementor_mcp_require_preview_before_write';

/**
 * Whether the rule is switched on.
 *
 * Both '1' and true are accepted, the same test elementor_mcp_is_enabled() makes.
...
function check(WP_Ability $ability, mixed $input): ?WP_Error
{
    ...
    if (str_starts_with($name, 'elementor-mcp/preview-ability') || str_starts_with($name, 'elementor-mcp/apply-preview')) {
        return null;
    }
    ...
    return new WP_Error(
        'elementor_mcp_preview_required',
        sprintf(
            'This site requires a reviewed preview before writes. Call elementor-mcp/preview-ability with ability_name '
                . '"%s" and the same input, show the user the diff, then apply it with elementor-mcp/apply-preview.',
            $name,
        ),
        ['status' => 409, 'ability' => $name, 'preview_ability' => 'elementor-mcp/preview-ability'],
    );
}
...
function register_setting(mixed $sections): mixed
{
    ...
    $sections[] = [
        'id' => 'elementor-mcp-preview',
        'title' => __('Preview before write', domain: 'elementor-mcp'),
        'description' => __(
            'Agents can always call Elementor MCP Preview on their own. This makes it mandatory for the writes it covers.',
        ...
        'fields' => [
            [
                'type' => 'toggle',
                'name' => OPTION,
                'label' => __('Require a reviewed preview before agent writes (MCP and REST)', domain: 'elementor-mcp'),
                'help' => __(
                    'A write that Elementor MCP can preview is refused unless it goes through elementor-mcp/preview-ability and elementor-mcp/apply-preview, so a person sees the diff first. Abilities Elementor MCP cannot preview are unaffected, rather than being blocked with no way through. The built-in Chat screen is not covered — it has its own approval step.',
                    domain: 'elementor-mcp',
                ),
                'value' => required(),
