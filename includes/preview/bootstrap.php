 ...
 * because that is where elementor_mcp_register_builtin_abilities() looks.
 */
...
if (is_admin()) {
    ...
    add_filter('elementor_mcp_nav_map', __NAMESPACE__ . '\\Admin\\register_nav');
}
...
// ElementorMCP\Mcp\call_tool(). Chat calls WP_Ability::execute() directly and is out
// of scope, which the setting's own label says.
add_filter('elementor_mcp_pre_ability_execute', __NAMESPACE__ . '\\Gate\\filter_pre_ability_execute', priority: 8, accepted_args: 3);
