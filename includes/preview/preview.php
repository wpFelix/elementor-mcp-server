...
namespace ElementorMCP\Preview;
 ...
 *
 * Both entry points — the elementor-mcp/apply-preview ability and the admin screen's
 ...
 * so the state is set explicitly instead — the role elementor_mcp_change_is_suppressed()
...
{
    $ability_name = (string) \elementor_mcp_sanitize_requested_ability_name($ability_name);
    if (!\elementor_mcp_is_valid_ability_name($ability_name)) {
        return new WP_Error('elementor_mcp_preview_invalid_ability', 'Ability name is not valid.', ['status' => 400]);
    }
    ...
    $before = \elementor_mcp_capture_before_image($ability_name, $input);
    if ($before === null) {
        return [
            'supported' => false,
            'ability' => $ability_name,
    ...
    if (($before['type'] ?? '') === 'oversize') {
        return [
            ...
            'message' => sprintf(
                ...
                (int) ($before['bytes'] ?? 0),
                ELEMENTOR_MCP_CHANGE_SNAPSHOT_MAX_BYTES,
            ),
            'diff' => null,
        ];
    }
    ...
    $record = [
        ...
        'input' => \elementor_mcp_redact_for_log($input),
        'input_payload' => $input_payload,
        'risk' => \elementor_mcp_ability_risk($ability),
        'supported' => true,
        'would_fail' => false,
        'target' => target_summary($before),
        'base_fingerprint' => (string) ($before['fingerprint'] ?? \elementor_mcp_snapshot_fingerprint($before)),
        ...
        'agent' => \elementor_mcp_current_agent(),
    ...
    $record['apply_ability'] = 'elementor-mcp/apply-preview';
    $record['apply_params'] = ['preview_id' => $id, 'confirm' => true];
    $record['user_instruction'] = sprintf(
        'Open %s to review this diff and apply or discard it, or call elementor-mcp/apply-preview with preview_id "%s" '
...
function apply(string $preview_id, bool $confirm): array|WP_Error
{
    ...
    if ($record === null) {
        return new WP_Error('elementor_mcp_preview_not_found', 'No such preview.', ['status' => 404]);
    }

    $status = (string) ($record['status'] ?? '');
    if ($status === Store\STATUS_EXPIRED) {
        return new WP_Error(
            'elementor_mcp_preview_expired',
            'This preview has expired. Run elementor-mcp/preview-ability again to see the current diff.',
            ['status' => 409, 'preview_id' => $preview_id],
        );
    }
    if ($status !== Store\STATUS_PENDING) {
        return new WP_Error(
            'elementor_mcp_preview_not_pending',
    ...
    if (!Store\claim_lock($preview_id)) {
        return new WP_Error(
            'elementor_mcp_preview_apply_raced',
    ...
    try {
        ...
        if (\elementor_mcp_ability_requires_confirmation($ability) && !$confirm) {
        ...
        if ($payload === '') {
            ...
            return new WP_Error(
                'elementor_mcp_preview_legacy_payload',
        ...
        if (!is_array($decoded_input)) {
            Store\update($preview_id, ['status' => Store\STATUS_FAILED]);
            return new WP_Error(
                'elementor_mcp_preview_payload_invalid',
        ...
        $current = \elementor_mcp_capture_before_image($ability_name, $input);
        if ($current === null || ($current['type'] ?? '') === 'oversize') {
            Store\update($preview_id, ['status' => Store\STATUS_CONFLICTED]);
            return new WP_Error(
                'elementor_mcp_preview_target_missing',
        ...
        if ($expected_scope !== '' && !hash_equals($expected_scope, $observed_scope)) {
            Store\update($preview_id, ['status' => Store\STATUS_CONFLICTED]);
            return new WP_Error(
                'elementor_mcp_preview_drifted',
                'The target changed after this preview was created. Nothing was applied. Run '
                    . 'elementor-mcp/preview-ability again to see the current diff.',
                [
                    'status' => 409,
...
function discard(string $preview_id): array|WP_Error
{
    ...
    if (!Store\claim_lock($preview_id)) {
        return new WP_Error(
            'elementor_mcp_preview_discard_raced',
...
function scope_fingerprint(array $snapshot, array $diff): string
{
    ...
    ksort($scoped);
    return \elementor_mcp_snapshot_fingerprint($scoped);
}
...
function unavailable_error(string $ability_name): WP_Error
{
    ...
    if (function_exists('elementor_mcp_get_ability_rules')) {
        $rules = \elementor_mcp_get_ability_rules();
        if (($rules[$ability_name]['disabled'] ?? false) === true) {
            $hint = 'It is switched off on the Elementor MCP Abilities screen.';
        }
    }

    if (function_exists('elementor_mcp_get_safety_profile')) {
        $profile = \elementor_mcp_get_safety_profile();
        $hint .= sprintf(' The active safety profile is "%s".', $profile);
    }

    return new WP_Error(
        'elementor_mcp_preview_ability_unavailable',
...
function preview_url(string $id): string
{
    return add_query_arg(
        ['page' => 'elementor-mcp-preview', 'preview' => $id],
        admin_url('admin.php'),
    );
}
