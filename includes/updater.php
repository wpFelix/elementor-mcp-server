 ...
 *
 * Checks https://elementormcp.com/api/elementor-mcp/info for new versions
...
add_filter('site_transient_update_plugins', callback: 'elementor_mcp_check_for_updates');
add_filter('plugins_api', callback: 'elementor_mcp_plugins_api', priority: 10, accepted_args: 3);
 ...
 * @return mixed
 */
function elementor_mcp_check_for_updates($transient)
{
    ...
    $remote = elementor_mcp_fetch_update_info();
    $plugin_file = plugin_basename(dirname(__DIR__) . '/elementor-mcp.php');

    if ($remote === null || !version_compare(ELEMENTOR_MCP_VERSION, $remote['version'], operator: '<')) {
        // No remote info or up-to-date. Report as no_update so WordPress.org cannot override.
        $transient->no_update[$plugin_file] = (object) [
            'slug' => 'elementor-mcp',
            'plugin' => $plugin_file,
            'new_version' => ELEMENTOR_MCP_VERSION,
            'url' => '',
            'package' => '',
        ];
 ...
 */
function elementor_mcp_plugins_api($result, $action, $args)
{
    if ($action !== 'plugin_information' || ($args->slug ?? '') !== 'elementor-mcp') {
        return $result;
    }
 ...
 */
function elementor_mcp_fetch_update_info()
{
    $cache_key = 'elementor_mcp_update_info';
    ...
    $raw = elementor_mcp_request_update_info();
    if ($raw === null) {
        set_transient($cache_key, value: 'error', expiration: HOUR_IN_SECONDS);
        return null;
    }

    $data = elementor_mcp_normalize_update_response($raw);
 ...
 */
function elementor_mcp_request_update_info()
{
    ...
    $url = add_query_arg(['v' => ELEMENTOR_MCP_VERSION], 'https://elementormcp.com/api/elementor-mcp/info');
 ...
 */
function elementor_mcp_normalize_update_response($raw)
{
    ...
    return [
        'name' => (string) ($raw['name'] ?? 'Elementor MCP'),
