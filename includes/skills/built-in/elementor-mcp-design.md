...
1. Call `elementor-mcp/get-active-design`.
2. If `active` is true, first require `readiness.ready: true`. Use the structured
   ...
   appear. Only `readiness`, `token_sources`, and `waivers` are Elementor MCP-computed
...
First call `elementor-mcp/list-design-library` — if a design was already saved, offer
...
`elementor-mcp/save-design` (`activate: true`).

`elementor-mcp/list-design-examples` holds seven worked directions (the same set the Design screen offers as **starter kits**) — light and dark,
...
a classic-theme Customizer, Elementor's global kit, or
simply the **rendered CSS of a real page** (the actual colors and fonts in the
live output, which reflects the design system the site runs
) — then synthesize those into a DESIGN.md and save it. Reading the site is
always safe; a surface-specific read ability, where installed, can read a
Elementor's global kit more precisely, while rendered-output reading works on any
surface without it. This is the accurate, fast way to seed — or **update** — a
direction from what the site already looks like.

`elementor-mcp/adopt-design-from-site` does this read for you and returns a DESIGN.md
draft: Elementor's global kit, the standard
Customizer colours, and the rendered page styles,
most-authoritative-first. It saves nothing, and it reports the
...
Elementor MCP stays the editor on every one. Which surface to build on is a choice
...
- If you make a new committed choice (e.g. an accent for a component), fold it
  into the DESIGN.md and call `elementor-mcp/save-design` again so the active design
...
come from, which treats a static interface as forbidden — on Elementor MCP a **still
...
The realization skill for your surface carries the medium's specifics (Elementor's
...
- **Every control passes contrast.** Check button text against its own background,
  ...
  for body, 3:1 for large). `elementor-mcp/check-contrast` does the arithmetic against
...
text) to `elementor-mcp/check-design`. On the Elementor surface, pass the page's
**front-end rendering** — the HTML/CSS the site actually serves — not the
Elementor's internal JSON or widget config: the check reads what a visitor sees,
...
Then check the **published** page with `elementor-mcp/verify-rendered-page`. The
pre-flight reads the string you produced; this fetches what the server actually
sends, which is your output plus the theme, Elementor and every other plugin.
...
written into Elementor's own typography presets, so they apply site-wide
