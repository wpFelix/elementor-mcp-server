...
|----------------+-------------------+------------------------------|
Elementor widgets  CUSTOM THEME (this skill)
and templates      (HTML / CSS / JS / PHP)
```
...
`elementor-mcp/get-active-design` and build strictly within its tokens, `dials`, and
Do's / Don'ts. The same DESIGN.md drives every surface; only the realization
differs. If none is active, establish one first (the `elementor-mcp-design` skill).
Use `elementor-mcp/get-design` to read a specific saved direction to evolve.

Map the direction into code as one source of truth:

- Tokens to CSS custom properties: `:root { --bg; --ink; --accent; --font-display; --radius; --space }`. Keep the token layer in the custom theme so the coded surface and Elementor direction stay aligned.
...
- **A full-custom page on an existing, live site:** do NOT swap the theme, and do NOT
  ...
  removing it. The **Elementor MCP blank canvas** is the clean, plugin-owned surface for
...
1. **Read the direction.** `elementor-mcp/get-active-design` (or `get-design` for a
   specific slug). If none, establish one via `elementor-mcp-design` first.
2. **Create the theme you chose above** with `elementor-mcp/write-file` (a blank starter
...
3. **Activate** the child theme and **create the page**, assigning the template
   (`_wp_page_template`). Use `elementor-mcp/execute-php` where no dedicated ability
...
5. **Pre-flight** the rendered output with `elementor-mcp/check-design`; fix every
   `fail`.
...
forbidden and motion as mandatory. That stance does not hold here. Elementor MCP serves
...
These bite in practice on a real site running a heavy theme plus plugins plus
plugins:

- **Global CSS leaks into your full-custom template, and `wp_dequeue_style` is not enough.** A standalone template still loads the site's global styles, but the damaging leaks arrive through channels dequeue cannot touch. Three seen in practice on a real theme-and-plugin stack:
  - The parent theme **inlines its whole stylesheet** as an inline `<style id="...-inline-css">` block, and plugins print inline reset `<style>` blocks. `wp_dequeue_style` only removes *enqueued* `<link>`s, never inline `<style>`.
  - A leaked property you never declared still wins: a theme `border-left` / `padding` landed on a bare `<blockquote>` because scoping only overrides properties you actually set.
  - A plugin can **re-inject a stylesheet after your template closes**: it echoes a placeholder comment on `wp_head` / `wp_footer` and expands it into real `<link>`s via `str_replace` on the whole response, so at your strip time it is a comment, not a link.

  Do NOT fix this by stripping the whole document (`ob_start` plus removing every inline `<style>` and foreign `<link>`). That nukes legitimate CSS too: the WordPress **admin bar** loses its styles, and any functional plugin CSS on the page (forms, widgets) breaks. Instead: for a full-custom page, render it as a **self-owned minimal document** that never loads the foreign CSS in the first place (the **Elementor MCP blank canvas**); for a page that keeps the theme, **scope and override** (higher specificity, `:where()` resets, explicit fonts and colors), and reset `border` / `margin` / `padding` on any bare element you reuse (`blockquote`, `figure`, `ul`, `dl`), since scoping guards only declared properties.
...
- **The display font must actually load, or the platform font wins.** Setting `font-family` is not enough if the face is never delivered: the site's global font (often Inter, enqueued by the theme or a plugin) becomes the fallback. Self-host your faces with `@font-face` + `font-display: swap` and enqueue them on this template, or set the site's global font tokens so the whole site is on-brand, not just this page.
...
Before calling it done, pass the rendered HTML / CSS to `elementor-mcp/check-design`.
...
- Filesystem: `elementor-mcp/write-file`, `edit-file`, `read-file`, `list-directory` for the child theme, `style.css`, `functions.php`, template, `assets/`.
- `elementor-mcp/execute-php` narrowly for WP operations with no dedicated ability (activate theme, insert page, assign template, enqueue).
- `elementor-mcp/get-active-design` / `get-design` and `elementor-mcp/check-design` for direction and pre-flight.
- NOT the Elementor document abilities. This skill is the coded-theme surface.

## The real tradeoff (state it honestly to the user)

Custom theme is not "static" or "developer-only". Elementor MCP stays underneath, so
...
- **Custom theme**: changes go through Elementor MCP (conversational). No visual canvas, but fully editable via the agent.
