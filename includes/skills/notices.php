...
namespace ElementorMCP\Skills\Notices;
...
const RELOAD_NOTICE_TRANSIENT = 'elementor_mcp_skill_pending_reload_notice';

const RELOAD_NOTICE_TTL = 60;

/**
 * Mark that the skill index has changed and a reload-MCP-client notice
 * should be shown to the admin the next time they land on a Elementor MCP
 ...
 *    current admin screen belongs to Elementor MCP.
 */
function render(): void
{
    ...
    $inline_key = 'elementor_mcp_skill_admin_notice_' . get_current_user_id();
    ...
    // and we're on the Skills admin page specifically — other Elementor MCP
    // screens don't need this prompt.
    $screen = function_exists('get_current_screen') ? get_current_screen() : null;
    if (!$screen instanceof \WP_Screen) {
        return;
    }
    if ($screen->id !== 'elementor-mcp_page_' . Admin\PAGE_SLUG) {
        return;
    }
