...
namespace ElementorMCP\Skills\Abilities\SkillWrite;
...
function register(): void
{
    ...
    wp_register_ability('elementor-mcp/skill-write', [
        'label' => __('Write Skill', domain: 'elementor-mcp'),
        'description' => __(
            'Create or update a Elementor MCP user skill. The `title` is the only identifier — it is sanitised server-side (lowercase, dash-separated) and becomes both the human label and the slug used for lookups.',
...
function execute(array $input): array|WP_Error
{
    ...
    if (strlen($content) > Parser\MAX_BODY_BYTES) {
        return new WP_Error('body_too_large', __('Body exceeds 1 MB.', domain: 'elementor-mcp'));
    }
    ...
    if ($external_label !== null) {
        return new WP_Error(
            ...
            sprintf(
                ...
                __('Slug "%1$s" is already used by source "%2$s". Choose a different title.', domain: 'elementor-mcp'),
                $title,
                $external_label,
            ),
...
function resolve_conflict(string $title, string $on_conflict): WP_Error|array
{
    ...
    if ($on_conflict === 'fail') {
        return new WP_Error('slug_exists', __('A skill with this title already exists.', domain: 'elementor-mcp'), [
            'slug' => $title,
