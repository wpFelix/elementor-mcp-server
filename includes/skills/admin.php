...
namespace ElementorMCP\Skills\Admin;

use ElementorMCP\Skills\Abilities\SkillWrite;
use ElementorMCP\Skills\Cpt;
use ElementorMCP\Skills\Notices;
use ElementorMCP\Skills\Parser;
...
const PAGE_SLUG = 'elementor-mcp-skills';
...
/**
 * Skills lives as a submenu under the Elementor MCP top-level menu registered
 * in `elementor-mcp.php`. The parent slug `elementor-mcp-connect` is the canonical
 * Configuration page. Its position in the Elementor MCP submenu is set by the
...
function register_menu(): void
{
    add_submenu_page(
        ...
        page_title: \elementor_mcp_nav_label('elementor-mcp-skills'),
        menu_title: \elementor_mcp_nav_label('elementor-mcp-skills'),
...
function render_page(): void
{
    if (!current_user_can_manage()) {
        wp_die(esc_html__('You do not have permission to manage skills.', domain: 'elementor-mcp'));
    }
...
{
    $actions = [
        'elementor_mcp_skill_create',
        'elementor_mcp_skill_update',
        'elementor_mcp_skill_delete',
        'elementor_mcp_skill_restore',
        'elementor_mcp_skill_permanent_delete',
        'elementor_mcp_skill_toggle_status',
        'elementor_mcp_skill_toggle_activation',
        'elementor_mcp_skill_upload',
        'elementor_mcp_skill_download',
        'elementor_mcp_skill_download_all',
    ];
    foreach ($actions as $action) {
        $handler = 'handle_' . substr($action, strlen('elementor_mcp_skill_'));
...
function enqueue_assets(string $hook): void
{
    ...
    // the top-level menu slug ("elementor-mcp").
    if ($hook !== 'elementor-mcp_page_' . PAGE_SLUG) {
        return;
    }
    wp_enqueue_style(
        'elementor-mcp-skills-admin',
        (string) ELEMENTOR_MCP_PLUGIN_URL . 'includes/skills/assets/admin.css',
        [],
        \elementor_mcp_asset_version('includes/skills/assets/admin.css'),
    );
    ...
    wp_add_inline_script('code-editor', sprintf(
        'jQuery(function($){ var el = document.getElementById("elementor-mcp-skills-content"); if (el && window.wp && wp.codeEditor) { var inst = wp.codeEditor.initialize(el, %s); window.elementorMcpSkillsEditor = inst && inst.codemirror ? inst.codemirror : null; window.dispatchEvent(new CustomEvent("elementor-mcp-skills-editor-ready")); } });',
...
{
    require_capability_and_nonce('elementor_mcp_skill_create');
...
function handle_update(): void
{
    ...
    require_capability_and_nonce('elementor_mcp_skill_update_' . $post_id);
    ...
    if ($new_title === '') {
        redirect_with_notice(
            'error',
            __('Title must contain at least one letter or digit (lowercase, dash-separated).', domain: 'elementor-mcp'),
            $post_id,
        );
        return;
    }
    ...
    if (trim($input['description']) === '') {
        redirect_with_notice('error', __('Description is required.', domain: 'elementor-mcp'), $post_id);
        return;
    }
    ...
    if (strlen($content) > Parser\MAX_BODY_BYTES) {
        redirect_with_notice('error', __('Body exceeds 1 MB.', domain: 'elementor-mcp'), $post_id);
        return;
    }
    ...
    if ($new_title !== $post->post_name) {
        ...
        if ($external !== null) {
            redirect_with_notice(
                ...
                sprintf(
                    ...
                    __('Title "%1$s" is already used by source "%2$s". Choose a different title.', domain: 'elementor-mcp'),
                    $new_title,
                    $external,
                ),
                $post_id,
        ...
        if ($clash !== null && (int) $clash->ID !== $post_id) {
            redirect_with_notice('error', __('A skill with this title already exists.', domain: 'elementor-mcp'), $post_id);
            return;
        }
    }
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('elementor_mcp_skill_delete_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('elementor_mcp_skill_restore_' . $post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('elementor_mcp_skill_permanent_delete_' . $post_id);
    load_skill_post($post_id);
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('elementor_mcp_skill_toggle_status_' . $post_id);
    $post = load_skill_post($post_id);
...
function handle_toggle_activation(): void
{
    ...
    require_capability_and_nonce('elementor_mcp_skill_toggle_activation_' . $post_id);
    if (!in_array($field, ['enable_prompt', 'enable_agentic'], strict: true)) {
        wp_die(esc_html__('Invalid field.', domain: 'elementor-mcp'));
    }
...
{
    require_capability_and_nonce('elementor_mcp_skill_upload');

    $files = normalize_uploaded_files($_FILES['skill_file'] ?? null);
    if ($files === []) {
        redirect_with_notice('error', __('No files uploaded.', domain: 'elementor-mcp'));
        return;
    }
    if (count($files) > MAX_UPLOAD_FILES) {
        redirect_with_notice('error', sprintf(
            ...
            __('Too many files. Upload up to %d skills at a time.', domain: 'elementor-mcp'),
            MAX_UPLOAD_FILES,
        ));
        return;
    }
    ...
    if ($imported !== []) {
        ...
        if ($imported_ids !== []) {
            set_transient('elementor_mcp_skill_just_imported_' . get_current_user_id(), $imported_ids, expiration: 30);
        }
    }
...
function process_one_upload(array $file, string $on_conflict): string|\WP_Error
{
    if ($file['error'] !== UPLOAD_ERR_OK) {
        return new \WP_Error('upload_error', __('upload failed', domain: 'elementor-mcp'));
    }
    if (!str_ends_with(strtolower($file['name']), '.md')) {
        return new \WP_Error('not_md', __('not a .md file', domain: 'elementor-mcp'));
    }
    if ($file['size'] > Parser\MAX_BODY_BYTES) {
        return new \WP_Error('too_large', __('too large (max 1 MB)', domain: 'elementor-mcp'));
    }
    if ($file['tmp_name'] === '' || !is_readable($file['tmp_name'])) {
        return new \WP_Error('unreadable', __('could not be read', domain: 'elementor-mcp'));
    }
...
function build_upload_notice(array $imported, array $failed): string
{
    if ($imported === [] && $failed !== []) {
        return sprintf(
            ...
            __('Upload failed: %s', domain: 'elementor-mcp'),
            implode(', ', $failed),
        );
    }
    $head = sprintf(
        /* translators: %d: count */
        _n('%d skill imported.', plural: '%d skills imported.', number: count($imported), domain: 'elementor-mcp'),
        count($imported),
    );
    if ($failed === []) {
        return $head;
    ...
    return sprintf(
        ...
        __('%1$s Failed: %2$s', domain: 'elementor-mcp'),
        $head,
        implode(', ', $failed),
    );
}
...
{
    $post_id = read_post_id();
    require_capability_and_nonce('elementor_mcp_skill_download_' . $post_id);
    $post = load_skill_post($post_id);
...
{
    require_capability_and_nonce('elementor_mcp_skill_download_all');
    ...
    if ($posts === []) {
        redirect_with_notice('error', __('No skills to download.', domain: 'elementor-mcp'));
        return;
    }

    $tmp = wp_tempnam('elementor-mcp-skills-' . time() . '.zip');
    if ($tmp === '') {
        redirect_with_notice('error', __('Could not create temporary ZIP file.', domain: 'elementor-mcp'));
        return;
    }
    ...
    if ($zip->open($tmp, \ZipArchive::OVERWRITE) !== true) {
        wp_delete_file($tmp);
        redirect_with_notice('error', __('Could not open the ZIP file for writing.', domain: 'elementor-mcp'));
        return;
    }
    ...
    wp_delete_file($tmp);
    if ($raw === false) {
        redirect_with_notice('error', __('Could not read the generated ZIP.', domain: 'elementor-mcp'));
        return;
    }

    $filename = sprintf('elementor-mcp-skills-%s.zip', wp_date('Y-m-d'));
    nocache_headers();
...
{
    set_transient(
        'elementor_mcp_skill_admin_notice_' . get_current_user_id(),
