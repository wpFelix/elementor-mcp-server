...
wp_register_ability('elementor-mcp/delete-file', [
    'label' => __('Delete File', domain: 'elementor-mcp'),
    'description' => __(
    ...
    'execute_callback' => 'elementor_mcp_delete_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'SANDBOX NOTES:',
                '- Files in wp-content/elementor-mcp-sandbox/ (the PHP sandbox) can be deleted.',
                '- To exit safe mode after a crash, delete: wp-content/elementor-mcp-sandbox/.crashed',
            ]),
            'readonly' => false,
            'destructive' => true,
            'idempotent' => true,
 ...
 */
function elementor_mcp_delete_file($input)
{
    $resolved = elementor_mcp_resolve_path((string) $input['path'], must_exist: false);
    ...
    // Delete a directory.
    if (is_dir($resolved)) {
        return elementor_mcp_delete_directory($resolved, $recursive);
    }
 ...
 * @return array|WP_Error
 */
function elementor_mcp_delete_directory($resolved, $recursive)
{
