...
wp_register_ability('elementor-mcp/create-admin-access-link', [
    'label' => __('Create Admin Access Link', domain: 'elementor-mcp'),
    'description' => __(
    ...
    'input_schema' => [
        ...
        'properties' => [
            ...
            'admin_path' => [
                'type' => 'string',
                'description' => 'Optional wp-admin-relative path to open after login, such as "plugins.php" or "admin.php?page=elementor-mcp-connect". External URLs are rejected.',
                'default' => '',
            ],
        ],
    ...
    'execute_callback' => 'elementor_mcp_create_admin_access_link',
 ...
 */
function elementor_mcp_create_admin_access_link(array $input = [])
{
    $user_id = get_current_user_id();
    if ($user_id <= 0 || !elementor_mcp_current_user_can_manage()) {
    ...
    $access = elementor_mcp_create_admin_access_token($user_id, $expires_in, $session_expires_in, $admin_path);
    if (is_wp_error($access)) {
        return $access;
    }

    $exchange_url = rest_url('elementor-mcp/v1/admin-access');
    $token_header = 'X-Elementor MCP-Admin-Access-Token';
    $nonce_header = 'X-Elementor MCP-Admin-Access-Nonce';
