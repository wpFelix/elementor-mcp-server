...
wp_register_ability('elementor-mcp/write-file', [
    'label' => __('Write File', domain: 'elementor-mcp'),
    'description' => __(
        'Writes small UTF-8 text content to a file on the server filesystem. PHP files (*.php) and PHP execution control files can ONLY be written to the sandbox directory (wp-content/elementor-mcp-sandbox/). Other non-PHP files can intentionally go anywhere under ABSPATH. The sandbox is for loading and crash recovery of generated PHP, not security isolation for all filesystem writes. Does not accept base64 or binary uploads; use elementor-mcp/create-upload-link for ZIPs, plugins, themes, media, binary files, or other large uploads. Automatically creates parent directories when needed.',
    ...
    'input_schema' => [
        ...
        'properties' => [
            ...
            'content' => [
                'type' => 'string',
                'description' => 'Small UTF-8 text content to write to the file. Do not pass base64 or binary data; use elementor-mcp/create-upload-link for binary or large uploads.',
            ],
            'encoding' => [
                'type' => 'string',
                'description' => 'Content encoding. Only UTF-8 text is supported; base64 and binary uploads are rejected. Use elementor-mcp/create-upload-link for those.',
                'enum' => ['utf-8'],
                'default' => 'utf-8',
            ],
            'mode' => [
    ...
    'execute_callback' => 'elementor_mcp_write_file',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'PHP FILE SANDBOX:',
                '- PHP files (*.php) and PHP execution control files can ONLY be written to: wp-content/elementor-mcp-sandbox/',
                '- Use a path like "wp-content/elementor-mcp-sandbox/my-feature.php"',
                ...
                '- For ZIPs, plugins, themes, media, binary files, or other large uploads, use elementor-mcp/create-upload-link instead.',
                ...
                '- After fixing, delete the file "wp-content/elementor-mcp-sandbox/.crashed"',
 ...
 */
function elementor_mcp_decode_write_content(string $content, string $encoding): string|WP_Error
{
    if ($encoding === 'base64') {
        return new WP_Error(
            'base64_not_supported',
            'elementor-mcp/write-file does not accept base64 or binary content. Use elementor-mcp/create-upload-link for ZIPs, plugins, themes, media, binary files, or other large uploads.',
        );
    }

    if ($encoding !== 'utf-8') {
        return new WP_Error('unsupported_encoding', 'elementor-mcp/write-file only accepts UTF-8 text content.');
    }
 ...
 */
function elementor_mcp_write_file($input)
{
    $resolved = elementor_mcp_resolve_path(path: (string) $input['path'], must_exist: false);
    ...
    $content = elementor_mcp_decode_write_content((string) $input['content'], $encoding);
