...
wp_register_ability('elementor-mcp/list-directory', [
    'label' => __('List Directory', domain: 'elementor-mcp'),
    'description' => __(
    ...
    'execute_callback' => 'elementor_mcp_list_directory',
    'permission_callback' => 'elementor_mcp_permission_callback',
    'meta' => [
        'show_in_rest' => true,
        'mcp' => ['public' => true],
        'annotations' => [
            'instructions' => 'TIP: AI-written PHP plugins live in wp-content/elementor-mcp-sandbox/. Check wp-content/elementor-mcp-sandbox/.crashed to see if safe mode is active.',
            'readonly' => true,
            'destructive' => false,
            'idempotent' => true,
        ],
 ...
 */
function elementor_mcp_list_directory(array $input = [])
{
    ...
    $resolved = elementor_mcp_resolve_path($path, must_exist: true);
    ...
    $all_entries = $recursive
        ? elementor_mcp_collect_recursive_entries($resolved, $pattern, $include_hidden, $max_depth)
        : elementor_mcp_collect_flat_entries($resolved, $pattern, $include_hidden);
 ...
 */
function elementor_mcp_collect_recursive_entries($resolved, $pattern, $include_hidden, $max_depth)
{
    $entries = [];
    ...
    foreach ($iterator as $item) {
        ...
        $entry = elementor_mcp_build_entry($item, $pattern, $include_hidden);
        if ($entry !== null) {
            $entries[] = $entry;
        }
    }
 ...
 */
function elementor_mcp_collect_flat_entries($resolved, $pattern, $include_hidden)
{
    ...
    while (($filename = readdir($dir_handle)) !== false) {
        ...
        $entry = elementor_mcp_build_entry($info, $pattern, $include_hidden);
        if ($entry !== null) {
            $entries[] = $entry;
        }
    }
 ...
 */
function elementor_mcp_build_entry($info, $pattern, $include_hidden)
{
