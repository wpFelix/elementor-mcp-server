 ...
 *
 * 1. The `ELEMENTOR_MCP_WP_CLI_COMMAND` constant. Either a full argv array, for example
 ...
 * 3. The `elementor_mcp_wp_cli_command` filter, which receives the resolved argv list (or null when
 *    nothing was found) and may replace it outright. This runs last, so it wins over everything.
 *
 * `ELEMENTOR_MCP_WP_CLI_PHP_INI` overrides the php.ini passed with `-c` when a phar is run through a
...
wp_register_ability('elementor-mcp/run-wp-cli', [
    'label' => __('Run WP-CLI Command', domain: 'elementor-mcp'),
    'description' => __(
    ...
    'execute_callback' => 'elementor_mcp_run_wp_cli',
...
wp_register_ability('elementor-mcp/get-wp-cli-job', [
    'label' => __('Get WP-CLI Job Status', domain: 'elementor-mcp'),
    'description' => __('Checks the status of an asynchronous background WP-CLI job.', domain: 'elementor-mcp'),
    'category' => 'code-execution',
    'input_schema' => [
        'type' => 'object',
        'properties' => [
    ...
    'execute_callback' => 'elementor_mcp_get_wp_cli_job',
    'permission_callback' => 'elementor_mcp_permission_callback',
    'meta' => [
        'show_in_rest' => true,
        'mcp' => ['public' => true],
 ...
 */
function elementor_mcp_run_wp_cli(array $input)
{
    ...
    foreach ($raw_args as $arg) {
        if (!is_string($arg)) {
            return new WP_Error('invalid_wp_cli_arg', __('WP-CLI arguments must be strings.', domain: 'elementor-mcp'));
        }
        $args[] = $arg;
    }
    ...
    $wp_command = elementor_mcp_find_wp_cli_command();
    if ($wp_command === null) {
        return new WP_Error('wp_cli_not_found', elementor_mcp_wp_cli_not_found_message(PHP_OS_FAMILY));
    }

    // Automatically append --allow-root if executing as root.
    if (elementor_mcp_is_current_user_root() && !in_array(needle: '--allow-root', haystack: $args, strict: true)) {
        array_unshift($args, '--allow-root');
    }

    if ($async) {
        return elementor_mcp_run_wp_cli_async($wp_command, $args);
    }

    return elementor_mcp_run_wp_cli_sync($wp_command, $args);
}
 ...
 * @return array|WP_Error
 */
function elementor_mcp_get_wp_cli_job(array $input)
{
    $job_id = (string) ($input['job_id'] ?? '');
    if (!preg_match('/^[a-f0-9]{16}$/i', $job_id)) {
        return new WP_Error('invalid_job_id', __('Invalid job ID format.', domain: 'elementor-mcp'));
    }

    $job_dir = elementor_mcp_get_sandbox_dir(ensure_exists: false) . 'wp-cli-jobs/';
    $log_file = $job_dir . 'job_' . $job_id . '.log';
    $status_file = $job_dir . 'job_' . $job_id . '.status';

    if (!is_file($log_file)) {
        return elementor_mcp_get_missing_wp_cli_job_result($job_id, $status_file);
    }
    ...
    $slice = elementor_mcp_read_log_slice($log_file, $offset, $limit);
 ...
 * @return array
 */
function elementor_mcp_get_missing_wp_cli_job_result(string $job_id, string $status_file): array
{
    if (!is_file($status_file)) {
        return [
            'success' => false,
 ...
 */
function elementor_mcp_read_log_slice(string $log_file, int $offset, int $limit): array|WP_Error
{
    ...
    if ($handle === false) {
        ...
        return new WP_Error('read_failed', sprintf(__('Could not open log file: %s', domain: 'elementor-mcp'), $log_file));
    }
    ...
    if ($content === false) {
        ...
        return new WP_Error('read_failed', sprintf(__('Could not read log file: %s', domain: 'elementor-mcp'), $log_file));
    }
 ...
 */
function elementor_mcp_find_wp_cli_command(): ?array
{
    $command = null;

    if (defined('ELEMENTOR_MCP_WP_CLI_COMMAND')) {
        $command = elementor_mcp_normalize_wp_cli_command(constant('ELEMENTOR_MCP_WP_CLI_COMMAND'));
    }

    if ($command === null) {
        $command = PHP_OS_FAMILY === 'Windows'
            ? elementor_mcp_find_wp_cli_command_windows()
            : elementor_mcp_find_wp_cli_command_unix();
    }
     ...
     */
    /** @var mixed $filtered */
    $filtered = apply_filters('elementor_mcp_wp_cli_command', $command);

    return elementor_mcp_normalize_wp_cli_command($filtered);
}
 ...
 */
function elementor_mcp_normalize_wp_cli_command(mixed $value): ?array
{
    if (is_string($value)) {
        $value = [$value];
    }
 ...
 * @return list<string>|null
 */
function elementor_mcp_find_wp_cli_command_unix(): ?array
{
    if (function_exists('exec')) {
        $path = elementor_mcp_first_command_output_line('which wp 2>/dev/null');
        if ($path !== null) {
            return [$path];
        }

        $path = elementor_mcp_first_command_output_line('command -v wp 2>/dev/null');
        if ($path !== null) {
            return [$path];
        }
    }
 ...
 * @return list<string>|null
 */
function elementor_mcp_find_wp_cli_command_windows(): ?array
{
    $php_binary = elementor_mcp_find_php_binary();
    $php_ini = elementor_mcp_find_wp_cli_php_ini();

    $candidates = [];
    if (function_exists('exec')) {
        foreach (['wp', 'wp.phar', 'wp-cli.phar'] as $name) {
            foreach (elementor_mcp_command_output_lines('where ' . escapeshellarg($name) . ' 2>NUL') as $line) {
                $candidates[] = $line;
            }
        }
    }
    foreach (elementor_mcp_windows_wp_cli_candidate_paths() as $path) {
        $candidates[] = $path;
    }
    ...
    foreach ($candidates as $candidate) {
        ...
        $command = elementor_mcp_wp_cli_command_for_path($candidate, $php_binary, $php_ini);
        if ($command !== null) {
            return $command;
        }
    }
 ...
 * @return list<string>|null
 */
function elementor_mcp_wp_cli_command_for_path(string $path, ?string $php_binary, ?string $php_ini): ?array
{
    ...
    if ($extension === 'bat' || $extension === 'cmd') {
        $phar = elementor_mcp_wp_cli_phar_beside_launcher($path);

        return $phar === null ? null : elementor_mcp_wp_cli_command_for_path($phar, $php_binary, $php_ini);
    }
 ...
 * @return string|null
 */
function elementor_mcp_wp_cli_phar_beside_launcher(string $launcher): ?string
{
 ...
 * @return list<string>
 */
function elementor_mcp_windows_wp_cli_candidate_paths(): array
{
    $patterns = [];
 ...
 * @return bool
 */
function elementor_mcp_is_php_cli_binary(string $path): bool
{
    if (trim($path) === '') {
        return false;
    }
 ...
 * @return string|null
 */
function elementor_mcp_find_php_binary(): ?string
{
    ...
    if (elementor_mcp_is_php_cli_binary(PHP_BINARY)) {
 ...
 * @return string|null
 */
function elementor_mcp_find_wp_cli_php_ini(): ?string
{
    if (defined('ELEMENTOR_MCP_WP_CLI_PHP_INI')) {
        /** @var mixed $configured */
        $configured = constant('ELEMENTOR_MCP_WP_CLI_PHP_INI');
        if (is_string($configured) && trim($configured) !== '') {
 ...
 * @return string
 */
function elementor_mcp_windows_command_interpreter(): string
{
 ...
 * @return string|null
 */
function elementor_mcp_first_command_output_line(string $command): ?string
{
    $output = [];
    $return_var = 0;
 ...
 * @return list<string>
 */
function elementor_mcp_command_output_lines(string $command): array
{
    $output = [];
    $return_var = 0;
    exec($command, $output, $return_var);
 ...
 * @return string
 */
function elementor_mcp_wp_cli_not_found_message(string $os_family): string
{
    if ($os_family === 'Windows') {
        return __(
            'WP-CLI was not found. Searched the PATH with "where" for wp, wp.phar and wp-cli.phar, the Local (Local by Flywheel) resource directories, the WordPress directory and its parent for wp-cli.phar. A wp.bat or wp.cmd launcher is only used when the wp-cli.phar it runs sits beside it, because starting a batch file would put command arguments through cmd.exe. Set the ELEMENTOR_MCP_WP_CLI_COMMAND constant to the full invocation, for example array( "<php.exe>", "-c", "<site php.ini>", "<wp-cli.phar>" ), or use the "elementor_mcp_wp_cli_command" filter. ELEMENTOR_MCP_WP_CLI_PHP_INI overrides only the php.ini used to run a phar.',
            domain: 'elementor-mcp',
        );
    }

    return __(
        'WP-CLI was not found. Searched the PATH with "which wp" and "command -v wp", then /usr/local/bin, /usr/bin, /bin, /usr/local/sbin and /usr/sbin. Set the ELEMENTOR_MCP_WP_CLI_COMMAND constant to the wp executable path or the full invocation, or use the "elementor_mcp_wp_cli_command" filter.',
        domain: 'elementor-mcp',
    );
}
 ...
 * @return bool
 */
function elementor_mcp_is_current_user_root(): bool
{
 ...
 * @return array
 */
function elementor_mcp_run_wp_cli_sync(array $wp_command, array $args): array
{
    ...
    if (!is_resource($process)) {
        return [
            ...
            'exit_code' => -1,
            'stdout' => '',
            'stderr' => __('Failed to start process.', domain: 'elementor-mcp'),
    ...
    [$stdout, $stderr] = elementor_mcp_collect_process_output(stdout_pipe: $pipes[1] ?? null, stderr_pipe: $pipes[2] ?? null);
 ...
 */
function elementor_mcp_collect_process_output(mixed $stdout_pipe, mixed $stderr_pipe): array
{
    ...
    while (is_resource($stdout_pipe) || is_resource($stderr_pipe)) {
        ...
        foreach ($read as $pipe) {
            elementor_mcp_append_process_pipe_output($pipe, $stdout_pipe, $stderr_pipe, $stdout, $stderr);
        }
    }
 ...
 */
function elementor_mcp_append_process_pipe_output(
    mixed $pipe,
    mixed $stdout_pipe,
    mixed $stderr_pipe,
    string &$stdout,
 ...
 * @return array
 */
function elementor_mcp_run_wp_cli_async(array $wp_command, array $args): array
{
    ...
    $job_dir = elementor_mcp_get_sandbox_dir(ensure_exists: true) . 'wp-cli-jobs/';
    if (!is_dir($job_dir)) {
        wp_mkdir_p($job_dir);
    }
    ...
    if (file_put_contents($log_file, '') === false) {
        return [
            ...
            'stderr' => sprintf(__('Failed to create log file: %s', domain: 'elementor-mcp'), $log_file),
        ];
    }

    $wp_cmd = elementor_mcp_build_wp_cli_shell_command($wp_command, $args);

    if (PHP_OS_FAMILY === 'Windows') {
        return elementor_mcp_start_wp_cli_windows_job($wp_cmd, [
            'job_id' => $job_id,
            'dir' => $job_dir,
            'log' => $log_file,
            'status' => $status_file,
    ...
    if ($rc !== 0 || $pid === null) {
        ...
        return [
            ...
            'exit_code' => $rc,
            'stdout' => '',
            'stderr' => __('Failed to start background WP-CLI process.', domain: 'elementor-mcp'),
        ];
    }
 ...
 * @return string
 */
function elementor_mcp_quote_windows_batch_path(string $path): string
{
    return (
        '"'
 ...
 * @return string
 */
function elementor_mcp_escape_windows_batch_arg(string $value): string
{
    $quoted = '"';
    $length = strlen($value);
    $index = 0;
 ...
 * @return string
 */
function elementor_mcp_build_wp_cli_shell_command(array $wp_command, array $args, ?bool $windows = null): string
{
    ...
    foreach ($wp_command as $index => $element) {
        $parts[] = $index === 0
            ? elementor_mcp_quote_windows_batch_path($element)
            : elementor_mcp_escape_windows_batch_arg($element);
    }

    return implode(' ', $parts) . ' ' . implode(' ', array_map('elementor_mcp_escape_windows_batch_arg', $args));
}
 ...
 * @return string
 */
function elementor_mcp_build_windows_job_script(string $wp_cmd, string $working_dir, string $log, string $status): string
{
    return (
        "@echo off\r\n"
        . "chcp 65001 > nul\r\n"
        ...
        . 'cd /d '
        . elementor_mcp_quote_windows_batch_path($working_dir)
        . "\r\n"
        . $wp_cmd
        . ' > '
        . elementor_mcp_quote_windows_batch_path($log)
        . " 2>&1\r\n"
        . '> '
        . elementor_mcp_quote_windows_batch_path($status)
        . " echo %ERRORLEVEL%\r\n"
    );
}
 ...
 * @return array
 */
function elementor_mcp_start_wp_cli_windows_job(string $wp_cmd, array $job): array
{
    $script_name = 'job_' . $job['job_id'] . '.cmd';
    $script_file = $job['dir'] . $script_name;
    $script = elementor_mcp_build_windows_job_script($wp_cmd, ABSPATH, $job['log'], $job['status']);
    ...
    if (file_put_contents($script_file, $script) === false) {
        return [
            ...
            'stderr' => sprintf(__('Failed to create job script: %s', domain: 'elementor-mcp'), $script_file),
        ];
    }
    ...
    $process = proc_open(
        [elementor_mcp_windows_command_interpreter(), '/c', 'start', '', '/B', $script_name],
