 ...
 * chosen result into the Media Library is `elementor-mcp/import-media-url`'s job,
...
const ELEMENTOR_MCP_OPENVERSE_ENDPOINT = 'https://api.openverse.org/v1/images/';
    ...
    'input_schema' => [
        ...
        'properties' => [
            ...
            'per_page' => [
                'type' => 'integer',
                'minimum' => 1,
                'maximum' => ELEMENTOR_MCP_OPENVERSE_MAX_PER_PAGE,
                'default' => 10,
            ],
        ],
        'required' => ['query'],
...
function wordpress_search_images(array $input): array|WP_Error
{
    ...
    if ($query === '') {
        return new WP_Error('elementor_mcp_search_query_required', __('query must not be empty.', domain: 'elementor-mcp'));
    }
    $page = max(1, (int) ($input['page'] ?? 1));
    $per_page = max(1, min(ELEMENTOR_MCP_OPENVERSE_MAX_PER_PAGE, (int) ($input['per_page'] ?? 10)));
    ...
    if ($license !== '') {
        ...
        if ($codes === []) {
            return new WP_Error('elementor_mcp_search_invalid_license', sprintf(
                /* translators: %s: the accepted licence codes. */
                __('license must be one or more of: %s.', domain: 'elementor-mcp'),
                implode(', ', $valid),
            ));
        }
    ...
    $response = wp_remote_get(add_query_arg($args, ELEMENTOR_MCP_OPENVERSE_ENDPOINT), [
        'timeout' => 15,
        'user-agent' => 'Elementor MCP/' . (defined('ELEMENTOR_MCP_VERSION') ? ELEMENTOR_MCP_VERSION : 'dev') . ' (https://elementormcp.com)',
    ]);
    if (is_wp_error($response)) {
        return new WP_Error('elementor_mcp_openverse_unreachable', sprintf(
            /* translators: %s: the transport error message. */
            __('Openverse could not be reached: %s', domain: 'elementor-mcp'),
            $response->get_error_message(),
        ));
    }
    $status = (int) wp_remote_retrieve_response_code($response);
    if ($status === 429) {
        return new WP_Error('elementor_mcp_openverse_rate_limited', __(
    ...
    if ($status !== 200) {
        return new WP_Error('elementor_mcp_openverse_error', sprintf(
            /* translators: %d: the HTTP status Openverse answered with. */
            __('Openverse answered HTTP %d.', domain: 'elementor-mcp'),
            $status,
        ));
    }
    /** @var mixed $body */
    ...
    if (!is_array($body) || !is_array($body['results'] ?? null)) {
        return new WP_Error('elementor_mcp_openverse_malformed', __(
    ...
    return [
        ...
        'provider' => 'openverse',
        'usage' => __(
            'Import a chosen result with elementor-mcp/import-media-url, carrying attribution into the caption. Attribution is required by every CC licence except CC0 and PDM.',
            domain: 'elementor-mcp',
        ),
    ];
}
