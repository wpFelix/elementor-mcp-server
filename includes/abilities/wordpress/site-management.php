...
register_core_ability('elementor-mcp/list-users', [
    'label' => __('List Users', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/get-user', [
    'label' => __('Get User', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/create-user', [
    'label' => __('Create User', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/update-user', [
    'label' => __('Update User', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/get-site-settings', [
    'label' => __('Get Site Settings', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/update-site-settings', [
    'label' => __('Update Site Settings', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/list-extensions', [
    'label' => __('List Plugins and Themes', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/list-menus', [
    'label' => __('List Navigation Menus', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/list-menu-items', [
    'label' => __('List Navigation Menu Items', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/upsert-menu-item', [
    'label' => __('Create or Update Menu Item', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/delete-menu-item', [
    'label' => __('Delete Menu Item', domain: 'elementor-mcp'),
    'description' => __(
        'Permanently deletes a classic navigation menu item. Requires explicit confirmation through Elementor MCP safety enforcement.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
function wordpress_get_user(array $input): array|WP_Error
{
    ...
    return $user instanceof WP_User
        ...
        : new WP_Error('elementor_mcp_user_not_found', __('User not found.', domain: 'elementor-mcp'));
}
...
function wordpress_create_user(array $input): array|WP_Error
{
    ...
    if (!array_key_exists($role, wp_roles()->roles)) {
        return new WP_Error('elementor_mcp_invalid_role', __('The requested role is not registered.', domain: 'elementor-mcp'));
    }
...
function wordpress_update_user(array $input): array|WP_Error
{
    ...
    if (!get_userdata($user_id) instanceof WP_User) {
        return new WP_Error('elementor_mcp_user_not_found', __('User not found.', domain: 'elementor-mcp'));
    }
    $data = ['ID' => $user_id];
    $field_map = [
        'email' => 'user_email',
    ...
    if (array_key_exists('role', $input)) {
        $role = sanitize_key((string) $input['role']);
        if (!array_key_exists($role, wp_roles()->roles)) {
            return new WP_Error('elementor_mcp_invalid_role', __(
...
function wordpress_validate_site_settings(array $input): ?WP_Error
{
    ...
    if (array_key_exists('timezone_string', $input)) {
        ...
        if ($timezone !== '' && !in_array($timezone, timezone_identifiers_list(), strict: true)) {
            return new WP_Error(
                'elementor_mcp_invalid_timezone',
    ...
    foreach (['page_on_front', 'page_for_posts'] as $key) {
        ...
        if (!$post instanceof WP_Post || $post->post_type !== 'page' || $post->post_status !== 'publish') {
            return new WP_Error(
                'elementor_mcp_invalid_reading_page',
    ...
    if ($effective['show_on_front'] === 'page' && (int) $effective['page_on_front'] === 0) {
        return new WP_Error(
            'elementor_mcp_front_page_required',
    ...
    if ((int) $effective['page_on_front'] > 0 && $effective['page_on_front'] === $effective['page_for_posts']) {
        return new WP_Error(
            'elementor_mcp_reading_pages_conflict',
...
function wordpress_list_menu_items(array $input): array|WP_Error
{
    ...
    if ($menu === false) {
        return new WP_Error('elementor_mcp_menu_not_found', __('Navigation menu not found.', domain: 'elementor-mcp'));
    }
...
function wordpress_upsert_menu_item(array $input): array|WP_Error
{
    ...
    if ($item_id > 0) {
        ...
        if (!$item instanceof WP_Post || $item->post_type !== 'nav_menu_item') {
            return new WP_Error(
                'elementor_mcp_menu_item_not_found',
                __('Navigation menu item not found.', domain: 'elementor-mcp'),
                ['status' => 404],
            );
        }
        ...
        if ($membership !== true) {
            return new WP_Error(
                'elementor_mcp_menu_item_wrong_menu',
                __('The navigation menu item does not belong to the requested menu.', domain: 'elementor-mcp'),
    ...
    if ($parent_id > 0) {
        if ($parent_id === $item_id) {
            return new WP_Error(
                'elementor_mcp_menu_item_parent_invalid',
                __('A navigation menu item cannot be its own parent.', domain: 'elementor-mcp'),
                ['status' => 422],
            );
        }
        ...
        if ($parent_membership !== true) {
            return new WP_Error(
                'elementor_mcp_menu_item_parent_wrong_menu',
                __('The parent navigation item does not belong to the requested menu.', domain: 'elementor-mcp'),
...
function wordpress_delete_menu_item(array $input): array|WP_Error
{
    ...
    $item = get_post($item_id);
    if (!$item instanceof WP_Post || $item->post_type !== 'nav_menu_item') {
        return new WP_Error('elementor_mcp_menu_item_not_found', __(
            'Navigation menu item not found.',
            domain: 'elementor-mcp',
        ));
    }
    if (wp_delete_post($item_id, force_delete: true) === false) {
        return new WP_Error('elementor_mcp_menu_item_delete_failed', __(
