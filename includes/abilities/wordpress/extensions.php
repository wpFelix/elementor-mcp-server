...
register_core_ability('elementor-mcp/search-extensions', [
    'label' => __('Search Plugins and Themes', domain: 'elementor-mcp'),
    'description' => __(
        'Searches the WordPress.org directory for plugins or themes and returns slug, name, version, author, rating, active-install count, last update, and the WordPress/PHP versions each one requires. Read-only: it installs nothing. Call this before elementor-mcp/install-plugin or elementor-mcp/install-theme to resolve a human name ("a contact form plugin") into the exact slug those abilities need, and check the returned `requires` and `requires_php` against elementor-mcp/system-status before installing.',
...
register_core_ability('elementor-mcp/get-extension', [
    'label' => __('Get Plugin or Theme', domain: 'elementor-mcp'),
    'description' => __(
        'Returns one plugin or theme in detail: the installed copy (version, active state, network activation, update availability, plugin file path) and, when the slug exists on WordPress.org, the directory record (latest version, requirements, last update, homepage). Read-only. Use it to confirm a slug before installing, to read the exact `file` value that elementor-mcp/activate-plugin needs, or to check whether an update is actually available before calling elementor-mcp/update-plugin.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
register_core_ability('elementor-mcp/deactivate-plugin', [
    'label' => __('Deactivate Plugin', domain: 'elementor-mcp'),
    'description' => __(
        'Deactivates an active plugin by its plugin file. Deactivation removes whatever the plugin provides — shortcodes stop rendering, blocks disappear from saved content, integrations go dark — so it requires explicit confirmation. Elementor MCP and Elementor MCP Pro cannot be deactivated through this ability: doing so would sever the connection carrying the call. The change is recorded in the change ledger and can be rolled back.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
register_core_ability('elementor-mcp/update-plugin', [
    'label' => __('Update Plugin', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/update-theme', [
    'label' => __('Update Theme', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/switch-theme', [
    'label' => __('Switch Theme', domain: 'elementor-mcp'),
    'description' => __(
...
register_core_ability('elementor-mcp/install-plugin', [
    'label' => __('Install Plugin', domain: 'elementor-mcp'),
    'description' => __(
        'Downloads and installs a plugin from the WordPress.org directory by slug, or from an explicit HTTPS ZIP URL. Writes executable code to the server, so it is a critical operation: blocked by Production Safe and Read Only, available in Developer Full Access with explicit confirmation. The plugin is left INACTIVE — review it, then call elementor-mcp/activate-plugin separately. Resolve the slug with elementor-mcp/search-extensions first. Not reversible through the change ledger; remove it with elementor-mcp/delete-plugin.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
register_core_ability('elementor-mcp/install-theme', [
    'label' => __('Install Theme', domain: 'elementor-mcp'),
    'description' => __(
        'Downloads and installs a theme from the WordPress.org directory by slug, or from an explicit HTTPS ZIP URL. Writes code to the server, so it is a critical operation: blocked by Production Safe and Read Only, available in Developer Full Access with explicit confirmation. The theme is installed but NOT activated — call elementor-mcp/switch-theme separately once the site is ready for it. Not reversible through the change ledger; remove it with elementor-mcp/delete-theme.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
register_core_ability('elementor-mcp/delete-plugin', [
    'label' => __('Delete Plugin', domain: 'elementor-mcp'),
    'description' => __(
        'Permanently deletes an installed plugin\'s files from the server. Critical and irreversible: blocked by Production Safe and Read Only, available in Developer Full Access with explicit confirmation. The plugin must be deactivated first — deactivate it with elementor-mcp/deactivate-plugin, confirm the site still behaves, then delete. Data the plugin stored in the database is not removed unless the plugin ships an uninstall routine that WordPress runs.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
register_core_ability('elementor-mcp/delete-theme', [
    'label' => __('Delete Theme', domain: 'elementor-mcp'),
    'description' => __(
        'Permanently deletes an installed theme\'s files from the server. Critical and irreversible: blocked by Production Safe and Read Only, available in Developer Full Access with explicit confirmation. The active theme, and the parent of the active theme, are refused — switch away with elementor-mcp/switch-theme first.',
        domain: 'elementor-mcp',
    ),
    'category' => 'wordpress',
    'input_schema' => [
...
function wordpress_search_extensions(array $input): array|WP_Error
{
    ...
    if (!is_object($response)) {
        return new WP_Error('elementor_mcp_directory_unavailable', __(
...
function wordpress_get_extension(array $input): array|WP_Error
{
    ...
    if ($result['installed'] === null && $result['directory'] === null) {
        return new WP_Error(
            'elementor_mcp_extension_not_found',
            sprintf(
                /* translators: %s: the requested plugin or theme slug */
                __('"%s" is neither installed on this site nor listed on WordPress.org.', domain: 'elementor-mcp'),
                $slug,
            ),
            ['status' => 404],
        );
...
function wordpress_deactivate_plugin(array $input): array|WP_Error
{
    ...
    if (is_plugin_active($file)) {
        return new WP_Error(
            'elementor_mcp_plugin_still_active',
            __('WordPress reported no error but the plugin is still active.', domain: 'elementor-mcp'),
        );
    }
...
function wordpress_switch_theme(array $input): array|WP_Error
{
    ...
    if (!$theme->exists()) {
        return new WP_Error(
            'elementor_mcp_theme_not_found',
            sprintf(
                /* translators: %s: the requested theme stylesheet directory */
                __('Theme "%s" is not installed.', domain: 'elementor-mcp'),
                $stylesheet,
            ),
            ['status' => 404],
        );
    ...
    if ($errors instanceof WP_Error) {
        return new WP_Error(
            'elementor_mcp_theme_broken',
            sprintf(
                /* translators: 1: theme stylesheet directory, 2: the underlying WordPress error */
                __('Theme "%1$s" cannot be activated: %2$s', domain: 'elementor-mcp'),
                $stylesheet,
    ...
    if ($now !== $stylesheet) {
        return new WP_Error(
            'elementor_mcp_theme_switch_failed',
            __('WordPress did not switch the active theme.', domain: 'elementor-mcp'),
        );
    }
...
function wordpress_update_plugin(array $input): array|WP_Error
{
    ...
    $failure = wordpress_upgrader_error($result, $skin, __('plugin update', domain: 'elementor-mcp'));
    if ($failure !== null) {
        return $failure;
    }
    ...
    // A single-plugin upgrade can leave the plugin inactive on some WordPress
    // and filesystem combinations. Updating an active dependency must preserve
    // its activation state or an otherwise successful upgrade can take the
    // feature (or the whole integration) offline without reporting a failure.
    if (($was_active && !is_plugin_active($file)) || ($was_network_active && !is_plugin_active_for_network($file))) {
        $reactivated = activate_plugin(
            $file,
            redirect: '',
        ...
        if (is_wp_error($reactivated)) {
            return new WP_Error(
                'elementor_mcp_plugin_reactivation_failed',
                sprintf(
                    /* translators: 1: plugin file, 2: activation error */
                    __('Plugin "%1$s" was updated but could not be reactivated: %2$s', domain: 'elementor-mcp'),
                    $file,
                    $reactivated->get_error_message(),
                ),
                ['status' => 500],
            );
        }
...
function wordpress_update_theme(array $input): array|WP_Error
{
    ...
    $failure = wordpress_upgrader_error($result, $skin, __('theme update', domain: 'elementor-mcp'));
    if ($failure !== null) {
        return $failure;
    }
...
function wordpress_install_plugin(array $input): array|WP_Error
{
    ...
    $failure = wordpress_upgrader_error($result, $skin, __('plugin install', domain: 'elementor-mcp'));
    if ($failure !== null) {
        return $failure;
    }
    ...
    return [
        ...
        'file' => $file,
        'active' => false,
        'next_step' => 'Review the plugin, then call elementor-mcp/activate-plugin with this "file" value.',
...
function wordpress_install_theme(array $input): array|WP_Error
{
    ...
    $failure = wordpress_upgrader_error($result, $skin, __('theme install', domain: 'elementor-mcp'));
    if ($failure !== null) {
        return $failure;
    }
    ...
    return [
        ...
        'stylesheet' => $stylesheet,
        'active' => false,
        'next_step' => 'Call elementor-mcp/switch-theme with this "stylesheet" value when the site is ready for it.',
...
function wordpress_delete_plugin(array $input): array|WP_Error
{
    ...
    if (is_plugin_active($file) || is_plugin_active_for_network($file)) {
        return new WP_Error(
            'elementor_mcp_plugin_still_active',
            __(
                'This plugin is active. Deactivate it with elementor-mcp/deactivate-plugin and confirm the site still behaves before deleting its files.',
                domain: 'elementor-mcp',
            ),
            ['status' => 409],
        );
    ...
    if ($deleted !== true) {
        return new WP_Error('elementor_mcp_plugin_delete_failed', __(
...
function wordpress_delete_theme(array $input): array|WP_Error
{
    ...
    if ($stylesheet === get_stylesheet() || $stylesheet === get_template()) {
        return new WP_Error(
            'elementor_mcp_theme_in_use',
            __(
                'This theme is in use — it is the active theme or the parent of the active child theme. Switch to another theme with elementor-mcp/switch-theme before deleting it.',
                domain: 'elementor-mcp',
            ),
            ['status' => 409],
        );
    ...
    if ($deleted !== true) {
        return new WP_Error('elementor_mcp_theme_delete_failed', __(
...
{
    return new WP_Error(
        'elementor_mcp_invalid_extension_slug',
        sprintf(
...
function wordpress_plugin_file(string $value): string|WP_Error
{
    ...
    if ($file === '' || validate_file($file) !== 0 || !str_ends_with(strtolower($file), '.php')) {
        return new WP_Error(
            'elementor_mcp_invalid_plugin_file',
            __(
                'The plugin file must be the "directory/file.php" identifier reported by elementor-mcp/list-extensions, with no path traversal.',
                domain: 'elementor-mcp',
            ),
            ['status' => 422],
        );
    ...
    if (!array_key_exists($file, get_plugins())) {
        return new WP_Error(
            'elementor_mcp_plugin_not_found',
            sprintf(
                /* translators: %s: the requested plugin file identifier */
                __('Plugin "%s" is not installed on this site.', domain: 'elementor-mcp'),
                $file,
            ),
            ['status' => 404],
        );
 ...
 *
 * Deactivating or deleting Elementor MCP mid-request leaves the agent with no way to
...
{
    $own = [];
    if (defined('ELEMENTOR_MCP_PLUGIN_FILE')) {
        $own[] = plugin_basename((string) constant('ELEMENTOR_MCP_PLUGIN_FILE'));
    }
    if (defined('ELEMENTOR_MCP_PRO_FILE')) {
        $own[] = plugin_basename((string) constant('ELEMENTOR_MCP_PRO_FILE'));
    }
    ...
    return new WP_Error(
        'elementor_mcp_cannot_target_self',
        $deleting
            ? __(
                'Elementor MCP cannot delete itself: the request is being served by this plugin. Remove it from the Plugins screen in wp-admin instead.',
                domain: 'elementor-mcp',
            )
            : __(
                'Elementor MCP cannot deactivate itself: doing so would sever the connection carrying this call. Deactivate it from the Plugins screen in wp-admin instead.',
                domain: 'elementor-mcp',
            ),
        ['status' => 409],
    );
...
function wordpress_network_scope_error(bool $network_wide): ?WP_Error
{
    ...
    if (!is_multisite()) {
        return new WP_Error(
            'elementor_mcp_not_multisite',
            __('network_wide only applies to a multisite network.', domain: 'elementor-mcp'),
            ['status' => 422],
        );
    }
    if (!current_user_can('manage_network_plugins')) {
        return new WP_Error(
            'elementor_mcp_cannot_manage_network_plugins',
            __('Network-wide activation requires network administrator rights.', domain: 'elementor-mcp'),
            ['status' => 403],
        );
    }
...
function wordpress_file_mod_error(string $context): ?WP_Error
{
    ...
    return new WP_Error(
        'elementor_mcp_file_mods_disallowed',
        __(
 ...
 *
 * Elementor MCP runs headless: `request_filesystem_credentials()` has no screen to
...
function wordpress_filesystem_error(): ?WP_Error
{
    ...
    if ($method !== 'direct') {
        return new WP_Error(
            'elementor_mcp_filesystem_not_direct',
            sprintf(
    ...
    if (!WP_Filesystem()) {
        return new WP_Error(
            'elementor_mcp_filesystem_unavailable',
            __('WordPress could not initialise its filesystem API.', domain: 'elementor-mcp'),
            ['status' => 409],
        );
    }
...
function wordpress_install_source(array $input, bool $is_theme): array|WP_Error
{
    ...
    if ($has_slug === $has_zip) {
        return new WP_Error(
            'elementor_mcp_install_source_ambiguous',
            __('Provide exactly one of "slug" (WordPress.org) or "zip_url" (an HTTPS package URL).', domain: 'elementor-mcp'),
            ['status' => 422],
        );
    }
    ...
    if ($has_zip) {
        ...
        if (strtolower((string) wp_parse_url($url, PHP_URL_SCHEME)) !== 'https') {
            return new WP_Error(
                'elementor_mcp_zip_url_not_https',
                __('A package URL must use HTTPS.', domain: 'elementor-mcp'),
                ['status' => 422],
            );
        }
        if (esc_url_raw($url) === '') {
            return new WP_Error(
                'elementor_mcp_zip_url_invalid',
                __('The package URL is not a valid URL.', domain: 'elementor-mcp'),
                ['status' => 422],
            );
        }
    ...
    if ($download === '') {
        return new WP_Error(
            'elementor_mcp_no_download_link',
            sprintf(
                /* translators: %s: the requested slug */
                __('WordPress.org returned no download package for "%s".', domain: 'elementor-mcp'),
                $slug,
            ),
            ['status' => 404],
        );
    ...
    if ($result === false || $result === null) {
        return new WP_Error(
            ...
            sprintf(
                ...
                __('The %s did not complete and WordPress reported no reason.', domain: 'elementor-mcp'),
                $operation,
            ),
        );
    }
