...
register_core_ability('elementor-mcp/update-post', [
    'label' => __('Update Post', domain: 'elementor-mcp'),
    'description' => __(
        'Updates an existing WordPress post of any post type with partial-update semantics. Use this ability for status, title/slug, featured-image, and ordinary-meta changes. Leave content empty when Elementor owns the page body and use Elementor content abilities for layout changes. Identifies the target via `post_id` (short alias: `id`). Accepts both short names (title, slug, status, content, excerpt, parent, author, date) and WordPress-native aliases (post_title, post_name, post_status, post_content, post_excerpt, post_parent, post_author, post_date).',
    ...
    'input_schema' => [
        ...
        'properties' => [
            ...
            'status' => [
                ...
                'description' => 'New post status. Goes through wp_update_post so transition_post_status hooks fire for WordPress and Elementor integrations.',
            ],
            'content' => [
                'type' => 'string',
                'description' => 'New post_content. A non-empty value on an Elementor-owned post requires the explicit allow_raw_content_on_builder_post handshake because Elementor renders its stored document instead. Elementor storage metadata is always rejected.',
            ],
            'allow_raw_content_on_builder_post' => [
                'type' => 'boolean',
                'default' => false,
                'description' => 'Set true ONLY after the user has explicitly confirmed they want raw post_content on an Elementor-owned post — where it changes nothing visitors see. Never set it on your own initiative; use Elementor abilities to change the page.',
            ],
            'excerpt' => [
                'type' => 'string',
            ...
            'meta' => [
                'type' => 'object',
                'description' => 'Map of ordinary meta_key => meta_value to upsert via update_post_meta. Only the keys you pass are touched. Elementor storage keys (_elementor_data, _elementor_edit_mode, and related protected keys) are REJECTED — use Elementor abilities instead.',
            ],
            'featured_image_id' => [
                'type' => 'integer',
    ...
    'meta' => [
        ...
        'mcp' => ['public' => true],
        'annotations' => [
            'instructions' => 'Partial update — only fields you pass are touched. Common uses: flip a draft to publish (status="publish"), set a featured image (featured_image_id=<attachment id>), or set ordinary post meta. Elementor storage metadata is rejected. Non-empty post_content on an Elementor-owned post requires allow_raw_content_on_builder_post:true only after explicit user confirmation; use Elementor content abilities for the page instead.',
            'readonly' => false,
            'destructive' => false,
            'idempotent' => true,
        ],
...
function wordpress_update_post(array $input): array|WP_Error
{
    ...
    // Elementor leaves post_content looking ordinary, so the marker guard above
    // cannot see ownership. Without this, the write is accepted, stored, and
...
function wordpress_update_post_core_fields(int $post_id, array $input, array &$updated_fields): ?WP_Error
{
    ...
    foreach ($string_fields as $input_key => $wp_key) {
        ...
        // literal backslashes in JSON or other structured content. Slash so
