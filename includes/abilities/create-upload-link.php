...
wp_register_ability('elementor-mcp/create-upload-link', [
    ...
    'execute_callback' => 'elementor_mcp_create_upload_link',
    ...
    'meta' => [
        ...
        'annotations' => [
            'instructions' => implode("\n", [
                'Use this instead of elementor-mcp/write-file for ZIPs, plugins, themes, media, binary files, base64-encoded payloads, or any large upload.',
                ...
                'PHP files (*.php) and PHP execution control files can ONLY be uploaded to wp-content/elementor-mcp-sandbox/.',
 ...
 */
function elementor_mcp_create_upload_link($input)
{
    ...
    $token = elementor_mcp_sign_upload_payload($payload);
    if (is_wp_error($token)) {
        return $token;
    }

    $upload_url = rest_url('elementor-mcp/v1/upload');
    $token_header = 'X-Elementor MCP-Upload-Token';
