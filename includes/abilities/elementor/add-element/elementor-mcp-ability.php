...
function elementor_add_element(array $input): array
{
    ...
    if (in_array($parsed['element_type'], ELEMENTOR_MCP_ATOMIC_CONTAINER_TYPES, strict: true)) {
...
function el_add_widget_element(array $parsed): array
{
    ...
    if ($schema === null) {
        return [
            'success' => false,
            'error' => sprintf(
                'Unknown widget_type "%s". It is not registered with Elementor on this site. Use elementor-mcp/elementor-get-schema action:"list" to enumerate every available widget.',
                $widget_type,
            ),
...
{
    $schema = el_extract_schema(ELEMENTOR_MCP_COMPACT_SCHEMA_CONTAINER_KEY);
    if ($schema === null) {
        return [
            'success' => false,
    ...
    $validation = el_validate_settings($parsed['settings'], $schema);
    if (!$validation['ok']) {
        return build_single_element_error_response('container', ELEMENTOR_MCP_COMPACT_SCHEMA_CONTAINER_KEY, $validation);
    }
