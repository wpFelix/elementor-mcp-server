...
function el_build_styling_hint(array $misplaced_style_props): array
{
    ...
    $message = sprintf(
        'On atomic (v4) elements, layout/spacing/typography live in the `styles` map on the element, not in `settings`. Move [%s] into a per-element style like the example, then retry. Call elementor-mcp/elementor-get-style-schema properties:["%s"] for the exact value shape.',
        implode(', ', $misplaced_style_props),
        $first,
    );
    ...
    return [
        'message' => $message,
        'example' => $example,
        'reference' => 'elementor-mcp/elementor-get-style-schema',
    ];
}
