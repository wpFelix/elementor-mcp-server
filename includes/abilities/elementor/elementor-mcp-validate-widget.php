...
define('ELEMENTOR_MCP_VALID_PROP_TYPES', [
    'image',
    'string',
    'link',
    'html',
 ...
 */
define('ELEMENTOR_MCP_DEFAULTLESS_PROP_TYPES', [
    'string',
    'email',
    'textarea',
    'select',
...
define('ELEMENTOR_MCP_RESERVED_PROP_NAMES', [
    'classes',
    'attributes',
    '_cssid',
    'tag',
...
define('ELEMENTOR_MCP_TWIG_BLOCKED_PATTERNS', [
    '{% include',
    '{% extends',
    '{% import',
    '{% embed',
...
define('ELEMENTOR_MCP_JS_BLOCKED_PATTERNS', [
    '<script',
    '</script',
    'document.cookie',
    'localStorage',
...
function vwp_validate_prop_name(string $prefix, array $prop, array &$seen_names, array &$errors): string
{
    ...
    if (in_array($prop_name, ELEMENTOR_MCP_RESERVED_PROP_NAMES, strict: true)) {
...
function vwp_validate_prop_entry(
    ...
    if ($prop_type !== '' && !in_array($prop_type, ELEMENTOR_MCP_VALID_PROP_TYPES, strict: true)) {
        $errors[] =
            "{$prefix}.type must be one of: " . implode(', ', ELEMENTOR_MCP_VALID_PROP_TYPES) . ". Got '{$prop_type}'.";
    }
    ...
    if ($default === null && in_array($prop_type, ELEMENTOR_MCP_DEFAULTLESS_PROP_TYPES, strict: true)) {
        $warnings[] = sprintf(
...
function validate_widget_twig(string $twig, array $props): array
{
    ...
    foreach (ELEMENTOR_MCP_TWIG_BLOCKED_PATTERNS as $pattern) {
...
function validate_widget_js(string $js): array
{
    ...
    foreach (ELEMENTOR_MCP_JS_BLOCKED_PATTERNS as $pattern) {
