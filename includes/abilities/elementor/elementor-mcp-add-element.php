 ...
 *
 *   elementor-mcp-ability.php                   entry points and the shared insert-and-write path
 *   elementor-mcp-input-parsing.php             parsing and validating ability input
 *   elementor-mcp-boxed-containers.php          splitting a boxed container into outer and inner
 *   elementor-mcp-v3-container-translation.php  translating v3 container settings to v4
 *   elementor-mcp-style-variants.php            style variants across breakpoints
 *   elementor-mcp-style-classes.php             style class ids on an element
 *   elementor-mcp-dynamic-tags.php              dynamic tag values in element settings
 *   elementor-mcp-error-responses.php           building the error response
 *
...
/*
 * The registration lives here rather than in add-element/elementor-mcp-ability.php
 ...
 * call any of it, while elementor-mcp/elementor-get-schema and
 * elementor-mcp/elementor-edit-element both told agents to use it by name.
 */
...
require_once __DIR__ . '/add-element/elementor-mcp-ability.php';
require_once __DIR__ . '/add-element/elementor-mcp-input-parsing.php';
require_once __DIR__ . '/add-element/elementor-mcp-boxed-containers.php';
require_once __DIR__ . '/add-element/elementor-mcp-v3-container-translation.php';
require_once __DIR__ . '/add-element/elementor-mcp-style-variants.php';
require_once __DIR__ . '/add-element/elementor-mcp-style-classes.php';
require_once __DIR__ . '/add-element/elementor-mcp-dynamic-tags.php';
require_once __DIR__ . '/add-element/elementor-mcp-error-responses.php';

wp_register_ability('elementor-mcp/elementor-add-element', [
    'label' => __('Add Elementor Element', domain: 'elementor-mcp'),
    'description' => __(
        'Inserts a new widget or container into an Elementor document, or grafts a pre-built subtree onto it. Use this instead of reading the whole document, editing the tree by hand and writing it back with elementor-mcp/elementor-set-content: only the new element is described, ids are generated, and the rest of the page is untouched. Four shapes: element_type="widget" with widget_type (a v3 widget such as heading, or a v4 atomic one such as e-heading); element_type="container" for a v3 container; element_type="e-flexbox" or "e-div-block" for a v4 atomic container; or tree={...} to insert a whole prepared subtree, whose ids are regenerated so it can be inserted repeatedly without collisions. parent_id selects the container to insert into and defaults to the document root; position is a zero-based index among that parent\'s children and defaults to the end. element_id sets a semantic id, which is what per-element style classes are named after, so it is worth setting on anything you intend to style later. Validation runs server-side: an unknown control id or an invalid enum value aborts the insert and returns the compact schema of the affected widget inline, so a correction is one roundtrip away.',
    ...
    'execute_callback' => 'ElementorMCP\Abilities\Elementor\elementor_add_element',
