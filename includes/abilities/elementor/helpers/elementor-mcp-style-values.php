...
const ELEMENTOR_MCP_STYLE_SCHEMA_CLASS = 'Elementor\\Modules\\AtomicWidgets\\Styles\\Style_Schema';
...
function el_validate_style_props(array $styles): array
{
    ...
    if ($unknown !== [] || $invalid !== []) {
        ...
        $error = [
            'error' => sprintf(
                'Invalid style payload — %s. See "invalid_values"/"unknown_properties" for per-property details; call elementor-mcp/elementor-get-style-schema only if you need the full list of valid CSS properties.',
                implode('; ', $parts),
            ),
        ];
        if ($unknown !== []) {
...
function el_validate_single_style_prop(string $property, mixed $value, object $prop_type): ?array
{
    ...
    if (!method_exists($prop_type, 'validate') || !$prop_type->validate($value)) {
        ...
        return [
            ...
            'expected_types' => $expected_types,
            'reason' => sprintf(
                'Value does not match. Got $$type="%s", expected one of: %s. Call elementor-mcp/elementor-get-style-schema for the exact shape of "%s".',
                $received_type,
