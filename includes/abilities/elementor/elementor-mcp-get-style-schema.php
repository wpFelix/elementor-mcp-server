...
wp_register_ability('elementor-mcp/elementor-get-style-schema', [
    'label' => __('Get Elementor v4 Style Schema', domain: 'elementor-mcp'),
    'description' => __(
        'Returns the authoritative Elementor v4 atomic-widgets Style Schema — CSS property → prop-type descriptor. v4-ONLY: governs Global Classes, atomic widget base styles, and style variants. Does NOT apply to legacy v3 widgets (use elementor-mcp/elementor-get-schema for those). Progressive disclosure: the full schema is ~35KB (60+ properties); prefer `overview:true` for a flat `{property: prop_kind}` discovery view, or `properties:["gap","padding",...]` to get detailed descriptors only for the props you plan to write. Per property, lists the prop kind (`size`, `color`, `string`, `number`, `union`, `dimensions`, `background`, `box-shadow`, …), the enum values when applicable, and for union / object props the nested alternatives.',
    ...
    'execute_callback' => 'ElementorMCP\Abilities\Elementor\elementor_get_style_schema',
...
{
    if (!class_exists(ELEMENTOR_MCP_STYLE_SCHEMA_CLASS)) {
        return [
            'success' => false,
