 ...
 * public schema ability is `elementor-mcp/elementor-get-schema` in
 * `elementor-mcp-get-widget-elementor-mcp-schema.php`, which delegates into the helpers below.
 *
 ...
 */
const ELEMENTOR_MCP_COMPACT_SCHEMA_SKIP_TYPES = [
    'section',
    'divider',
    'heading',
    'raw_html',
 ...
 */
const ELEMENTOR_MCP_COMPACT_SCHEMA_STYLE_TABS = [
    'style',
    'advanced',
    'dce_visibility',
];
 ...
 */
const ELEMENTOR_MCP_COMPACT_SCHEMA_ARRAY_TYPES = [
    'select2',
    'multiselect',
    'checkbox_list',
    'gallery',
 ...
 */
const ELEMENTOR_MCP_COMPACT_SCHEMA_CONTAINER_KEY = '__container__';
 ...
 */
const ELEMENTOR_MCP_ATOMIC_CONTAINER_TYPES = ['e-flexbox', 'e-div-block'];
...
{
    if ($widget_type === ELEMENTOR_MCP_COMPACT_SCHEMA_CONTAINER_KEY) {
        return build_container_compact_schema($opts);
    }

    if (in_array($widget_type, ELEMENTOR_MCP_ATOMIC_CONTAINER_TYPES, strict: true)) {
...
function build_container_compact_schema(array $opts): ?array
{
    ...
    return [
        'widgetType' => ELEMENTOR_MCP_COMPACT_SCHEMA_CONTAINER_KEY,
        'label' => 'Container',
...
{
    if (in_array($type, ELEMENTOR_MCP_COMPACT_SCHEMA_SKIP_TYPES, strict: true)) {
        return false;
    }
    ...
    return !in_array($tab, ELEMENTOR_MCP_COMPACT_SCHEMA_STYLE_TABS, strict: true);
}
...
{
    if (in_array($type, ELEMENTOR_MCP_COMPACT_SCHEMA_ARRAY_TYPES, strict: true)) {
        return true;
    }
...
function build_compact_description(\Elementor\Widget_Base $widget): string
{
    ...
    // Every elementor-mcp-generated widget carries the hard-coded 'elementor-mcp' keyword; on
    // its own it's pure noise in the discovery surface, so treat it as no keywords.
    if ($keywords === [] || $keywords === ['elementor-mcp']) {
        return $title . ' widget';
    }
