...
wp_register_ability('elementor-mcp/elementor-edit-element', [
    ...
    'execute_callback' => 'ElementorMCP\Abilities\Elementor\elementor_edit_element',
...
function el_validate_update_styles(array $target, array $styles): array
{
    ...
    $message = sprintf(
        'Invalid styles for widget "%s": %d style entry error(s). Each style must be {id, type, label, variants:[{meta:{breakpoint,state}, props:{CSS_PROP: {$$type, value}, ...}}]}. Use elementor-mcp/elementor-get-style-schema to inspect the prop shapes.',
        $widget_type,
        count($result['errors']),
    );
...
function el_expand_ergonomic_styles_for_target(array $target, array $styles, string $widget_type): array
{
    ...
    if (array_key_exists('error', $props_validation)) {
        ...
        $error_response['error'] = sprintf(
            'Invalid ergonomic styles for widget "%s" — %s. See "unknown_style_properties"/"invalid_style_values" for per-property details; call elementor-mcp/elementor-get-style-schema only if you need the full list of valid CSS properties.',
            $widget_type,
