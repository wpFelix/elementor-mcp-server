 ...
 * `update_settings()`, for the reason recorded in elementor-mcp-manage-global-styles-v3.php:
...
function el_require_settings_document(int $post_id): object
{
    if (!class_exists('Elementor\\Plugin')) {
        return new WP_Error(
            'elementor_required',
            __('Elementor is not active on this site.', domain: 'elementor-mcp'),
            ['status' => 400],
        );
    }
    ...
    if (!is_object($document) || !method_exists($document, 'get_controls')) {
        return new WP_Error(
            ...
            sprintf(
                /* translators: %d: post id */
                __('Elementor could not open post %d as a document. Only Elementor-enabled posts have page settings.', domain: 'elementor-mcp'),
                $post_id,
            ),
            ['status' => 422],
        );
...
function el_page_setting_controls(object $document): array
{
    ...
    if (!array_key_exists('custom_css', $out) && class_exists('ElementorPro\\Modules\\CustomCss\\Module')) {
        $out['custom_css'] = [
            'type' => 'code',
            'label' => __('Custom CSS', domain: 'elementor-mcp'),
            'options' => [],
            'default' => '',
        ];
    }
...
wp_register_ability('elementor-mcp/elementor-get-page-settings', [
    'label' => __('Get Elementor Page Settings', domain: 'elementor-mcp'),
    'description' => __(
    ...
    'execute_callback' => 'ElementorMCP\\Abilities\\Elementor\\elementor_get_page_settings',
...
wp_register_ability('elementor-mcp/elementor-set-page-settings', [
    ...
    'execute_callback' => 'ElementorMCP\\Abilities\\Elementor\\elementor_set_page_settings',
    'permission_callback' => 'elementor_mcp_permission_callback',
    'meta' => [
        'show_in_rest' => true,
        'mcp' => ['public' => true, 'type' => 'tool'],
        'annotations' => [
            'instructions' => 'Read elementor-mcp/elementor-get-page-settings first for the settings this document type accepts — they differ between a page, a header and a popup.',
            'readonly' => false,
            'destructive' => false,
            'idempotent' => true,
        ],
...
function elementor_set_page_settings(array $input): array|WP_Error
{
    ...
    if (!is_array($supplied) || $supplied === []) {
        return new WP_Error(
            ...
            __('settings must be a non-empty object of setting name to value.', domain: 'elementor-mcp'),
            ['status' => 422],
        );
    }
    ...
    if ($controls === []) {
        return new WP_Error(
            ...
            sprintf(
                /* translators: %d: post id */
                __('Elementor exposed no page settings for document %d.', domain: 'elementor-mcp'),
                $post_id,
            ),
            ['status' => 500],
        );
        ...
        if (!array_key_exists($key, $controls)) {
            return new WP_Error(
                ...
                sprintf(
                    ...
                    __('"%1$s" is not a page setting on this document. Valid names: %2$s.', domain: 'elementor-mcp'),
                    $key,
            ...
            if (!in_array((string) $value, $valid, strict: true)) {
                return new WP_Error(
                    ...
                    sprintf(
                        ...
                        __('"%1$s" is not a page template on this site. Valid values: %2$s.', domain: 'elementor-mcp'),
                        (string) $value,
                        implode(', ', $valid),
                    ),
            ...
            if (!in_array((string) $value, $options, strict: true)) {
                return new WP_Error(
                    ...
                    sprintf(
                        ...
                        __('"%1$s" is not a valid value for %2$s. Valid values: %3$s.', domain: 'elementor-mcp'),
                        (string) $value,
                        $key,
                        implode(', ', $options),
                    ),
    ...
    if (!method_exists($document, 'save')) {
        return new WP_Error(
            'elementor_page_settings_unavailable',
            __('This Elementor document cannot be saved.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
    ...
    if ($saved === false) {
        return new WP_Error(
            ...
            sprintf(
                /* translators: %d: post id */
                __('Elementor refused to save page settings for document %d.', domain: 'elementor-mcp'),
                $post_id,
            ),
            ['status' => 500],
        );
    ...
    if (array_key_exists('template', $supplied) && $fresh['template'] !== (string) $supplied['template']) {
        ...
        $warnings[] = sprintf(
            ...
            __('The page layout could not be set to "%1$s"; it is still "%2$s".', domain: 'elementor-mcp'),
