 ...
 *
 * `elementor-mcp/elementor-get-schema` answers with everything, which for a heading
 ...
 * aside and under which rule, and `elementor-mcp/elementor-get-schema` still returns
...
wp_register_ability('elementor-mcp/elementor-get-widget-params', [
    'label' => __('Get Elementor Widget Params', domain: 'elementor-mcp'),
    'description' => __(
        'Returns the settings of an Elementor widget that a build actually sets — the heading\'s text, link, tag and size — instead of the complete control list, which for one heading is 186 controls and around 22kb of motion effects, sticky rules, transform popovers, hover backgrounds and per-breakpoint duplicates. Use this before adding or editing a widget; reach for elementor-mcp/elementor-get-schema when you need a control this view sets aside, such as a motion effect or a per-device override. Works on any registered widget, including ones an addon pack installed, because the rules read the live registry rather than a fixed catalog. The response says how many controls were set aside and why, so nothing disappears quietly.',
    ...
    'input_schema' => [
        ...
        'properties' => [
            'widget_types' => [
                ...
                'minItems' => 1,
                'maxItems' => 20,
                'description' => 'Widget types to describe, e.g. ["heading", "button"]. Names come from elementor-mcp/elementor-get-schema with action "list".',
            ],
            'include_styles' => [
                'type' => 'boolean',
                'default' => false,
    ...
    'execute_callback' => 'ElementorMCP\\Abilities\\Elementor\\elementor_get_widget_params',
...
function elementor_get_widget_params(array $input): array|WP_Error
{
    if (!el_min_runtime_available()) {
        return new WP_Error(
            'elementor_required',
            __('Elementor is not active on this site, or is below the minimum supported version.', domain: 'elementor-mcp'),
            ['status' => 400],
        );
    }
    ...
    if (!is_array($raw_types) || $raw_types === []) {
        return new WP_Error(
            ...
            __('widget_types must list at least one widget type.', domain: 'elementor-mcp'),
            ['status' => 422],
        );
    }
    ...
    foreach ($raw_types as $raw_type) {
        ...
        $widgets[$widget_type] = [
            ...
            'set_aside' => [
                ...
                'note' => 'Set-aside controls are still reachable through elementor-mcp/elementor-get-schema.',
            ],
        ];
    }
    ...
    if ($widgets === []) {
        return new WP_Error(
            ...
            sprintf(
                ...
                __('None of these widget types are registered on this site: %s.', domain: 'elementor-mcp'),
                implode(', ', $unknown),
            ),
