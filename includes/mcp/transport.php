...
{
    foreach (['/mcp/elementor-mcp', '/mcp/mcp-adapter-default-server'] as $base) {
...
function pre_dispatch(mixed $result, WP_REST_Server $server, WP_REST_Request $request): mixed
{
    ...
    // revision drops the GET stream and the session lifecycle, but Elementor MCP is
...
function handle_modern(WP_REST_Request $request, array $body): array
{
    ...
    return match ($method) {
        'server/discover' => success(build_discover_result(
            ...
            function_exists('elementor_mcp_get_safety_profile') ? \elementor_mcp_get_safety_profile() : 'unknown',
        ), $id, $method),
...
{
    return defined('ELEMENTOR_MCP_VERSION') ? (string) constant('ELEMENTOR_MCP_VERSION') : '0.0.0';
}
...
function list_tools(): array
{
    ...
    foreach (wp_get_abilities() as $ability) {
        ...
        if (function_exists('elementor_mcp_safety_profile_allows_ability') && !\elementor_mcp_safety_profile_allows_ability(
            $ability,
        )) {
            continue;
        }
 ...
 *
 * Elementor MCP's prompts are its skills. They take no arguments, so `arguments` is
 ...
 *
 *     Ability "elementor-mcp/elementor-build-page" is destructive or critical.
...
{
    if (!function_exists('elementor_mcp_ability_requires_confirmation')) {
        return $schema;
    }
    if (!\elementor_mcp_ability_requires_confirmation($ability)) {
        return $schema;
    }
    ...
    $properties['confirm'] = [
        'type' => 'boolean',
        'description' => 'Must be true. This operation is destructive or critical, and Elementor MCP refuses it '
 ...
 * ELEMENTOR_MCP_NO_INPUT_SCHEMA — reaches the client as a JSON array where JSON
...
function call_tool(array $params, mixed $id): array
{
    ...
    if (function_exists('elementor_mcp_safety_check_ability')) {
        ...
        if ($allowed === false) {
            return tool_error(
                new WP_Error('elementor_mcp_safety_blocked', 'The active safety profile does not allow this operation.'),
                $id,
            );
        }
    }
    ...
    // On the legacy path it is applied by elementor_mcp_safety_pre_mcp_tool_call(),
    ...
    if (function_exists('elementor_mcp_ability_requires_confirmation') && \elementor_mcp_ability_requires_confirmation($ability)) {
        if (($arguments['confirm'] ?? null) !== true) {
            return tool_error(\elementor_mcp_confirmation_required_error($ability), $id);
        }
    }
    ...
    if (
        ...
        && function_exists('elementor_mcp_ability_schema_has_property')
        && !\elementor_mcp_ability_schema_has_property($ability, 'confirm')
    ) {
    ...
    if (function_exists('ElementorMCP\\Preview\\Gate\\check')) {
        $preview_required = \ElementorMCP\Preview\Gate\check($ability, $arguments);
        if ($preview_required instanceof WP_Error) {
            return tool_error($preview_required, $id);
        }
    }

    if (function_exists('elementor_mcp_rate_pre_ability_execute')) {
        $limited = \elementor_mcp_rate_pre_ability_execute($arguments, $ability, 'mcp');
    ...
    /** @var mixed $gated */
    $gated = apply_filters('elementor_mcp_modern_mcp_pre_ability_execute', $arguments, $ability, 'mcp');
 ...
 * null. An ability that declares ELEMENTOR_MCP_NO_INPUT_SCHEMA — an object schema
 * with no properties, which is what every no-input Elementor MCP ability declares —
 * validates its input with rest_is_object(), and `rest_is_object(null)` is
 * false, so null comes back as:
 *
 *     Ability "elementor-mcp/system-status" has invalid input.
...
{
    if (function_exists('elementor_mcp_normalize_empty_ability_input')) {
        return \elementor_mcp_normalize_empty_ability_input($ability, []);
    }
