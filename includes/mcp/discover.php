 ...
 * not what the plugin is capable of in principle. A Elementor MCP site with every
 ...
 *
 * - `subscriptions`: Elementor MCP has no change-notification producer. Nothing in
 ...
 * - `logging`: deprecated in this revision, and Elementor MCP emits no
...
function build_capabilities(int $tool_count, int $prompt_count, int $resource_count): array
{
    ...
    if ($tool_count > 0) {
        // No `listChanged`: Elementor MCP does not emit tools/list_changed, and
