...
add_action('rest_api_init', callback: 'elementor_mcp_register_upload_route');

/**
 * Register the REST endpoint used by signed uploads.
 */
function elementor_mcp_register_upload_route(): void
{
    ...
    register_rest_route($route_namespace, $route, [
        'methods' => ['POST', 'PUT'],
        'callback' => 'elementor_mcp_handle_signed_upload',
 ...
 */
function elementor_mcp_sign_upload_payload(array $payload): string|WP_Error
{
    ...
    $body = elementor_mcp_base64url_encode($json);
    $signature = hash_hmac('sha256', $body, elementor_mcp_upload_token_secret(), binary: true);

    return $body . '.' . elementor_mcp_base64url_encode($signature);
}
 ...
 */
function elementor_mcp_verify_upload_token(string $token): array|WP_Error
{
    ...
    $expected = elementor_mcp_base64url_encode(hash_hmac('sha256', $body, elementor_mcp_upload_token_secret(), binary: true));
    if (!hash_equals($expected, $signature)) {
        return new WP_Error('invalid_upload_token', 'Invalid upload token signature.', ['status' => 401]);
    }

    $json = elementor_mcp_base64url_decode($body);
    if ($json === false) {
 ...
 */
function elementor_mcp_handle_signed_upload(WP_REST_Request $request)
{
    if (!elementor_mcp_is_enabled()) {
        return new WP_Error('elementor_mcp_disabled', 'Elementor MCP abilities are disabled.', ['status' => 403]);
    }

    $token = elementor_mcp_get_upload_token_from_request($request);
    if ($token === '') {
        return new WP_Error('missing_upload_token', 'Missing upload token.', ['status' => 401]);
    }

    $payload = elementor_mcp_verify_upload_token($token);
    if (is_wp_error($payload)) {
        return $payload;
    }

    $destination = elementor_mcp_prepare_upload_destination($payload);
    if (is_wp_error($destination)) {
        return $destination;
    }

    $source = elementor_mcp_open_upload_source($request);
    ...
    $result = $destination['overwrite']
        ? elementor_mcp_overwrite_upload_stream(
            source: $stream,
            resolved: $destination['path'],
            max_bytes: $destination['max_bytes'],
        )
        : elementor_mcp_create_upload_stream(
            source: $stream,
            resolved: $destination['path'],
            max_bytes: $destination['max_bytes'],
        );
 ...
 */
function elementor_mcp_prepare_upload_destination(array $payload): array|WP_Error
{
    ...
    $resolved = elementor_mcp_resolve_path(path: $payload['path'], must_exist: false);
    if (is_wp_error($resolved)) {
        return $resolved;
    }

    $symlink_error = elementor_mcp_reject_final_path_symlink($resolved);
    ...
    $sandbox_error = elementor_mcp_check_php_execution_sandbox($resolved);
    ...
    if (!is_dir($parent_dir)) {
        ...
        $directories_created = elementor_mcp_ensure_parent_dir($parent_dir);
 ...
 */
function elementor_mcp_get_upload_token_from_request(WP_REST_Request $request): string
{
    return elementor_mcp_rest_header_token($request, header_name: 'x-elementor-mcp-upload-token');
}
 ...
 */
function elementor_mcp_open_upload_source(WP_REST_Request $request): array|WP_Error
{
    $file = elementor_mcp_get_multipart_upload_file($request);
    if ($file !== null) {
        return elementor_mcp_open_multipart_upload_source($file);
    }
 ...
 */
function elementor_mcp_get_multipart_upload_file(WP_REST_Request $request): ?array
{
 ...
 * @return array{stream: resource, source: string, filename: string}|WP_Error
 */
function elementor_mcp_open_multipart_upload_source(array $file): array|WP_Error
{
    $error = (int) ($file['error'] ?? UPLOAD_ERR_NO_FILE);
    if ($error !== UPLOAD_ERR_OK) {
        return new WP_Error('upload_failed', elementor_mcp_upload_error_message($error));
    }
 ...
 */
function elementor_mcp_create_upload_stream($source, string $resolved, int $max_bytes): array|WP_Error
{
    // phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink,WordPress.WP.AlternativeFunctions.rename_rename,WordPress.WP.AlternativeFunctions.file_system_operations_fopen,WordPress.WP.AlternativeFunctions.file_system_operations_fclose,WordPress.WP.AlternativeFunctions.file_system_operations_fread,WordPress.WP.AlternativeFunctions.file_system_operations_fwrite,WordPress.WP.AlternativeFunctions.file_system_operations_chmod,WordPress.WP.AlternativeFunctions.file_system_operations_mkdir,WordPress.WP.AlternativeFunctions.file_system_operations_rmdir,WordPress.WP.AlternativeFunctions.file_system_operations_readfile,WordPress.WP.AlternativeFunctions.file_system_operations_is_writable -- WP_Filesystem is not usable here: it takes credentials from an interactive admin form, which a REST/MCP request has no way to show, and it offers no file handle — uploads are streamed to disk in chunks so a large file never has to fit in memory.
    ...
    $bytes_written = elementor_mcp_copy_limited_stream(source: $source, target: $target, max_bytes: $max_bytes);
    // phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink,WordPress.WP.AlternativeFunctions.rename_rename,WordPress.WP.AlternativeFunctions.file_system_operations_fopen,WordPress.WP.AlternativeFunctions.file_system_operations_fclose,WordPress.WP.AlternativeFunctions.file_system_operations_fread,WordPress.WP.AlternativeFunctions.file_system_operations_fwrite,WordPress.WP.AlternativeFunctions.file_system_operations_chmod,WordPress.WP.AlternativeFunctions.file_system_operations_mkdir,WordPress.WP.AlternativeFunctions.file_system_operations_rmdir,WordPress.WP.AlternativeFunctions.file_system_operations_readfile,WordPress.WP.AlternativeFunctions.file_system_operations_is_writable -- WP_Filesystem is not usable here: it takes credentials from an interactive admin form, which a REST/MCP request has no way to show, and it offers no file handle — uploads are streamed to disk in chunks so a large file never has to fit in memory.
    fclose($target);
 ...
 * @return array{bytes_written: int, created: bool}|WP_Error
 */
function elementor_mcp_overwrite_upload_stream($source, string $resolved, int $max_bytes): array|WP_Error
{
    $created = !file_exists($resolved);
    $temporary_path = tempnam(dirname($resolved), prefix: '.elementor-mcp-upload-');
 ...
 * @return int|WP_Error
 */
function elementor_mcp_copy_limited_stream($source, $target, int $max_bytes): int|WP_Error
{
    $bytes_written = 0;
    while (!feof($source)) {
        // phpcs:ignore WordPress.WP.AlternativeFunctions.unlink_unlink,WordPress.WP.AlternativeFunctions.rename_rename,WordPress.WP.AlternativeFunctions.file_system_operations_fopen,WordPress.WP.AlternativeFunctions.file_system_operations_fclose,WordPress.WP.AlternativeFunctions.file_system_operations_fread,WordPress.WP.AlternativeFunctions.file_system_operations_fwrite,WordPress.WP.AlternativeFunctions.file_system_operations_chmod,WordPress.WP.AlternativeFunctions.file_system_operations_mkdir,WordPress.WP.AlternativeFunctions.file_system_operations_rmdir,WordPress.WP.AlternativeFunctions.file_system_operations_readfile,WordPress.WP.AlternativeFunctions.file_system_operations_is_writable -- WP_Filesystem is not usable here: it takes credentials from an interactive admin form, which a REST/MCP request has no way to show, and it offers no file handle — uploads are streamed to disk in chunks so a large file never has to fit in memory.
 ...
 */
function elementor_mcp_upload_error_message(int $error): string
{
    return match ($error) {
 ...
 */
function elementor_mcp_upload_token_secret(): string
{
    return wp_salt('auth') . '|' . wp_salt('secure_auth') . '|elementor-mcp-upload-link';
}

/**
 * Encode bytes with base64url.
 */
function elementor_mcp_base64url_encode(string $value): string
{
 ...
 */
function elementor_mcp_base64url_decode(string $value): string|false
{
