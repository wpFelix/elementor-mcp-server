...
/**
 * The AI clients Elementor MCP knows about, and how each one connects.
 *
 ...
 */
function elementor_mcp_clients_native_oauth(): array
{
    return [
        'claude-code' => [
            'label' => 'Claude Code',
 ...
 * @return array<string, array<string, mixed>>
 */
function elementor_mcp_clients_web_ui(): array
{
    return [
        ...
        'claude-web' => [
            'label' => __('Claude (web)', domain: 'elementor-mcp'),
            'match' => ['claude-web'],
            'methods' => ['oauth', 'token'],
            'note' => __(
        ...
        'codex-app' => [
            'label' => __('Codex (desktop app)', domain: 'elementor-mcp'),
            'match' => ['codex-app', 'codex desktop', 'codex-desktop'],
            'methods' => ['oauth'],
            'note' => __(
                'In the Codex desktop app open Settings from your account menu, add Elementor MCP as a remote MCP server, and approve the browser sign-in.',
                domain: 'elementor-mcp',
            ),
        ],
    ];
 ...
 * @return array<string, array<string, mixed>>
 */
function elementor_mcp_clients_native_oauth_editors(): array
{
    return [
        'codex' => [
            'label' => __('Codex CLI', domain: 'elementor-mcp'),
            'match' => ['codex'],
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('After adding the server, run codex mcp login to authorise it.', domain: 'elementor-mcp'),
        ],
        'cursor' => [
            'label' => 'Cursor',
            'match' => ['cursor'],
        ...
        'vscode' => [
            ...
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Requires VS Code 1.101 or later for remote MCP with OAuth.', domain: 'elementor-mcp'),
        ],
        ...
        // metadata where the server supports it — so it reaches Elementor MCP's OAuth
        ...
        'factory-droid' => [
            ...
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Add the server with /mcp inside Droid, or put it in ~/.factory/mcp.json.', domain: 'elementor-mcp'),
        ],
        // Kimi Code CLI takes a whole server in one command, like Claude Code,
        // and stores it in a standard mcpServers file.
        'kimi-cli' => [
            'label' => __('Kimi Code CLI', domain: 'elementor-mcp'),
            'match' => ['kimi', 'kimi-cli', 'kimi code'],
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Servers live in ~/.kimi/mcp.json, or add one with kimi mcp add.', domain: 'elementor-mcp'),
        ],
        ...
        'qwen-code' => [
            'label' => __('Qwen Code', domain: 'elementor-mcp'),
            'match' => ['qwen-code', 'qwen code', 'qwencode'],
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Add to settings.json. Remote servers use "httpUrl", not "url".', domain: 'elementor-mcp'),
        ],
        'gemini-cli' => [
            'label' => __('Gemini CLI', domain: 'elementor-mcp'),
            'match' => ['gemini-cli', 'gemini cli'],
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Add to settings.json. Remote servers use "httpUrl", not "url".', domain: 'elementor-mcp'),
        ],
        'zcode' => [
            'label' => __('ZCode (GLM)', domain: 'elementor-mcp'),
        ...
        'github-copilot' => [
            ...
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Uses the VS Code MCP configuration.', domain: 'elementor-mcp'),
        ],
        'antigravity-cli' => [
            'label' => 'Antigravity CLI',
            'match' => ['antigravity-cli', 'antigravity cli'],
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Add the server with /mcp, then complete the browser sign-in.', domain: 'elementor-mcp'),
        ],
        'antigravity-ide' => [
 ...
 * @return array<string, array<string, mixed>>
 */
function elementor_mcp_clients_proxied_oauth(): array
{
    return [
        ...
        'zed' => [
            ...
            'match' => ['zed'],
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Configured as a context server.', domain: 'elementor-mcp'),
        ],
        'cline' => [
            'label' => 'Cline',
            'match' => ['cline'],
        ...
        'openclaw' => [
            ...
            'methods' => ['oauth', 'token', 'password'],
            'note' => __('Add the server to your OpenClaw MCP configuration and restart the agent.', domain: 'elementor-mcp'),
        ],
 ...
 * @return array<string, array<string, mixed>>
 */
function elementor_mcp_client_proxy_shapes(): array
{
    return [
        'mcp-remote' => [
            'label' => __('mcp-remote proxy', domain: 'elementor-mcp'),
            'match' => ['mcp-remote'],
            'oauth' => 'native',
            'methods' => ['oauth'],
            'note' => __('An AI client connecting through the OAuth wrapper.', domain: 'elementor-mcp'),
            'hidden' => true,
        ],
        'wordpress-remote' => [
            'label' => __('WordPress MCP proxy', domain: 'elementor-mcp'),
            'match' => ['mcp-wordpress-remote', 'wordpress-remote'],
            'oauth' => 'proxy',
            'methods' => ['password'],
            'note' => __('An AI client connecting with an application password.', domain: 'elementor-mcp'),
            'hidden' => true,
        ],
    ];
}
 ...
 * @return array<string, array<string, mixed>>
 */
function elementor_mcp_clients_with_oauth_route(array $group, string $oauth): array
{
    $stamped = [];
...
/**
 * Every AI client Elementor MCP can describe a connection for.
 *
 * @return array<string, array<array-key, mixed>>
 */
function elementor_mcp_clients(): array
{
    $clients = array_merge(
        elementor_mcp_clients_with_oauth_route(elementor_mcp_clients_native_oauth(), oauth: 'native'),
        elementor_mcp_clients_with_oauth_route(elementor_mcp_clients_web_ui(), oauth: 'native'),
        elementor_mcp_clients_with_oauth_route(elementor_mcp_clients_native_oauth_editors(), oauth: 'native'),
        elementor_mcp_clients_with_oauth_route(elementor_mcp_clients_proxied_oauth(), oauth: 'proxy'),
        elementor_mcp_client_proxy_shapes(),
    );
     ...
     */
    /** @var mixed $filtered */
    $filtered = apply_filters('elementor_mcp_clients', $clients);
 ...
 * @return array<string, array<array-key, mixed>>
 */
function elementor_mcp_selectable_clients(): array
{
    return array_filter(elementor_mcp_clients(), static fn(array $client): bool => ($client['hidden'] ?? false) !== true);
}
 ...
 */
function elementor_mcp_client_needles(): array
{
    /** @var list<array{needle: string, key: string}> $needles */
    $needles = [];

    foreach (elementor_mcp_clients() as $key => $client) {
        /** @var mixed $matches */
 ...
 * suffixes are common.
 */
function elementor_mcp_client_key(string $reported): ?string
{
    ...
    foreach (elementor_mcp_client_needles() as $candidate) {
 ...
 */
function elementor_mcp_client_label(string $reported): string
{
    $key = elementor_mcp_client_key($reported);
    if ($key !== null) {
        return (string) (elementor_mcp_clients()[$key]['label'] ?? $reported);
    }

    $trimmed = trim($reported);

    return $trimmed !== '' ? $trimmed : __('Unidentified client', domain: 'elementor-mcp');
}
