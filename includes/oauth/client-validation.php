...
namespace ElementorMCP\OAuth\ClientValidation;
...
function allowed_schemes(): array
{
     ...
     */
    /** @var mixed $schemes */
    $schemes = apply_filters('elementor_mcp_oauth_allowed_redirect_schemes', ALLOWED_SCHEMES);
    if (!is_array($schemes)) {
 ...
 * it with the `elementor_mcp_oauth_max_clients` filter.
 */
function max_clients_per_site(): int
{
    // @mago-expect analysis:mixed-assignment
    $cap = apply_filters('elementor_mcp_oauth_max_clients', MAX_CLIENTS_PER_SITE);
...
{
    $key = 'elementor_mcp_oauth_dcr_' . hash('sha256', $client_ip);
...
function within_endpoint_rate_limit(string $bucket, string $client_ip): bool
{
    ...
    $key = 'elementor_mcp_oauth_rl_' . $bucket . '_' . hash('sha256', $client_ip);
    $current = (int) get_transient($key);
 ...
 *
 * The schema is installed by ElementorMCP\OAuth\boot(), which only runs once AI
...
function active_client_count(): int
{
    ...
    // @mago-expect analysis:possibly-invalid-argument
    $sql = $wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}elementor_mcp_oauth_clients
...
function client_count_for_ip(string $client_ip): int
{
    ...
    // @mago-expect analysis:possibly-invalid-argument
    $sql = $wpdb->prepare(
        "SELECT COUNT(*) FROM {$wpdb->prefix}elementor_mcp_oauth_clients WHERE registered_by_ip_hash = %s",
...
function release_client_slot_if_unused(string $client_id): void
{
    ...
    $clients = $wpdb->prefix . 'elementor_mcp_oauth_clients';
...
function prune_dead_clients(): void
{
    ...
    // @mago-expect analysis:possibly-invalid-argument
    $sql = $wpdb->prepare(
        "DELETE FROM {$wpdb->prefix}elementor_mcp_oauth_clients
