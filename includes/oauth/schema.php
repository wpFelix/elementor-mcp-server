...
namespace ElementorMCP\OAuth\Schema;

if (!defined('ABSPATH')) {
    exit();
}

const SCHEMA_VERSION_OPTION = 'elementor_mcp_oauth_schema_version';
...
function maybe_install(): void
{
    ...
    $p = $wpdb->prefix . 'elementor_mcp_oauth_';
...
if (!wp_next_scheduled('elementor_mcp_oauth_gc')) {
    wp_schedule_event(timestamp: time() + HOUR_IN_SECONDS, recurrence: 'daily', hook: 'elementor_mcp_oauth_gc');
}
add_action('elementor_mcp_oauth_gc', __NAMESPACE__ . '\\gc');
