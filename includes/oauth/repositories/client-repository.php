...
namespace ElementorMCP\OAuth\Repositories;
...
final class ClientRepository implements ClientRepositoryInterface
{
    ...
    public function create(
        ...
        $wpdb->insert($wpdb->prefix . 'elementor_mcp_oauth_clients', [
            'client_id' => $client_id,
    ...
    public function getClientEntity(mixed $clientIdentifier): ?ClientEntityInterface
    {
        ...
        $row = $wpdb->get_row($wpdb->prepare("SELECT client_id, client_name, redirect_uris, is_confidential
                 FROM {$wpdb->prefix}elementor_mcp_oauth_clients
    ...
    private function metadata_document_client(string $clientIdentifier): ?ClientEntityInterface
    {
        ...
        if (!\ElementorMCP\OAuth\ClientIdMetadata\is_metadata_document_client_id($clientIdentifier)) {
            return null;
        }

        $document = \ElementorMCP\OAuth\ClientIdMetadata\fetch_client_metadata($clientIdentifier);
    ...
    public function revoke(string $client_id): void
    {
        ...
        global $wpdb;
        /** @var \wpdb $wpdb */
        $wpdb->delete($wpdb->prefix . 'elementor_mcp_oauth_clients', ['client_id' => $client_id]);
    }
    ...
    public function find_admin_client_id(string $client_name): ?string
    {
        ...
        // @mago-expect analysis:possibly-invalid-argument
        // @mago-expect analysis:possibly-invalid-argument
        $id = $wpdb->get_var($wpdb->prepare("SELECT client_id FROM {$wpdb->prefix}elementor_mcp_oauth_clients
