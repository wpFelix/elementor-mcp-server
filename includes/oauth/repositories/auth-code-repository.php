...
final class AuthCodeRepository implements AuthCodeRepositoryInterface
{
    ...
    public function persistNewAuthCode(AuthCodeEntityInterface $authCodeEntity): void
    {
        ...
        global $wpdb;
        /** @var \wpdb $wpdb */
        $wpdb->insert($wpdb->prefix . 'elementor_mcp_oauth_auth_codes', [
    ...
    public function isAuthCodeRevoked(mixed $codeId): bool
    {
        ...
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT revoked, expires_at FROM {$wpdb->prefix}elementor_mcp_oauth_auth_codes WHERE identifier_hash = %s",
                hash('sha256', $codeId),
            ),
            ARRAY_A,
        );
