...
final class RefreshTokenRepository implements RefreshTokenRepositoryInterface
{
    ...
    public function persistNewRefreshToken(RefreshTokenEntityInterface $refreshTokenEntity): void
    {
        ...
        global $wpdb;
        /** @var \wpdb $wpdb */
        $wpdb->insert($wpdb->prefix . 'elementor_mcp_oauth_refresh_tokens', [
    ...
    public function isRefreshTokenRevoked(mixed $tokenId): bool
    {
        ...
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT revoked, expires_at FROM {$wpdb->prefix}elementor_mcp_oauth_refresh_tokens WHERE identifier_hash = %s",
                hash('sha256', $tokenId),
            ),
            ARRAY_A,
        );
    ...
    public function accessTokenHashFor(string $refreshJti): string
    {
        ...
        $value = $wpdb->get_var($wpdb->prepare(
            "SELECT access_token_hash FROM {$wpdb->prefix}elementor_mcp_oauth_refresh_tokens WHERE identifier_hash = %s",
