...
final class AccessTokenEntity implements AccessTokenEntityInterface
{
    ...
    public function __toString(): string
    {
        ...
        $audience = \ElementorMCP\OAuth\resource_identifier();
...
final class AccessTokenRepository implements AccessTokenRepositoryInterface
{
    ...
    public function persistNewAccessToken(AccessTokenEntityInterface $accessTokenEntity): void
    {
        ...
        global $wpdb;
        /** @var \wpdb $wpdb */
        $wpdb->insert($wpdb->prefix . 'elementor_mcp_oauth_access_tokens', [
    ...
    public function revokeGrantByAccessHash(string $accessTokenHash, string $requestClientId): void
    {
        ...
        $owner = $wpdb->get_var($wpdb->prepare(
            "SELECT client_id FROM {$wpdb->prefix}elementor_mcp_oauth_access_tokens WHERE identifier_hash = %s",
            $accessTokenHash,
        ));
    ...
    public function isAccessTokenRevoked(mixed $tokenId): bool
    {
        ...
        $row = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT revoked, expires_at FROM {$wpdb->prefix}elementor_mcp_oauth_access_tokens WHERE identifier_hash = %s",
                hash('sha256', $tokenId),
            ),
            ARRAY_A,
        );
