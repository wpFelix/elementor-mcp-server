...
namespace ElementorMCP\OAuth\ConnectedApps;
...
function register(): void
{
    $hook = add_submenu_page(
        ...
        menu_slug: 'elementor-mcp-connected-apps',
...
function handle_load(): void
{
    ...
    if (!is_user_logged_in() || !\elementor_mcp_current_user_can_manage()) {
        return;
    }
...
function render(): void
{
    ...
    if (!\elementor_mcp_current_user_can_manage()) {
        wp_die('You are not allowed to manage Elementor MCP connected apps.', title: '', args: ['response' => 403]);
        return;
    }
...
{
    $action = $_POST['elementor_mcp_action'] ?? '';
    if ($action === 'delete_admin_client') {
        check_admin_referer('elementor_mcp_connected_apps_delete');
        ...
        if ($client_id !== '') {
            ...
            (new \ElementorMCP\OAuth\Repositories\ClientRepository())->revoke($client_id);
        }
        wp_redirect(add_query_arg(['deleted' => '1'], admin_url('admin.php?page=elementor-mcp-connected-apps')));
        exit();
    }

    check_admin_referer('elementor_mcp_connected_apps_revoke');
    ...
    wp_redirect(add_query_arg(['revoked' => '1'], admin_url('admin.php?page=elementor-mcp-connected-apps')));
    exit();
}
...
function revoke_client_access(string $client_id, int $user_id): void
{
    ...
    global $wpdb;
    /** @var \wpdb $wpdb */
    $t = $wpdb->prefix . 'elementor_mcp_oauth_access_tokens';
    $r = $wpdb->prefix . 'elementor_mcp_oauth_refresh_tokens';
    ...
    \ElementorMCP\OAuth\ClientValidation\release_client_slot_if_unused($client_id);
}
...
function render_page(int $user_id): void
{
    ...
    $t = $wpdb->prefix . 'elementor_mcp_oauth_access_tokens';
    $r = $wpdb->prefix . 'elementor_mcp_oauth_refresh_tokens';
    $c = $wpdb->prefix . 'elementor_mcp_oauth_clients';
    ...
    echo '<div class="wrap">';
    echo '<h1>' . esc_html__('Connected Apps', domain: 'elementor-mcp') . '</h1>';
    echo
        '<p>'
            . esc_html__(
                'These applications have been granted access to your WordPress account via Elementor MCP. The connection renews automatically while in use; the expiry shown is when it lapses if the app stops connecting.',
    ...
    if ($was_revoked) {
        echo
            '<div class="notice notice-success is-dismissible"><p>'
                . esc_html__('Access revoked successfully.', domain: 'elementor-mcp')
                . '</p></div>'
        ;
    }

    if ($apps === []) {
        echo '<p>' . esc_html__('No apps are currently connected to your account.', domain: 'elementor-mcp') . '</p>';
    ...
    echo '<thead><tr>';
    echo '<th>' . esc_html__('Application', domain: 'elementor-mcp') . '</th>';
    echo '<th>' . esc_html__('Scope', domain: 'elementor-mcp') . '</th>';
    echo '<th>' . esc_html__('Connection expires', domain: 'elementor-mcp') . '</th>';
    echo '<th></th>';
    echo '</tr></thead><tbody>';
    ...
    foreach ($apps as $app) {
        ...
        wp_nonce_field('elementor_mcp_connected_apps_revoke');
        echo '<input type="hidden" name="client_id" value="' . esc_attr($cid) . '">';
        echo '<button type="submit" class="button">' . esc_html__('Revoke Access', domain: 'elementor-mcp') . '</button>';
        echo '</form>';
        echo '</td>';
        echo '</tr>';
    }
...
{
    $clients = (new \ElementorMCP\OAuth\Repositories\ClientRepository())->list_admin_clients();

    $raw_deleted = $_GET['deleted'] ?? null;
    if (is_string($raw_deleted) && $raw_deleted === '1') {
        echo
            '<div class="notice notice-success is-dismissible"><p>'
                . esc_html__('Client ID deleted.', domain: 'elementor-mcp')
                . '</p></div>'
        ;
    }
    ...
    echo '<h2 style="margin-top:24px;">' . esc_html__('Manually created client IDs', domain: 'elementor-mcp') . '</h2>';
    echo
        '<p>'
            . esc_html__(
    ...
    echo '<thead><tr>';
    echo '<th>' . esc_html__('Application', domain: 'elementor-mcp') . '</th>';
    echo '<th>' . esc_html__('Client ID', domain: 'elementor-mcp') . '</th>';
    echo '<th>' . esc_html__('Created', domain: 'elementor-mcp') . '</th>';
    echo '<th>' . esc_html__('First used', domain: 'elementor-mcp') . '</th>';
    echo '<th></th>';
    echo '</tr></thead><tbody>';
    foreach ($clients as $client) {
        ...
        echo
            '<span style="font-size:11px; font-weight:600; color:#646970; background:#f0f0f1; border-radius:10px; padding:1px 8px;">'
                . esc_html__('manually created', domain: 'elementor-mcp')
                . '</span></td>'
        ;
        echo '<td><code>' . esc_html($client['client_id']) . '</code></td>';
        echo '<td>' . esc_html($client['created_at']) . '</td>';
        echo '<td>' . esc_html($client['last_used_at'] ?? __('Never', domain: 'elementor-mcp')) . '</td>';
        echo '<td>';
        echo '<form method="post">';
        wp_nonce_field('elementor_mcp_connected_apps_delete');
        echo '<input type="hidden" name="elementor_mcp_action" value="delete_admin_client">';
        echo '<input type="hidden" name="client_id" value="' . esc_attr($client['client_id']) . '">';
        echo '<button type="submit" class="button">' . esc_html__('Delete', domain: 'elementor-mcp') . '</button>';
        echo '</form>';
        echo '</td>';
        echo '</tr>';
    }
