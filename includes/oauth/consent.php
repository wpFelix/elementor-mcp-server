...
namespace ElementorMCP\OAuth\Consent;

use League\OAuth2\Server\Exception\OAuthServerException;
use ElementorMCP\OAuth\Bridge;
use ElementorMCP\OAuth\Endpoints\Authorize;
use ElementorMCP\OAuth\Keys\KeyBootstrapError;
use ElementorMCP\OAuth\Repositories\ClientRepository;
use ElementorMCP\OAuth\Repositories\UserEntity;
use ElementorMCP\OAuth\ServerFactory;
...
function register(): void
{
    $hook = add_submenu_page(
        ...
        menu_slug: 'elementor-mcp-oauth-consent',
...
function resolve_pending(): ?array
{
    ...
    if (!\elementor_mcp_current_user_can_manage()) {
        wp_die('You are not allowed to authorize Elementor MCP applications.', title: '', args: ['response' => 403]);
        return null;
    }
...
{
    check_admin_referer('elementor_mcp_oauth_consent_' . $token);
    ...
    try {
        ...
        if (\elementor_mcp_debug_logging_enabled()) {
            ...
            error_log('Elementor MCP OAuth: ' . $e->getMessage());
        }
        wp_die(
            esc_html__(
                'Elementor MCP could not create the OAuth signing keys for this site. Run "wp elementor-mcp oauth-keys generate" from WP-CLI to create the keys, then authorize again. With WP_DEBUG enabled, the PHP error log records the OpenSSL reason.',
...
function render_form(string $token, string $client_name, string $redirect_uri, string $scope): void
{
    ...
    echo '<div class="wrap">';
    echo '<h1>' . esc_html__('Authorize Application', domain: 'elementor-mcp') . '</h1>';
    echo
        sprintf(
            /* translators: 1: name of the connecting client, 2: label of the access being requested */
            '<p>' . esc_html__('%1$s is requesting %2$s.', domain: 'elementor-mcp') . '</p>',
    ...
    echo
        '<p><strong>'
            . esc_html__('Requested OAuth scope:', domain: 'elementor-mcp')
            . '</strong> <code>'
            . esc_html($scope)
            . '</code></p>'
    ;
    if ($grant['risks'] !== []) {
        echo '<p><strong>' . esc_html__('This grant can:', domain: 'elementor-mcp') . '</strong></p><ul>';
    ...
    echo
        '<p><strong>'
            . esc_html__('Redirect destination:', domain: 'elementor-mcp')
            . '</strong> '
    ...
    wp_nonce_field('elementor_mcp_oauth_consent_' . $token);
    echo
        '<button type="submit" name="approve" value="1" class="button button-primary">'
            . esc_html__('Authorize', domain: 'elementor-mcp')
            . '</button> '
    ;
    echo
        '<button type="submit" name="deny" value="1" class="button">'
            . esc_html__('Deny', domain: 'elementor-mcp')
            . '</button>'
    ;
    echo '</form>';
    echo '</div>';
...
{
    return [
        'label' => __('full access to your WordPress site', domain: 'elementor-mcp'),
        'description' => __(
            'Full access permits execution of Elementor MCP capabilities through MCP and REST, including REST-visible abilities registered by compatible third-party plugins.',
            domain: 'elementor-mcp',
        ),
        'risks' => [
            __('Execute PHP and WP-CLI.', domain: 'elementor-mcp'),
            __('Read, write, and delete server files.', domain: 'elementor-mcp'),
            __('Change WordPress content and settings.', domain: 'elementor-mcp'),
            __('Create temporary administrator access.', domain: 'elementor-mcp'),
            __('Execute REST-visible abilities registered by compatible plugins.', domain: 'elementor-mcp'),
        ],
    ];
}
