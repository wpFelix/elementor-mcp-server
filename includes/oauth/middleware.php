...
namespace ElementorMCP\OAuth\Middleware;
...
{
    // WordPress cookie and Application Password authenticators run first. Elementor MCP only establishes
 ...
 * token, and a Elementor MCP access token. They are told apart by the token's fixed
 ...
 * denies the request outright, so a Elementor MCP token would never get a second look.
 *
...
function resolve_bearer_identity(mixed $user): mixed
{
    ...
    if (function_exists('elementor_mcp_token_looks_like') && \elementor_mcp_token_looks_like(bearer_secret($auth))) {
...
/**
 * Establish an identity from a Elementor MCP access token.
 *
...
function resolve_token_identity(mixed $user, string $auth): mixed
{
    ...
    $identity = \elementor_mcp_token_authenticate(bearer_secret($auth));
    if ($identity === null) {
        record_authentication_error('Invalid, expired, or revoked Elementor MCP access token.');
        return $user;
    }

    wp_set_current_user($identity['user_id']);
    record_oauth_identity($identity['user_id'], ['mcp'], 'token-' . $identity['id'], via: 'token');
    \elementor_mcp_token_touch($identity['id']);
 ...
 *
 * The optional server exists for focused tests; production always builds the server from Elementor MCP's
...
function validate_bearer_credential(string $auth, ?ResourceServer $server = null): array
{
    ...
    if ($validated->getAttribute('oauth_client_id') !== \ElementorMCP\OAuth\resource_identifier()) {
    ...
    if ($production_server) {
        ...
        if ($token_id !== '') {
            $client_id = (new \ElementorMCP\OAuth\Repositories\AccessTokenRepository())->get_client_id_by_token_identifier(
                $token_id,
            );
...
function authorize_routed_request(mixed $result, mixed $handler, WP_REST_Request $request): mixed
{
    ...
    if (!current_user_can_access_mcp()) {
        return new WP_Error('rest_oauth_error', 'User is no longer allowed to manage Elementor MCP.', [
            'status' => 403,
        ]);
    }
...
{
    $value = 'Bearer resource_metadata="' . \ElementorMCP\OAuth\Endpoints\Discovery\protected_resource_metadata_url() . '"';
    if ($error !== null) {
        $value .= ', error="' . $error . '"';
    }
...
{
    return $route === '/mcp/elementor-mcp-oauth' || str_starts_with($route, '/mcp/elementor-mcp-oauth/');
}

/**
 * Whether a route is any of the MCP endpoints this site serves.
 *
 * Access tokens are accepted on all of them, the canonical `/mcp/elementor-mcp`
...
{
    foreach (['/mcp/elementor-mcp-oauth', '/mcp/elementor-mcp', '/mcp/mcp-adapter-default-server'] as $base) {
...
/**
 * Exact REST routes on which a Elementor MCP-established identity may survive post-routing checks.
 */
function oauth_identity_may_use_route(string $route, string $method): bool
{
    ...
    return $method === 'POST' && preg_match('#^/elementor-mcp/v1/abilities/[^/]+(?:/[^/]+)+/run$#', $route) === 1;
}
