...
namespace ElementorMCP\OAuth\Endpoints\Authorize;
...
const PENDING_PREFIX = 'elementor_mcp_oauth_pending_';

const PAGE_SLUG = 'elementor-mcp-oauth-authorize';
 ...
 * (auth_redirect) and the Elementor MCP management capability, so the current user is
...
function handle(): void
{
    if (!\elementor_mcp_current_user_can_manage()) {
        wp_die(
            esc_html__('You are not allowed to authorize Elementor MCP applications.', domain: 'elementor-mcp'),
            title: '',
            args: [
                'response' => 403,
            ],
    ...
    if ($response_type !== 'code') {
        wp_die(esc_html__('response_type must be "code".', domain: 'elementor-mcp'), title: '', args: ['response' => 400]);
        return;
    }

    $client_id = get_param('client_id');
    if ($client_id === '') {
        wp_die(esc_html__('client_id is required.', domain: 'elementor-mcp'), title: '', args: ['response' => 400]);
        return;
    }

    $redirect_uri = get_param('redirect_uri');
    if ($redirect_uri === '') {
        wp_die(esc_html__('redirect_uri is required.', domain: 'elementor-mcp'), title: '', args: ['response' => 400]);
        return;
    }

    $code_challenge = get_param('code_challenge');
    if ($code_challenge === '') {
        wp_die(esc_html__('code_challenge is required (PKCE mandatory).', domain: 'elementor-mcp'), title: '', args: [
            'response' => 400,
        ]);
        return;
    }
    ...
    if ($code_challenge_method !== 'S256') {
        wp_die(esc_html__('code_challenge_method must be "S256".', domain: 'elementor-mcp'), title: '', args: [
            'response' => 400,
        ]);
        return;
    }

    $scope = \ElementorMCP\OAuth\normalize_authorization_scope(get_param('scope'));
    if ($scope === null) {
        wp_die(esc_html__('The requested OAuth scope is invalid.', domain: 'elementor-mcp'), title: '', args: [
            'response' => 400,
        ]);
        return;
    }
    ...
    if (!\ElementorMCP\OAuth\resource_request_allowed($resource, \ElementorMCP\OAuth\resource_identifier())) {
        wp_die(
            esc_html__('The requested resource is not served by this authorization server.', domain: 'elementor-mcp'),
            title: '',
            args: ['response' => 400],
        );
        return;
    ...
    if ($client === null) {
        wp_die(esc_html__('Unknown client_id.', domain: 'elementor-mcp'), title: '', args: ['response' => 400]);
        return;
    }
    ...
    if (!$uri_validator->validateRedirectUri($redirect_uri)) {
        wp_die(esc_html__('redirect_uri not registered for this client.', domain: 'elementor-mcp'), title: '', args: [
            'response' => 400,
        ]);
        return;
    }
    ...
    wp_safe_redirect(admin_url('admin.php?page=elementor-mcp-oauth-consent&token=' . rawurlencode($token)));
    exit();
}
...
{
    wp_die(esc_html__('Invalid authorization request.', domain: 'elementor-mcp'), title: '', args: ['response' => 400]);
}
