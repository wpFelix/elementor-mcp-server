...
namespace ElementorMCP\OAuth\Endpoints\Register;
...
{
    register_rest_route('elementor-mcp/v1', route: '/oauth/register', args: [
        'methods' => 'POST',
...
{
    $token = trim((string) $req->get_header('x-elementor-mcp-self-test'));
    if ($token === '') {
        return false;
    }
    $key = 'elementor_mcp_oauth_selftest_' . hash('sha256', $token);
    /** @var mixed $found */
