 ...
 *
 * Before this, the only signal was `elementor_mcp_mcp_last_request`: two integers for
...
const ELEMENTOR_MCP_CONNECTIONS_SCHEMA_VERSION = 2;

const ELEMENTOR_MCP_CONNECTIONS_SCHEMA_OPTION = 'elementor_mcp_connections_schema_version';

function elementor_mcp_connections_table(): string
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
    return $wpdb->prefix . 'elementor_mcp_connections';
}

function elementor_mcp_connections_schema_maybe_install(): void
{
    if ((int) get_option(ELEMENTOR_MCP_CONNECTIONS_SCHEMA_OPTION, default_value: 0) >= ELEMENTOR_MCP_CONNECTIONS_SCHEMA_VERSION) {
        return;
    }
...
function elementor_mcp_connections_schema_install(): void
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
    ...
    $table = elementor_mcp_connections_table();
    ...
    update_option(ELEMENTOR_MCP_CONNECTIONS_SCHEMA_OPTION, ELEMENTOR_MCP_CONNECTIONS_SCHEMA_VERSION, autoload: false);
}
 ...
 */
function elementor_mcp_connection_credential(string $method, int $user_id): ?array
{
    // @mago-expect lint:no-insecure-comparison -- transport slug, not a secret.
    if ($method === 'token') {
        return elementor_mcp_connection_token_credential();
    }

    if ($method === 'oauth') {
        $identity = function_exists('ElementorMCP\\OAuth\\Middleware\\request_oauth_identity')
            ? \ElementorMCP\OAuth\Middleware\request_oauth_identity()
            : null;
        ...
        return [
            ...
            'label' => __('OAuth client', domain: 'elementor-mcp'),
        ];
    }
    ...
    return ['key' => $uuid, 'label' => elementor_mcp_connection_password_label($user_id, $uuid)];
}
 ...
 * @return array{key: string, label: string}|null
 */
function elementor_mcp_connection_token_credential(): ?array
{
    $identity = function_exists('ElementorMCP\\OAuth\\Middleware\\request_oauth_identity')
        ? \ElementorMCP\OAuth\Middleware\request_oauth_identity()
        : null;
    ...
    $name = $token_id > 0 ? elementor_mcp_token_name($token_id) : '';

    return ['key' => $client_id, 'label' => $name !== '' ? $name : __('Access token', domain: 'elementor-mcp')];
}
 ...
 */
function elementor_mcp_current_agent(?array $set = null): array
{
 ...
 * @return array{method: string, credential: string, label: string, client: string, client_version: string}
 */
function elementor_mcp_resolve_agent(string $method, int $user_id, array $client): array
{
    $credential = elementor_mcp_connection_credential($method, $user_id);
 ...
 */
function elementor_mcp_connection_password_label(int $user_id, string $uuid): string
{
 ...
 */
function elementor_mcp_connection_client_ttl(): int
{
 ...
 */
function elementor_mcp_connection_client(WP_REST_Request $request): array
{
    $session = trim((string) $request->get_header('mcp_session_id'));

    $introduced = elementor_mcp_connection_client_from_handshake($request->get_json_params());
    if ($introduced !== null) {
        ...
        // brand-new session to elementor_mcp_remember_connection_client().
        if ($session !== '') {
            set_transient(elementor_mcp_connection_session_key($session), $introduced, elementor_mcp_connection_client_ttl());
        }
    ...
    if ($session !== '') {
        ...
        $remembered = get_transient(elementor_mcp_connection_session_key($session));
    ...
    if ($agent !== '' && elementor_mcp_client_key($agent) !== null) {
 ...
 */
function elementor_mcp_connection_client_from_handshake(mixed $params): ?array
{
...
function elementor_mcp_connection_session_key(string $session): string
{
    return 'elementor_mcp_client_' . md5($session);
}
 ...
 * @return mixed
 */
function elementor_mcp_remember_connection_client(mixed $response, mixed $server, WP_REST_Request $request): mixed
{
    if (!$response instanceof WP_REST_Response || !str_starts_with($request->get_route(), '/mcp/')) {
        return $response;
    }

    $client = elementor_mcp_connection_client_from_handshake($request->get_json_params());
    if ($client === null) {
        return $response;
    }
    ...
    if ($session !== '') {
        set_transient(elementor_mcp_connection_session_key($session), $client, elementor_mcp_connection_client_ttl());
    }
 ...
 */
function elementor_mcp_connection_client_key(string $client_name): string
{
    if ($client_name === '') {
        return '';
    }

    return elementor_mcp_client_key($client_name) ?? substr(md5(strtolower($client_name)), offset: 0, length: 32);
}
 ...
 */
function elementor_mcp_record_connection(string $method, int $user_id, array $client = ['name' => '', 'version' => '']): void
{
    if ($user_id <= 0) {
        return;
    }
    ...
    $sql = $wpdb->prepare(
        ...
        $credential['key'],
        $credential['label'],
        elementor_mcp_connection_client_key($client['name']),
        $client['name'],
        $client['version'],
        $now,
        $now,
 ...
 */
function elementor_mcp_get_connections(int $limit = 50): array
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
 ...
 */
function elementor_mcp_connection_credential_exists(string $method, string $credential_key, int $user_id): bool
{
 ...
 */
function elementor_mcp_forget_connection(int $id): int
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
    ...
    $deleted = $wpdb->delete(elementor_mcp_connections_table(), ['id' => $id], ['%d']);
 ...
 */
function elementor_mcp_forget_stale_connections(): int
{
    $removed = 0;
    ...
    foreach (elementor_mcp_get_connections(limit: 200) as $connection) {
        ...
        if ($id <= 0 || elementor_mcp_connection_credential_exists($method, $key, $user_id)) {
            continue;
        }
...
add_action('plugins_loaded', callback: 'elementor_mcp_connections_schema_maybe_install');

add_filter('rest_post_dispatch', callback: 'elementor_mcp_remember_connection_client', priority: 20, accepted_args: 3);
