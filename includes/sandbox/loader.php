 ...
 *
 * The web-server guards written by elementor_mcp_harden_sandbox_dir() live in the
 ...
 */
function elementor_mcp_sandbox_loadable_files(string $sandbox_dir): array
{
    ...
    $guards = elementor_mcp_sandbox_guard_files();
 ...
 */
function elementor_mcp_sandbox_crash_handler(string $crashed_file, ?string $current_sandbox_file): void
{
...
(static function () {
    $sandbox_dir = ELEMENTOR_MCP_SANDBOX_DIR;
    ...
    if (!$is_safe_mode && ($_GET['elementor_mcp_safe_mode'] ?? null) === '1') {
        $is_safe_mode = true;
    }
    ...
    add_action('admin_notices', static function () use ($crashed_file) {
        ...
        if (file_exists($crashed_file)) {
            wp_admin_notice(
                sprintf(
                    '<strong>%s</strong> %s',
                    esc_html__('Elementor MCP Sandbox: Safe mode is active.', domain: 'elementor-mcp'),
                    esc_html__(
                        'A sandbox plugin caused a fatal error. All sandbox plugins are disabled. Fix or delete the broken plugin, then delete wp-content/elementor-mcp-sandbox/.crashed to resume.',
    ...
    register_shutdown_function(static function () use ($crashed_file, &$current_sandbox_file) {
        elementor_mcp_sandbox_crash_handler($crashed_file, $current_sandbox_file);
    });
