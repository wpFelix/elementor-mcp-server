 ...
 *
 * The sandbox is an operational boundary for generated PHP: it gives Elementor MCP
 ...
 * Authenticated Elementor MCP administrators may intentionally read, write, edit,
 ...
 */
function elementor_mcp_get_sandbox_dir($ensure_exists = false)
{
    if ($ensure_exists && !is_dir(ELEMENTOR_MCP_SANDBOX_DIR)) {
        wp_mkdir_p(ELEMENTOR_MCP_SANDBOX_DIR);
        elementor_mcp_harden_sandbox_dir();
    }

    return ELEMENTOR_MCP_SANDBOX_DIR;
}
 ...
 */
function elementor_mcp_sandbox_guard_files(): array
{
    return [
        'index.php' => "<?php\n// Silence is golden.\n",
        '.htaccess' =>
            "# Elementor MCP sandbox: agent-authored PHP. Never serve this directory over HTTP.\n"
 ...
 */
function elementor_mcp_harden_sandbox_dir(): void
{
    if (!is_dir(ELEMENTOR_MCP_SANDBOX_DIR)) {
        return;
    }

    foreach (elementor_mcp_sandbox_guard_files() as $basename => $contents) {
        $path = ELEMENTOR_MCP_SANDBOX_DIR . $basename;
        if (file_exists($path)) {
            continue;
        }
 ...
 */
function elementor_mcp_is_sandbox_guard_file(string $resolved): bool
{
    $real_sandbox = realpath(ELEMENTOR_MCP_SANDBOX_DIR);
    ...
    return array_key_exists(basename($resolved), elementor_mcp_sandbox_guard_files());
}
 ...
 */
function elementor_mcp_validate_sandbox_path($resolved)
{
    $sandbox_dir = elementor_mcp_get_sandbox_dir();
    $real_sandbox = realpath($sandbox_dir);
    if ($real_sandbox === false) {
        return new WP_Error('sandbox_not_found', __('The sandbox directory does not exist.', domain: 'elementor-mcp'));
    }
    ...
    if (!elementor_mcp_path_is_child_of_directory($real_resolved, $real_sandbox)) {
        return new WP_Error('outside_sandbox', sprintf(
            /* translators: %s: sandbox directory path */
            __('Only files inside the sandbox (%s) can be modified.', domain: 'elementor-mcp'),
            $sandbox_dir,
        ));
    }
 ...
 *
 * This is deliberately scoped to files that Elementor MCP may execute as PHP or
 ...
 */
function elementor_mcp_check_php_sandbox(string $resolved): bool|WP_Error
{
    $sandbox_dir = elementor_mcp_get_sandbox_dir(ensure_exists: false);
    $real_sandbox = realpath($sandbox_dir);
    ...
    if (!elementor_mcp_path_is_within_directory($parent_dir, $real_sandbox)) {
        return new WP_Error('php_sandbox_required', sprintf(
            'PHP files and PHP execution control files can only be written to the sandbox directory: %s. Use a path like "wp-content/elementor-mcp-sandbox/my-feature.php".',
            $sandbox_dir,
        ));
    }
    ...
    if (elementor_mcp_is_sandbox_guard_file($resolved)) {
 ...
 */
function elementor_mcp_path_requires_php_sandbox(string $resolved): bool
{
 ...
 */
function elementor_mcp_check_php_execution_sandbox(string $resolved): bool|WP_Error
{
    if (!elementor_mcp_path_requires_php_sandbox($resolved)) {
        return true;
    }

    return elementor_mcp_check_php_sandbox($resolved);
}
