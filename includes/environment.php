 ...
 *
 * Elementor MCP writes to the PHP error log only behind this. A plugin that logs
 ...
 */
function elementor_mcp_debug_logging_enabled(): bool
{
 ...
 * @return bool
 */
function elementor_mcp_is_enabled()
{
    /** @var mixed $value */
    $value = get_option('elementor_mcp_ai_abilities_enabled', default_value: false);
    ...
    $locked_domain = get_option('elementor_mcp_ai_abilities_domain', default_value: '');
 ...
 * @return bool
 */
function elementor_mcp_looks_like_production(): bool
{
    $host = elementor_mcp_normalized_home_host();
    if ($host === '') {
        return true;
    }
    ...
    return (
        !elementor_mcp_host_has_non_production_tld($host)
        && !elementor_mcp_host_has_non_production_subdomain_segment($host)
        && !elementor_mcp_host_has_non_production_keyword($host)
        && !elementor_mcp_host_has_non_production_suffix($host)
        && !elementor_mcp_wp_environment_is_non_production()
    );
}

function elementor_mcp_normalized_home_host(): string
{
...
function elementor_mcp_host_has_non_production_tld(string $host): bool
{
    $segments = explode('.', $host);
    $tld = end($segments);

    /** @var array<int, string> $non_prod_tlds */
    $non_prod_tlds = apply_filters('elementor_mcp_non_production_tlds', [
        'dev',
        'local',
        'staging',
        'test',
...
function elementor_mcp_host_has_non_production_subdomain_segment(string $host): bool
{
    /** @var array<int, string> $non_prod_subdomain_segments */
    $non_prod_subdomain_segments = apply_filters('elementor_mcp_non_production_subdomain_segments', [
        'dev',
        'local',
        'test',
        'staging',
...
function elementor_mcp_host_has_non_production_keyword(string $host): bool
{
    /** @var array<int, string> $non_prod_keyword_regex_words */
    $non_prod_keyword_regex_words = apply_filters('elementor_mcp_non_production_keyword_words', [
        'test',
        'dev',
        'staging',
        'stage',
...
function elementor_mcp_host_has_non_production_suffix(string $host): bool
{
    /** @var array<int, string> $non_prod_host_suffixes */
    $non_prod_host_suffixes = apply_filters('elementor_mcp_production_host_patterns', [
        'wpengine.com',
        'wpenginepowered.com',
        'sg-host.com',
        'cloudwaysapps.com',
...
function elementor_mcp_wp_environment_is_non_production(): bool
{
 ...
 */
function elementor_mcp_likely_local_http(): bool
{
    $home = home_url();
    ...
    $local_substrings = apply_filters('elementor_mcp_self_signed_host_patterns', [
        '.local',
        '.test',
        'localhost',
        '.lndo.site',
 ...
 * in elementor_mcp_build_oauth_configs(): this covers single-label hosts (e.g. "site") and private-IP
 * literals too, not only the `.local` / `.test`-style suffixes, so no local HTTPS site is left
 * without the bypass it needs.
 */
function elementor_mcp_likely_self_signed_https(): bool
{
    return str_starts_with(strtolower(home_url()), 'https://') && elementor_mcp_host_unreachable_from_cloud();
}

/**
 * Has the current user dismissed the production warning?
 */
function elementor_mcp_production_warning_dismissed(): bool
{
    /** @var mixed $value */
    $value = get_user_meta(get_current_user_id(), key: 'elementor_mcp_production_warning_dismissed', single: true);
 ...
 */
function elementor_mcp_handle_dismiss_production_warning(): void
{
    if (($_POST['elementor_mcp_dismiss_production_warning'] ?? null) === null) {
        return;
    }
    ...
    check_admin_referer('elementor_mcp_dismiss_production_warning');

    update_user_meta(get_current_user_id(), meta_key: 'elementor_mcp_production_warning_dismissed', meta_value: '1');
 ...
 * @return bool
 */
function elementor_mcp_is_domain_mismatch()
{
    /** @var mixed $value */
    $value = get_option('elementor_mcp_ai_abilities_enabled', default_value: false);
    if ($value !== '1' && $value !== true) {
 ...
 */
function elementor_mcp_app_passwords_status(): array
{
