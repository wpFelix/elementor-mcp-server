...
namespace ElementorMCP\Troubleshoot\Notice;

if (!defined('ABSPATH')) {
    exit();
}

const DISMISS_META = 'elementor_mcp_troubleshoot_dismissed';
...
function regressions(): array
{
    ...
    // Application Passwords turned off while Elementor MCP passwords have actually been used.
    $status = \elementor_mcp_app_passwords_status();
    if (!$status['available'] && $status['reason'] === 'filtered') {
        foreach (\elementor_mcp_get_mcp_passwords() as $password) {
    ...
    if (!\elementor_mcp_oauth_transport_allowed() && \ElementorMCP\OAuth\ClientValidation\active_client_count() > 0) {
        $found[] = __(
...
function maybe_render(): void
{
    ...
    $dismiss_url = wp_nonce_url(
        add_query_arg('elementor_mcp_troubleshoot_dismiss', $hash),
        action: 'elementor_mcp_troubleshoot_dismiss',
    );
    $connect_url = admin_url('admin.php?page=elementor-mcp-connect');
    echo
        '<div class="notice notice-error"><p><strong>'
            . esc_html__('Elementor MCP connections are broken.', domain: 'elementor-mcp')
            . '</strong></p>'
    ;
    ...
    echo
        '<p><a href="'
            . esc_url($connect_url)
            . '">'
            . esc_html__('Open the Connect page to run diagnostics', domain: 'elementor-mcp')
            . '</a> · <a href="'
            . esc_url($dismiss_url)
            . '">'
            . esc_html__('Dismiss', domain: 'elementor-mcp')
            . '</a></p></div>'
    ;
}
...
{
    $raw = $_GET['elementor_mcp_troubleshoot_dismiss'] ?? null;
    if (!is_string($raw) || $raw === '') {
        return;
    }
    check_admin_referer('elementor_mcp_troubleshoot_dismiss');
    if (!\elementor_mcp_current_user_can_manage()) {
        return;
    }
    update_user_meta(get_current_user_id(), DISMISS_META, sanitize_key($raw));
    wp_safe_redirect(remove_query_arg(['elementor_mcp_troubleshoot_dismiss', '_wpnonce']));
    exit();
}
