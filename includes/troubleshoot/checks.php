...
namespace ElementorMCP\Troubleshoot\Checks;

use ElementorMCP\OAuth\Repositories\ClientRepository;
 ...
 * local-only HTTPS host (self-signed or local-CA certs, see elementor_mcp_likely_self_signed_https)
...
{
    return ['timeout' => HTTP_TIMEOUT, 'sslverify' => !\elementor_mcp_likely_self_signed_https()];
}
...
function run_all(?string $method = null): array
{
    ...
    $results[] = $abilities['status'] === 'fail'
        ? skipped('mcp_handshake', __('MCP endpoint', domain: 'elementor-mcp'), $abilities, $transport, $abilities)
        : check_mcp_handshake();
    ...
    if ($include_oauth) {
        ...
        // ElementorMCP\OAuth\boot), so the probes would only restate that failure.
        $oauth_ok = $abilities['status'] !== 'fail' && $transport['status'] !== 'fail' && $schema['status'] !== 'fail';
        $discovery_headers = [];
        $discovery = $oauth_ok
            ? check_discovery($discovery_headers)
            : skipped('discovery', __('OAuth discovery', domain: 'elementor-mcp'), $abilities, $transport, $schema);
        $results[] = $discovery;
        ...
        $results[] = $oauth_ok
            ...
            : skipped(
                'registration',
                __('OAuth client registration', domain: 'elementor-mcp'),
                $abilities,
                $transport,
                $schema,
            );
        ...
        $results[] = $oauth_ok && $discovery['status'] === 'ok'
            ...
            : skipped(
                'bot_filter',
                __('Hosting bot filter', domain: 'elementor-mcp'),
                $abilities,
                $transport,
                $schema,
                $discovery,
        ...
        $results[] = $schema['status'] !== 'fail'
            ? check_limits()
            : skipped('limits', __('Registration limits', domain: 'elementor-mcp'), $schema);
...
function check_abilities(): array
{
    ...
    if (\elementor_mcp_is_enabled()) {
        return ok(
            'abilities',
            $label,
            __('AI Abilities are turned on; the MCP endpoints are registered.', domain: 'elementor-mcp'),
        );
    }
    return fail(
        'abilities',
        ...
        __('Turn on AI Abilities in Step 1 of the Connect page, then run these checks again.', domain: 'elementor-mcp'),
    );
}
...
function skipped(string $id, string $label, array ...$failed): array
{
    ...
    return result(
        ...
        message: sprintf(
            ...
            __('Skipped: fix "%s" first.', domain: 'elementor-mcp'),
            implode('", "', $names),
        ),
    );
}
...
{
    $label = __('Secure transport', domain: 'elementor-mcp');
    ...
    return fail(
        ...
        $label,
        __(
            'This site is served over plain HTTP on a non-local environment. Elementor MCP does not register any OAuth endpoint here, because authorization codes and tokens would travel in cleartext.',
            domain: 'elementor-mcp',
        ),
        __(
...
{
    $label = __('Permalink structure', domain: 'elementor-mcp');
 ...
 *
 * The sandbox holds agent-authored PHP under wp-content. Elementor MCP writes
...
{
    $label = __('PHP sandbox exposure', domain: 'elementor-mcp');

    if (!is_dir(ELEMENTOR_MCP_SANDBOX_DIR)) {
        return ok(
            'sandbox_exposure',
            $label,
            __('The sandbox directory does not exist yet, so there is nothing to serve.', domain: 'elementor-mcp'),
        );
    }

    $missing = [];
    foreach (array_keys(\elementor_mcp_sandbox_guard_files()) as $basename) {
        if (!file_exists(ELEMENTOR_MCP_SANDBOX_DIR . $basename)) {
            $missing[] = $basename;
        }
    }
    ...
    $probe_url = content_url('elementor-mcp-sandbox/index.php');
    ...
    if (is_wp_error($response)) {
        return warn(
            ...
            sprintf(
                ...
                __('Could not probe the sandbox directory over HTTP: %s', domain: 'elementor-mcp'),
                $response->get_error_message(),
            ),
            sprintf(
                /* translators: %s: sandbox URL */
                __('Open %s in a private browser window. It must not return a page.', domain: 'elementor-mcp'),
                $probe_url,
            ),
        );
    }
    ...
    if ($code >= 400) {
        return ok(
            ...
            sprintf(
                ...
                __('The web server refuses to serve the sandbox directory (HTTP %d).', domain: 'elementor-mcp'),
                $code,
            ),
        );
    }
    ...
    $server_hint = $missing === []
        ...
        : sprintf(
            ...
            __('These guard files are missing from the sandbox directory: %s.', domain: 'elementor-mcp'),
            implode(', ', $missing),
        );
    ...
    return fail(
        ...
        sprintf(
            ...
            __(
                'The sandbox directory is served over HTTP (HTTP %1$d at %2$s). Any PHP an agent writes there can be executed by anyone who knows the filename, outside WordPress and outside the Elementor MCP safety policy.',
                domain: 'elementor-mcp',
            ),
            $code,
            $probe_url,
        ...
        $server_hint . ' '
            . __(
                'Block the directory in your server configuration. For nginx, add inside the server block: location ^~ /wp-content/elementor-mcp-sandbox/ { deny all; return 404; } — then reload nginx and run these checks again.',
                domain: 'elementor-mcp',
            ),
    );
}
...
{
    $label = __('OAuth storage', domain: 'elementor-mcp');
    ...
    if (is_string($found) && $found === $table) {
        return ok('schema', $label, __('The OAuth tables are installed.', domain: 'elementor-mcp'));
    }
    return fail(
        'schema',
        $label,
        __('The OAuth tables are missing, so no OAuth client can register or authenticate.', domain: 'elementor-mcp'),
        __(
            'Deactivate and reactivate Elementor MCP to re-run the installer, then run these checks again.',
            domain: 'elementor-mcp',
        ),
    );
}
...
{
    $label = __('Anonymous REST API', domain: 'elementor-mcp');
    ...
    if (is_wp_error($response)) {
        return fail(
            ...
            sprintf(
                /* translators: %s: HTTP error message */
                __('The site could not fetch its own REST index: %s', domain: 'elementor-mcp'),
                $response->get_error_message(),
            ),
            __(
    ...
    if ($code === 401 || $code === 403) {
        return fail(
            ...
            __(
                'In your security plugin, allow unauthenticated access to the REST API (at minimum the elementor-mcp/v1 and mcp namespaces), then run these checks again.',
                domain: 'elementor-mcp',
            ),
        );
    }
    if ($code !== 200) {
        return warn(
            ...
            sprintf(
                /* translators: %d: HTTP status code */
                __('The REST index answered HTTP %d instead of 200.', domain: 'elementor-mcp'),
                $code,
            ),
        );
    }
    return ok('rest', $label, __('The REST API answers anonymous requests.', domain: 'elementor-mcp'));
}
...
{
    $label = __('MCP endpoint', domain: 'elementor-mcp');
    ...
    $options['body'] = (string) wp_json_encode([
        ...
        'params' => [
            ...
            'clientInfo' => ['name' => 'elementor-mcp-diagnostics', 'version' => (string) ELEMENTOR_MCP_VERSION],
        ],
    ]);
    ...
    if ($code === 404) {
        return fail(
            'mcp_handshake',
            $label,
            __('The MCP endpoint is not registered, so no AI client can connect.', domain: 'elementor-mcp'),
            __(
                'Turn on AI Abilities on the Overview screen. If it is already on, the MCP Adapter failed to load — reinstall the Elementor MCP release build ZIP.',
                domain: 'elementor-mcp',
            ),
        );
    }
...
{
    $label = __('OAuth discovery', domain: 'elementor-mcp');
    ...
    $probes = \ElementorMCP\OAuth\Endpoints\Discovery\discovery_probes(home_url(), \ElementorMCP\OAuth\resource_identifier());
...
function probe_discovery_document(array $probe, string $label, array &$headers): array
{
        ...
        return [
            ...
            'failure' => fail(
                ...
                sprintf(
                    ...
                    __('Fetching %1$s failed: %2$s', domain: 'elementor-mcp'),
                    $probe['url'],
                    $response->get_error_message(),
                ),
            ),
    ...
    if ($code >= 300 && $code < 400) {
        ...
        $location = is_string($location) && $location !== '' ? $location : __('another URL', domain: 'elementor-mcp');
        return [
            'ok' => false,
            'failure' => fail(
                'discovery',
...
{
    $label = __('OAuth client registration', domain: 'elementor-mcp');
    ...
    set_transient('elementor_mcp_oauth_selftest_' . hash('sha256', $token), value: '1', expiration: MINUTE_IN_SECONDS);
    $response = wp_remote_post(rest_url('elementor-mcp/v1/oauth/register'), [
        'timeout' => HTTP_TIMEOUT,
        'sslverify' => !\elementor_mcp_likely_self_signed_https(),
        'headers' => ['Content-Type' => 'application/json', 'X-Elementor MCP-Self-Test' => $token],
        'body' => (string) wp_json_encode([
            'client_name' => 'Elementor MCP Self-Test',
    ...
    if (is_wp_error($response)) {
        return fail(
            ...
            sprintf(
                /* translators: %s: HTTP error message */
                __('The registration test request failed: %s', domain: 'elementor-mcp'),
                $response->get_error_message(),
            ),
        );
    }
    ...
    return fail(
        ...
        __(
            'Allow anonymous POST requests to /wp-json/elementor-mcp/v1/oauth/register, then run these checks again.',
            domain: 'elementor-mcp',
        ),
    );
}
...
{
    /** @var mixed $agents */
    $agents = apply_filters('elementor_mcp_troubleshoot_bot_filter_user_agents', [
        'python-httpx/0.28.1',
...
{
    $label = __('Security & edge layers', domain: 'elementor-mcp');
    ...
    if ($found === []) {
        return ok(
            'security_edge',
            $label,
            __('No CDN, WAF, or security plugin was detected in front of the site.', domain: 'elementor-mcp'),
        );
    }
    ...
    return info(
        ...
        __(
            'Allow these paths through the CDN/WAF/security layer, by path (not by User-Agent): /wp-json/mcp/elementor-mcp and /.well-known/oauth-* . Keep every other protection on.',
            domain: 'elementor-mcp',
        ),
    );
}
...
{
    $label = __('Hosting bot filter', domain: 'elementor-mcp');
...
{
    $label = __('Registration limits', domain: 'elementor-mcp');
    // @mago-expect lint:no-global
    global $wpdb;
    /** @var \wpdb $wpdb */
    $active = \ElementorMCP\OAuth\ClientValidation\active_client_count();
    $cap = \ElementorMCP\OAuth\ClientValidation\max_clients_per_site();
    $pending_cutoff = gmdate('Y-m-d H:i:s', time() - \ElementorMCP\OAuth\ClientValidation\STALE_UNUSED_CLIENT_TTL);
    // @mago-expect analysis:possibly-invalid-argument
    // @mago-expect analysis:possibly-invalid-argument
    $pending = (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}elementor_mcp_oauth_clients
    ...
    if ($active >= $cap) {
        return fail(
            ...
            __('Revoke unused connections from the Connected Apps page.', domain: 'elementor-mcp'),
        );
    }
    if ($pending >= 5) {
        return warn(
...
{
    $label = __('Serving stack', domain: 'elementor-mcp');
    if ($headers === []) {
        return result(
            'environment',
            status: 'skipped',
            label: $label,
            message: __('No discovery response headers available (see the discovery check).', domain: 'elementor-mcp'),
        );
    }
    $seen = [];
    ...
    $stack = $seen === [] ? __('No identifying headers observed.', domain: 'elementor-mcp') : implode(' · ', $seen);
    $note = '';
 ...
 *
 * @param array{site_url: string, elementor_mcp_version: string, wp_version: string, php_version: string, method: string} $meta
...
{
    $lines = [];
    $lines[] = 'Elementor MCP connection diagnostic';
    $lines[] = 'Site: ' . $meta['site_url'];
    $lines[] =
        'Elementor MCP: '
        . $meta['elementor_mcp_version']
        . ' | WordPress: '
        . $meta['wp_version']
        . ' | PHP: '
        . $meta['php_version'];
    ...
    // since different Elementor MCP versions ship different checks, the list itself shows what this
    // version verified.
    if ($checks === []) {
        $lines[] = '- none';
    }
...
{
    $label = __('Application Passwords', domain: 'elementor-mcp');
    $status = \elementor_mcp_app_passwords_status();
    if ($status['available']) {
        return ok(
            'app_passwords',
            $label,
            __('Application Passwords are available for the password connection method.', domain: 'elementor-mcp'),
        );
    }
