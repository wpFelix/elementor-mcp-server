...
namespace ElementorMCP\Troubleshoot;
...
/**
 * Record the moment an authenticated request reaches any Elementor MCP MCP route, per auth method.
...
function record_mcp_request(mixed $result, mixed $server, WP_REST_Request $request): mixed
{
    ...
    $client = \elementor_mcp_connection_client($request);
    ...
    \elementor_mcp_record_connection($method, $user_id, $client);
    ...
    \elementor_mcp_current_agent(\elementor_mcp_resolve_agent($method, $user_id, $client));
    ...
    $stored[$method] = $now;
    update_option('elementor_mcp_mcp_last_request', $stored, autoload: false);
    return $result;
}

/**
 * Map a REST route to the Elementor MCP auth method it implies: the dedicated OAuth server route,
 * or the canonical + legacy-alias routes used by Application Password clients. Non-MCP routes
 * return null. Matching mirrors ElementorMCP\OAuth\Middleware\is_mcp_route (exact slug or slash
 * suffix — `/mcp/elementor-mcp-oauth` must never be treated as a `/mcp/elementor-mcp` prefix match).
 *
...
function mcp_request_method(string $route): ?string
{
    ...
    if (
        function_exists('ElementorMCP\\OAuth\\Middleware\\request_identity_via')
        && \ElementorMCP\OAuth\Middleware\request_identity_via() === 'token'
    ) {
        return 'token';
    }
...
{
    if ($route === '/mcp/elementor-mcp-oauth' || str_starts_with($route, '/mcp/elementor-mcp-oauth/')) {
        return 'oauth';
    }
    if ($route === '/mcp/elementor-mcp' || str_starts_with($route, '/mcp/elementor-mcp/')) {
        return 'password';
    }
