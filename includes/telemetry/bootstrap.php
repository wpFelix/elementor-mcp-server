 ...
 *
 * Opt-in: nothing is sent until an administrator switches it on under Elementor MCP >
 ...
 * scripts/package.sh, and elementor-mcp.php guards the require with file_exists() so
 * that removal is safe — the same arrangement includes/updater.php uses, for
 * the same reason.
 */

add_action(CRON_HOOK, __NAMESPACE__ . '\send');
add_filter('elementor_mcp_settings_sections', __NAMESPACE__ . '\register_setting');
