...
/**
 * Elementor MCP Chat: the $wpdb layer behind chat sessions.
 *
 ...
 */
function elementor_mcp_chat_persist_session_row(string $session_id, int $user_id, array $row, ?int $existing_user_id): void
{
    $wpdb = elementor_mcp_chat_wpdb();
    $table = elementor_mcp_chat_sessions_table();

    if ($existing_user_id !== null) {
        if (elementor_mcp_chat_update_session_row($wpdb, $table, $session_id, $user_id, $row) === false) {
            elementor_mcp_chat_log_storage_failure($session_id);
        }
        return;
    }
    ...
    $recovered = elementor_mcp_chat_update_session_row($wpdb, $table, $session_id, $user_id, $row);
 ...
 */
function elementor_mcp_chat_update_session_row(
    wpdb $wpdb,
    string $table,
    string $session_id,
    int $user_id,
 ...
 */
function elementor_mcp_chat_log_storage_failure(string $session_id): void
{
    $wpdb = elementor_mcp_chat_wpdb();
    ...
    if (elementor_mcp_debug_logging_enabled()) {
        // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
        error_log(sprintf('Elementor MCP Chat: failed to persist session %s. %s', $session_id, $wpdb->last_error));
    }
}

function elementor_mcp_chat_ensure_storage_ready(): void
{
    if (function_exists('elementor_mcp_chat_schema_maybe_install')) {
        elementor_mcp_chat_schema_maybe_install();
    }
}
 ...
 */
function elementor_mcp_chat_select_sessions_for_user(wpdb $wpdb, string $table, int $user_id): array
{
    ...
    $sql = $wpdb->prepare(
        ...
        $user_id,
        ELEMENTOR_MCP_CHAT_MAX_SESSIONS_PER_USER,
    );
    if (!is_string($sql)) {
        return [];
    }
    ...
    $clean = [];
    foreach ($rows as $row) {
        $clean[] = elementor_mcp_chat_assoc_array($row);
    }

    return $clean;
}

function elementor_mcp_chat_select_session_data(wpdb $wpdb, string $table, string $session_id, int $user_id): ?string
{
    // @mago-expect analysis:possibly-invalid-argument -- The table name is derived from $wpdb->prefix.
...
function elementor_mcp_chat_select_session_owner(wpdb $wpdb, string $table, string $session_id): ?int
{
    // @mago-expect analysis:possibly-invalid-argument -- The table name is derived from $wpdb->prefix.
 ...
 */
function elementor_mcp_chat_storage_row(array $session, string $session_id, int $user_id, string $json): array
{
    return [
        'id' => $session_id,
        'user_id' => $user_id,
        'created_at' => elementor_mcp_chat_mysql_datetime((int) $session['created_at']),
        'updated_at' => elementor_mcp_chat_mysql_datetime((int) $session['updated_at']),
        'data' => $json,
    ];
}
 ...
 */
function elementor_mcp_chat_decode_session_json(mixed $data, string $session_id): ?array
{
    ...
    $session = elementor_mcp_chat_assoc_array($decoded);
...
function elementor_mcp_chat_max_row_bytes(): int
{
    $max = (int) apply_filters('elementor_mcp_chat_max_row_bytes', ELEMENTOR_MCP_CHAT_MAX_ROW_BYTES);

    return $max > 0 ? $max : ELEMENTOR_MCP_CHAT_MAX_ROW_BYTES;
}

/**
 * @param array<string, mixed> $session
 */
function elementor_mcp_chat_encode_session_for_storage(array &$session): ?string
{
    $json = elementor_mcp_chat_json_encode($session);
    if ($json === null) {
        return null;
    }

    $max_bytes = elementor_mcp_chat_max_row_bytes();
    if (strlen($json) <= $max_bytes) {
        return $json;
    }

    $session = elementor_mcp_chat_prune_attachment_data($session, $max_bytes);
    $json = elementor_mcp_chat_json_encode($session);
 ...
 * @param array<string, mixed> $session
 */
function elementor_mcp_chat_json_encode(array $session): ?string
{
 ...
 */
function elementor_mcp_chat_prune_attachment_data(array $session, int $max_bytes): array
{
...
function elementor_mcp_chat_mysql_datetime(int $timestamp): string
{
    if ($timestamp <= 0) {
        $timestamp = time();
    }
...
function elementor_mcp_chat_prune_sessions_for_user(int $user_id): void
{
    if ($user_id <= 0) {
        return;
    }
    ...
    $wpdb = elementor_mcp_chat_wpdb();
    $table = elementor_mcp_chat_sessions_table();
    $old_ids = elementor_mcp_chat_select_prunable_session_ids($wpdb, $table, $user_id);
 ...
 * @return list<string>
 */
function elementor_mcp_chat_select_prunable_session_ids(wpdb $wpdb, string $table, int $user_id): array
{
    // @mago-expect analysis:possibly-invalid-argument -- The table name is derived from $wpdb->prefix.
    $sql = $wpdb->prepare(
