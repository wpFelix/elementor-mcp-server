...
/**
 * Elementor MCP Chat: the discovery meta-tools.
 *
 ...
 */
function elementor_mcp_chat_meta_tool_specs(): array
{
    return [
        [
            'name' => 'discover-abilities',
            'description' => 'Discover the WordPress abilities available in this system. Returns the list of ability names (which are descriptive, e.g. elementor-mcp/update-post). Call this first to find the exact ability name for a task, then copy the name verbatim; use get-ability-info if a name is unclear.',
            'schema' => [
                'type' => 'object',
        ...
        [
            ...
            'schema' => [
                ...
                'properties' => [
                    'ability_name' => [
                        'type' => 'string',
                        'description' => 'The full ability name from discover-abilities, e.g. elementor-mcp/update-post.',
                    ],
                ],
 ...
 * @return list<string>
 */
function elementor_mcp_chat_discovery_meta_names(): array
{
    return ['discover-abilities', 'get-ability-info'];
}

function elementor_mcp_chat_is_discovery_meta(string $function_name): bool
{
    return in_array($function_name, elementor_mcp_chat_discovery_meta_names(), strict: true);
}
 ...
 */
function elementor_mcp_chat_run_meta_discovery(string $function_name, array $args, array $tools): array
{
    if ($function_name === 'discover-abilities') {
        return elementor_mcp_chat_meta_discover($tools);
    }
    if ($function_name === 'get-ability-info') {
        return elementor_mcp_chat_meta_ability_info($args, $tools);
    }
 ...
 * @return array<string, mixed>
 */
function elementor_mcp_chat_meta_discover(array $tools): array
{
    $abilities = [];
    ...
    return [
        'elementor_mcp_instructions' => elementor_mcp_chat_server_instructions(),
        'abilities' => $abilities,
    ];
}
 ...
 * @return array<string, mixed>
 */
function elementor_mcp_chat_meta_ability_info(array $args, array $tools): array
{
    $name = is_string($args['ability_name'] ?? null) ? $args['ability_name'] : '';
    $tool = elementor_mcp_chat_find_tool($name, $tools);
    if ($tool === null) {
 ...
 * @return array<string, mixed>
 */
function elementor_mcp_chat_execute_meta_call(array $session, int $call_index): array
{
    ...
    $result = elementor_mcp_chat_run_meta_discovery($function_name, $arguments, elementor_mcp_chat_discover_tools());
