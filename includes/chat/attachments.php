...
/**
 * Elementor MCP Chat: image attachments on user messages.
 *
 ...
 */
function elementor_mcp_chat_request_attachments(mixed $params): array|WP_Error
{
    ...
    if (count($items) > ELEMENTOR_MCP_CHAT_MAX_ATTACHMENTS) {
        return new WP_Error(
            'elementor_mcp_chat_too_many_attachments',
            sprintf(
                /* translators: %d: Maximum number of attachments. */
                __('Attach up to %d images per message.', domain: 'elementor-mcp'),
                ELEMENTOR_MCP_CHAT_MAX_ATTACHMENTS,
            ),
            ['status' => 400],
        );
    }
    ...
    foreach ($items as $item) {
        if (!is_array($item)) {
            return elementor_mcp_chat_bad_attachment();
        }

        $attachment = elementor_mcp_chat_normalize_attachment($item);
 ...
 */
function elementor_mcp_chat_normalize_attachment(array $item): array|WP_Error
{
    ...
    if ($name === '') {
        $name = __('Attached image', domain: 'elementor-mcp');
    }

    $allowed_mime_types = ['image/jpeg', 'image/png', 'image/webp', 'image/gif'];
    if (!in_array($mime_type, $allowed_mime_types, strict: true)) {
        return new WP_Error(
            'elementor_mcp_chat_unsupported_attachment',
            __('Only JPEG, PNG, WebP, and GIF image attachments are supported.', domain: 'elementor-mcp'),
            ['status' => 400],
        );
    }

    $parsed = elementor_mcp_chat_parse_attachment_data($data, $mime_type, $size);
 ...
 */
function elementor_mcp_chat_parse_attachment_data(string $data, string $mime_type, int $size): array|WP_Error
{
    ...
    if ($decoded_size < 1 || $decoded_size > ELEMENTOR_MCP_CHAT_MAX_IMAGE_BYTES) {
        return elementor_mcp_chat_attachment_too_large();
    }
...
function elementor_mcp_chat_attachment_too_large(): WP_Error
{
    return new WP_Error(
        'elementor_mcp_chat_attachment_too_large',
        __('Each image attachment must be 3 MB or smaller.', domain: 'elementor-mcp'),
        ['status' => 400],
    );
}

function elementor_mcp_chat_bad_attachment(): WP_Error
{
    return new WP_Error('elementor_mcp_chat_bad_attachment', __('Invalid image attachment.', domain: 'elementor-mcp'), [
        'status' => 400,
    ]);
}
 ...
 */
function elementor_mcp_chat_message_attachments(array $message): array
{
    ...
    foreach ($items as $item) {
        ...
        $attachment = elementor_mcp_chat_message_attachment($item);
        if ($attachment === null) {
            continue;
        }
        $attachments[] = $attachment;
 ...
 */
function elementor_mcp_chat_message_attachment(array $item): ?array
{
