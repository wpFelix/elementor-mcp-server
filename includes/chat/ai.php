...
/**
 * Elementor MCP Chat: talking to the model.
 *
...
function elementor_mcp_chat_system_instruction(): string
{
    $lines = [
        'You are Elementor MCP Chat inside WordPress admin.',
...
/**
 * The shared Elementor MCP server instructions (environment, WordPress-native guidance,
 ...
 */
function elementor_mcp_chat_server_instructions(): string
{
    if (!function_exists('elementor_mcp_build_server_instructions')) {
        return '';
    }

    /** @var mixed $instructions */
    $instructions = apply_filters('elementor_mcp_discover_abilities_instructions', elementor_mcp_build_server_instructions());
 ...
 */
function elementor_mcp_chat_generate_native_step(array $session, array $tools): array|WP_Error
{
    if (!function_exists('wp_ai_client_prompt')) {
        return new WP_Error(
            'elementor_mcp_chat_missing_ai_client',
            __('WordPress AI text generation is not available.', domain: 'elementor-mcp'),
            ['status' => 503],
        );
    }

    $declarations = elementor_mcp_chat_build_function_declarations($tools);
    ...
    $messages = elementor_mcp_chat_build_ai_history($session);
    if ($messages === []) {
        return new WP_Error('elementor_mcp_chat_empty_history', __('The session has no prompt history.', domain: 'elementor-mcp'), [
            'status' => 400,
        ]);
    }
    ...
    if (!is_object($builder)) {
        return new WP_Error(
            'elementor_mcp_chat_bad_ai_builder',
            __('WordPress AI Client did not return a prompt builder.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
    ...
    $configured = $builder
        ...
        // @mago-expect analysis:mixed-method-access
        ->using_system_instruction(elementor_mcp_chat_system_instruction())
        // @mago-expect analysis:mixed-method-access
        ->using_function_declarations(...$declarations);
    if (!is_object($configured)) {
        return new WP_Error(
            'elementor_mcp_chat_bad_ai_builder',
            __('WordPress AI Client prompt builder cannot configure native tools.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
    ...
    if ($supported !== true) {
        return new WP_Error(
            'elementor_mcp_chat_native_tools_unsupported',
            __(
    ...
    if (!is_object($result)) {
        return new WP_Error(
            'elementor_mcp_chat_bad_model_response',
            __('The model did not return a native AI Client result.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }

    return elementor_mcp_chat_parse_native_result($result);
}
 ...
 * @return list<object>
 */
function elementor_mcp_chat_build_ai_history(array $session): array
{
    $events = array_merge(elementor_mcp_chat_build_ai_text_events($session), elementor_mcp_chat_build_ai_tool_events($session));
 ...
 */
function elementor_mcp_chat_build_ai_text_events(array $session): array
{
    $events = [];
    foreach (elementor_mcp_chat_session_list($session, key: 'messages') as $message) {
        $role = is_string($message['role'] ?? null) ? $message['role'] : '';
        if ($role === 'tool') {
            continue;
        }
        $content = is_string($message['content'] ?? null) ? $message['content'] : '';
        $attachments = elementor_mcp_chat_message_attachments($message);
        ...
        $events[] = [
            ...
            'message' => elementor_mcp_chat_text_message($role, $content, $attachments),
        ];
    }
 ...
 * @return list<array{created_at: int, order: int, message: object}>
 */
function elementor_mcp_chat_build_ai_tool_events(array $session): array
{
    $events = [];
    foreach (elementor_mcp_chat_session_list($session, key: 'tool_calls') as $tool_call) {
        $function_call_message = elementor_mcp_chat_tool_call_function_message($tool_call);
        ...
        $function_response_message = elementor_mcp_chat_tool_call_response_message($tool_call);
 ...
 */
function elementor_mcp_chat_tool_call_function_message(array $tool_call): ?object
{
    ...
    $function_name = is_string($tool_call['function_name'] ?? null)
        ...
        : elementor_mcp_chat_ability_to_function_name($ability);
    $arguments = elementor_mcp_chat_model_arguments($tool_call);
 ...
 * @param array<string, mixed> $tool_call
 */
function elementor_mcp_chat_tool_call_response_message(array $tool_call): ?object
{
    $call_id = is_string($tool_call['id'] ?? null) ? $tool_call['id'] : '';
    if ($call_id === '') {
        return null;
    ...
    if (!elementor_mcp_chat_tool_call_has_response($status)) {
        return null;
    }
    ...
    return new \WordPress\AiClient\Messages\DTO\Message(\WordPress\AiClient\Messages\Enums\MessageRoleEnum::user(), [new \WordPress\AiClient\Messages\DTO\MessagePart(
        new \WordPress\AiClient\Tools\DTO\FunctionResponse(
            $call_id,
            $function_name,
            elementor_mcp_chat_tool_call_response_payload($tool_call, $status),
        ),
    )]);
}

function elementor_mcp_chat_tool_call_has_response(string $status): bool
{
 ...
 * @param array<string, mixed> $tool_call
 */
function elementor_mcp_chat_tool_call_response_payload(array $tool_call, string $status): mixed
{
    if ($status === 'succeeded') {
    ...
    return [
        'error' => $error !== '' ? $error : __('Tool execution did not complete.', domain: 'elementor-mcp'),
    ];
}
 ...
 */
function elementor_mcp_chat_text_message(string $role, string $content, array $attachments = []): object
{
 ...
 * @return array{content: string, complete: bool, tool_calls: list<array<string, mixed>>}|WP_Error
 */
function elementor_mcp_chat_parse_native_result(object $result): array|WP_Error
{
    if (!method_exists($result, 'toMessages') && !method_exists($result, 'toMessage')) {
        return new WP_Error(
            'elementor_mcp_chat_bad_model_response',
            __('The AI Client result cannot expose native messages.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
    ...
    foreach ($messages as $message) {
        $parsed = elementor_mcp_chat_parse_native_message($message);
    ...
    if ($text === '' && $tool_calls === []) {
        return new WP_Error(
            'elementor_mcp_chat_empty_model_response',
            __('The model returned no text or native tool calls.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
 ...
 */
function elementor_mcp_chat_parse_native_message(object $message): array
{
    ...
    foreach ($parts as $part) {
        ...
        $parsed = elementor_mcp_chat_parse_native_part($part);
        if (is_string($parsed)) {
            $content[] = $parsed;
            continue;
        }
 ...
 */
function elementor_mcp_chat_parse_native_part(object $part): string|array|null
{
    ...
    $text = elementor_mcp_chat_parse_native_text_part($part, $type);
    if ($text !== null) {
        return $text;
    }

    return elementor_mcp_chat_parse_native_function_call_part($part, $type);
}

function elementor_mcp_chat_parse_native_text_part(object $part, object $type): ?string
{
    if (elementor_mcp_chat_native_part_type($type) !== 'text' || !method_exists($part, 'getText')) {
        return null;
    }
...
function elementor_mcp_chat_native_part_type(object $type): string
{
 ...
 */
function elementor_mcp_chat_parse_native_function_call_part(object $part, object $type): ?array
{
    if (elementor_mcp_chat_native_part_type($type) !== 'function_call' || !method_exists($part, 'getFunctionCall')) {
        return null;
    }
    ...
    return [
        ...
        'name' => elementor_mcp_chat_function_name_to_ability($function_name),
