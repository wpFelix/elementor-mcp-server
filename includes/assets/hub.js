...
(function () {
    ...
    var hub = document.querySelector('.elementor-mcp-hub');
    ...
    hub.addEventListener('submit', function (event) {
        ...
        if (!(form instanceof HTMLFormElement) || !form.matches('.elementor-mcp-hub-actions form')) {
            return;
        }

        var actionInput = form.querySelector('input[name="elementor_mcp_ability_hub_action"]');
        if (!actionInput || actionInput.value !== 'toggle_disabled') {
            return;
        }

        var row = form.closest('.elementor-mcp-hub-row');
        ...
        body.set('action', 'elementor_mcp_toggle_ability');
    ...
    hub.addEventListener('click', function (event) {
        if (event.target instanceof HTMLElement && event.target.closest('.elementor-mcp-hub-select-all')) {
    ...
    hub.addEventListener('change', function (event) {
        ...
        if (input.classList.contains('elementor-mcp-hub-select-all-input')) {
            var section = input.closest('.elementor-mcp-hub-section, .elementor-mcp-hub-subsection');
            if (section) {
                section.querySelectorAll('.elementor-mcp-hub-row input[type="checkbox"]').forEach(function (box) {
        ...
        if (input.closest('.elementor-mcp-hub-row')) {
            syncSelectAllStates();
        }
    });
    ...
    function syncSelectAllStates() {
        hub.querySelectorAll('.elementor-mcp-hub-select-all-input').forEach(function (selectAll) {
            var section = selectAll.closest('.elementor-mcp-hub-section, .elementor-mcp-hub-subsection');
            if (!section) {
                return;
            }
            var boxes = section.querySelectorAll('.elementor-mcp-hub-row input[type="checkbox"]');
            var checked = 0;
    ...
    var bulkForm = document.getElementById('elementor-mcp-abilities-bulk');
    if (bulkForm) {
        bulkForm.addEventListener('submit', function (event) {
            if (selectedBulkAction() !== 'disable') {
                return;
            }
            var count = hub.querySelectorAll('.elementor-mcp-hub-row input[type="checkbox"]:checked').length;
            if (count === 0) {
                return;
            }
    ...
    function selectedBulkAction() {
        var top = document.getElementById('elementor-mcp-bulk-action-selector-top');
        var bottom = document.getElementById('elementor-mcp-bulk-action-selector-bottom');
    ...
    function applyToggle(row, button, data) {
        ...
        var subsection = row.closest('.elementor-mcp-hub-subsection');
        if (subsection) {
            refreshSectionMeta(subsection);
        }
        var section = row.closest('.elementor-mcp-hub-section');
        if (section) {
    ...
    // "All disabled" pill, mirroring elementor_mcp_render_ability_header_meta() in PHP.
    function refreshSectionMeta(section) {
        var summary = section.querySelector(':scope > summary');
        if (!summary) {
            return;
        }
        var rows = section.querySelectorAll('.elementor-mcp-hub-row');
        ...
        var pill = summary.querySelector('.elementor-mcp-hub-alloff');
        if (enabled === 0 && total > 0) {
            if (!pill && heading) {
                pill = document.createElement('span');
                pill.className = 'pill status is-disabled elementor-mcp-hub-alloff';
