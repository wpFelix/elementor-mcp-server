 ...
 * SHA-256 digest, presented as `Authorization: Bearer emcp_...`.
 *
...
const ELEMENTOR_MCP_TOKENS_SCHEMA_VERSION = 1;

const ELEMENTOR_MCP_TOKENS_SCHEMA_OPTION = 'elementor_mcp_tokens_schema_version';

/**
 * The prefix every Elementor MCP access token carries.
 *
 * It is what lets the Bearer path tell a Elementor MCP token from an OAuth JWT without
 ...
 */
const ELEMENTOR_MCP_TOKEN_PREFIX = 'emcp_';

function elementor_mcp_tokens_table(): string
{
...
function elementor_mcp_tokens_schema_maybe_install(): void
{
    if ((int) get_option(ELEMENTOR_MCP_TOKENS_SCHEMA_OPTION, default_value: 0) >= ELEMENTOR_MCP_TOKENS_SCHEMA_VERSION) {
        return;
    }
...
function elementor_mcp_tokens_schema_install(): void
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
    ...
    $table = elementor_mcp_tokens_table();
    ...
    update_option(ELEMENTOR_MCP_TOKENS_SCHEMA_OPTION, ELEMENTOR_MCP_TOKENS_SCHEMA_VERSION, autoload: false);
}

/**
 * Whether a presented credential is shaped like a Elementor MCP access token.
 *
 * Shape only — this decides which validator runs, never whether the credential
 * is good.
 */
function elementor_mcp_token_looks_like(string $secret): bool
{
    return str_starts_with($secret, ELEMENTOR_MCP_TOKEN_PREFIX);
}

/**
 * The digest a secret is stored and looked up under.
 */
function elementor_mcp_token_hash(string $secret): string
{
 ...
 */
function elementor_mcp_token_create(int $user_id, string $name, int $ttl_days = 0): array|WP_Error
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */

    elementor_mcp_tokens_schema_maybe_install();

    if ($user_id <= 0 || !elementor_mcp_user_can_manage($user_id)) {
        return new WP_Error('elementor_mcp_token_user', __(
            'Only a user who can manage Elementor MCP can hold an access token.',
    ...
    if ($name === '') {
        $name = __('Access token', domain: 'elementor-mcp');
    }
    ...
    $secret = ELEMENTOR_MCP_TOKEN_PREFIX . rtrim(strtr($base64, from: '+/', to: '-_'), characters: '=');

    $now = current_time('mysql', gmt: true);
    $expires = $ttl_days > 0 ? gmdate('Y-m-d H:i:s', time() + ($ttl_days * DAY_IN_SECONDS)) : null;

    $inserted = $wpdb->insert(elementor_mcp_tokens_table(), [
        'user_id' => $user_id,
        'name' => $name,
        'token_hash' => elementor_mcp_token_hash($secret),
    ...
    if ($inserted === false) {
        return new WP_Error('elementor_mcp_token_store', __('Could not store the new access token.', domain: 'elementor-mcp'));
    }
 ...
 */
function elementor_mcp_token_authenticate(string $secret): ?array
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */

    if (!elementor_mcp_token_looks_like($secret)) {
        return null;
    }
    ...
    $row = $wpdb->get_row(
        (string) $wpdb->prepare(
            ...
            elementor_mcp_token_hash($secret),
        ),
        ARRAY_A,
    );
    ...
    if (!hash_equals((string) $row['token_hash'], elementor_mcp_token_hash($secret))) {
        return null;
    }
    ...
    if (!elementor_mcp_user_can_manage($user_id)) {
        return null;
    }
 ...
 */
function elementor_mcp_token_touch(int $token_id): void
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
 ...
 */
function elementor_mcp_tokens_for_user(int $user_id): array
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
 ...
 */
function elementor_mcp_token_name(int $token_id): string
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
 ...
 */
function elementor_mcp_token_revoke(int $token_id, int $user_id): bool
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
    ...
    $deleted = $wpdb->delete(elementor_mcp_tokens_table(), ['id' => $token_id, 'user_id' => $user_id], ['%d', '%d']);
 ...
 */
function elementor_mcp_tokens_delete_for_user(int $user_id): void
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
    ...
    $wpdb->delete(elementor_mcp_tokens_table(), ['user_id' => $user_id], ['%d']);
}
 ...
 */
function elementor_mcp_tokens_purge_expired(): void
{
    // @mago-expect lint:no-global -- $wpdb is WordPress' database handle.
    global $wpdb;
    /** @var wpdb $wpdb */
...
add_action('deleted_user', static function (mixed $user_id): void {
    elementor_mcp_tokens_delete_for_user((int) $user_id);
});

// Piggybacks on the OAuth garbage collector rather than scheduling a second
// daily event to delete a handful of rows.
add_action('elementor_mcp_oauth_gc', callback: 'elementor_mcp_tokens_purge_expired');
