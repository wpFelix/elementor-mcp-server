...
namespace ElementorMCP\Design\Abilities\VerifyRendered;

use WP_Error;
use WP_Post;
use ElementorMCP\Design\Abilities;
use ElementorMCP\Design\Rendered;
...
function register(): void
{
    ...
    wp_register_ability('elementor-mcp/verify-rendered-page', [
        ...
        'execute_callback' => static function (array $input): array|WP_Error {
            ...
            if ($post_id > 0 && $url !== '') {
                return new WP_Error(
                    'elementor_mcp_rendered_ambiguous',
                    __('Pass post_id or url, not both.', domain: 'elementor-mcp'),
                );
            }
            if ($post_id > 0) {
                $post = get_post($post_id);
                if (!$post instanceof WP_Post) {
                    return new WP_Error('elementor_mcp_rendered_no_post', __('No such post.', domain: 'elementor-mcp'));
                }
                ...
                if ($post->post_status !== 'publish') {
                    return new WP_Error('elementor_mcp_rendered_not_public', sprintf(
            ...
            if ($url === '') {
                return new WP_Error(
                    'elementor_mcp_rendered_no_target',
                    __('Pass either post_id or url.', domain: 'elementor-mcp'),
                );
            }
