...
namespace ElementorMCP\Design\Abilities\Contrast;
...
function register(): void
{
    ...
    wp_register_ability('elementor-mcp/check-contrast', [
        'label' => __('Check Design Contrast', domain: 'elementor-mcp'),
        'description' => __(
        ...
        'execute_callback' => static function (array $input): array|WP_Error {
            ...
            if ($foreground !== '' && $background !== '') {
                ...
                if ($ratio === null) {
                    return new WP_Error(
                        'elementor_mcp_contrast_unreadable_color',
                        __('Both foreground and background must be hex colours.', domain: 'elementor-mcp'),
                    );
                }
            ...
            if ($foreground !== '' && $background === '') {
                return new WP_Error(
                    'elementor_mcp_contrast_missing_background',
                    __('Checking a single pair needs background as well as foreground.', domain: 'elementor-mcp'),
                );
            }
            ...
            if ($slug === '') {
                return new WP_Error(
                    'elementor_mcp_contrast_no_design',
                    __('No design is active. Pass a slug, or pass foreground and background to check one pair.', domain: 'elementor-mcp'),
                );
            }
            ...
            if ($background !== '') {
                ...
                if ($best === '') {
                    return new WP_Error(
                        'elementor_mcp_contrast_no_foreground',
                        __('No colour in this palette can be measured against that background.', domain: 'elementor-mcp'),
                    );
                }
