...
namespace ElementorMCP\Design\Abilities\CheckComposition;
...
function register(): void
{
    ...
    wp_register_ability('elementor-mcp/check-composition', [
        ...
        'execute_callback' => static function (array $input): array|WP_Error {
            ...
            if (!is_array($raw) || $raw === []) {
                return new WP_Error(
                    'elementor_mcp_composition_empty',
                    __('A composition needs at least one block.', domain: 'elementor-mcp'),
                    ['status' => 422],
                );
            }
...
function note(array $blocks): string
{
    ...
    return __(
        'Valid. Keep it with elementor-mcp/memory-save as type "composition" so the next build on this site can start from it rather than from nothing, and diverge from it deliberately rather than by accident.',
        domain: 'elementor-mcp',
    );
}
