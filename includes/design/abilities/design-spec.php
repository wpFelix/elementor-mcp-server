...
namespace ElementorMCP\Design\Abilities\DesignSpec;

use WP_Error;
use ElementorMCP\Design\Abilities;
use ElementorMCP\Design\Spec;
...
function spec_schema(): array
{
    return [
        ...
        'properties' => [
            ...
            'sections' => [
                'type' => 'array',
                'description' => 'The page in order: [{surface, layout, gap, note}]. Layout is one of the named layouts; call elementor-mcp/get-design-spec with no slug to list them.',
                'items' => ['type' => 'object'],
            ],
        ],
...
function register(): void
{
    ...
    wp_register_ability('elementor-mcp/save-design-spec', [
        ...
        'execute_callback' => static function (array $input): array|WP_Error {
            ...
            $raw = $input['spec'] ?? null;
            if (!is_array($raw)) {
                return new WP_Error('elementor_mcp_spec_missing', __('A spec object is required.', domain: 'elementor-mcp'), ['status' => 400]);
            }
            ...
            return [
                ...
                'next' => __(
                    'Compile the spec into builder classes with elementor-mcp/elementor-compile-spec, build with role names rather than class ids, then grade the built page with elementor-mcp/compare-to-spec.',
                    domain: 'elementor-mcp',
                ),
            ];
        },
    ...
    wp_register_ability('elementor-mcp/get-design-spec', [
        'label' => __('Get Design Spec', domain: 'elementor-mcp'),
        'description' => __(
    ...
    wp_register_ability('elementor-mcp/compare-to-spec', [
        'label' => __('Compare To Spec', domain: 'elementor-mcp'),
        'description' => __(
        ...
        'execute_callback' => static function (array $input): array|WP_Error {
            ...
            if ($spec === null) {
                return new WP_Error(
                    'elementor_mcp_spec_absent',
                    __('That design has no spec to grade against. Save one with elementor-mcp/save-design-spec first.', domain: 'elementor-mcp'),
                    ['status' => 404],
                );
            }
            ...
            if (!is_array($actual)) {
                return new WP_Error('elementor_mcp_spec_actual', __('An "actual" object is required.', domain: 'elementor-mcp'), ['status' => 400]);
            }
            ...
            return [
                ...
                'note' => $result['diffs'] === []
                    ? __('Every measured property matches the spec.', domain: 'elementor-mcp')
                    : __(
