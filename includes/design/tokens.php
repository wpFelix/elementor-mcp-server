...
namespace ElementorMCP\Design\Tokens;
...
function css_vars(array $tokens): array
{
    ...
    if ($bg !== '') {
        $vars['--elementor-mcp-bg'] = $bg;
    }
    if ($ink !== '') {
        $vars['--elementor-mcp-ink'] = $ink;
    }
    if ($accent !== '') {
        $vars['--elementor-mcp-accent'] = $accent;
    }
    ...
    if ($heading !== '') {
        $vars['--elementor-mcp-font-heading'] = $heading;
    }
    if ($body !== '') {
        $vars['--elementor-mcp-font-body'] = $body;
    }
    ...
    if ($heading_weight !== '') {
        $vars['--elementor-mcp-weight-heading'] = $heading_weight;
    }
    $body_weight = pick_typography_prop($tokens['typography'], $body_roles, prop: 'fontWeight');
    if ($body_weight !== '') {
        $vars['--elementor-mcp-weight-body'] = $body_weight;
    }
    ...
    foreach ([
        '--elementor-mcp-size-heading' => [$heading_roles, 'fontSize'],
        '--elementor-mcp-size-body' => [$body_roles, 'fontSize'],
    ] as $var => [$roles, $prop]) {
    ...
    foreach ([
        '--elementor-mcp-leading-heading' => [$heading_roles, 'lineHeight'],
        '--elementor-mcp-leading-body' => [$body_roles, 'lineHeight'],
        '--elementor-mcp-tracking-heading' => [$heading_roles, 'letterSpacing'],
        '--elementor-mcp-tracking-body' => [$body_roles, 'letterSpacing'],
    ] as $var => [$roles, $prop]) {
        $value = pick_typography_prop($tokens['typography'], $roles, prop: $prop);
        if ($value !== '') {
    ...
    if ($measure !== '') {
        $vars['--elementor-mcp-measure'] = is_numeric($measure) ? $measure . 'ch' : css_value($measure);
    }
    $radius = $tokens['rounded']['md'] ?? first_value($tokens['rounded']);
    if ($radius !== '') {
        $vars['--elementor-mcp-radius'] = css_length(css_value($radius));
    }
    $space = $tokens['spacing']['md'] ?? first_value($tokens['spacing']);
    if ($space !== '') {
        $vars['--elementor-mcp-space'] = css_length(css_value($space));
    }
    return $vars;
}
