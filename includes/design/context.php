...
namespace ElementorMCP\Design\Context;
 ...
 * `elementor-mcp-design` skill makes the same point at greater length; the fence and
...
function render(): string
{
    ...
    $mode = function_exists('ElementorMCP\\Design\\Gate\\mode') ? \ElementorMCP\Design\Gate\mode() : 'off';
    if ($mode === 'enforce') {
        $lines[] = '';
...
function standards(): array
{
    return [
        ...
        '',
        '- Ask `elementor-mcp/list-layout-grammars` for the composition at each section index and build what it answers. A page whose sections all share one shape is a template with the fills swapped, and a stack of full-width bands is the shape a layout falls into when nobody chose one.',
        ...
        '- Pre-flight with `elementor-mcp/check-design` before calling a page finished, and read the `not_checked` list: what it names, nobody has checked but you.',
    ];
}
...
function cold_start(): string
{
    return implode("\n", [
        ...
        '',
        '- `elementor-mcp/generate-design` resolves a brief into a complete direction — palette, type pairing, scale, corners, spacing and permitted compositions — derived from this site rather than chosen, so two sites with the same brief do not land on the same design.',
        '- `elementor-mcp/adopt-design-from-site` reads what the site already uses, when there is a brand to keep.',
        '- `elementor-mcp/compare-references` turns measurements of competitor pages into what to honour and what to diverge from, when the user can point at some.',
        '',
        'Save it with `elementor-mcp/save-design`, then build against it.',
    ]) . "\n";
}
