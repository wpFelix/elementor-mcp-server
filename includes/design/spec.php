...
namespace ElementorMCP\Design\Spec;

use WP_Error;
use ElementorMCP\Design\Contrast;
use ElementorMCP\Design\Preflight;
...
const META_KEY = '_elementor_mcp_design_spec';
 ...
 * escaping the page edge — and `elementor-mcp/list-layout-grammars` returns all of it
 ...
 * does not land on them. elementormcp.com alone uses 46:54, 36:64 and 54:46, none of
...
function normalize(array $raw): array|WP_Error
{
    ...
    if (!in_array($source, SOURCES, strict: true)) {
        return new WP_Error(
            'elementor_mcp_spec_source',
            sprintf(
                /* translators: 1: the supplied source, 2: comma-separated valid sources */
                __('"%1$s" is not a spec source. Use one of: %2$s.', domain: 'elementor-mcp'),
                $source,
                implode(', ', SOURCES),
            ),
            ['status' => 422],
    ...
    foreach ($raw_surfaces as $name => $value) {
        ...
        if (is_array($value)) {
            ...
            if (count($stops) < 2) {
                return new WP_Error(
                    'elementor_mcp_spec_gradient_stops',
                    sprintf(
                        /* translators: %s: surface name */
                        __('Gradient surface "%s" needs at least two colour stops.', domain: 'elementor-mcp'),
                        (string) $name,
                    ),
                    ['status' => 422],
                );
        ...
        if ($hex === '') {
            return new WP_Error(
                'elementor_mcp_spec_surface_color',
                sprintf(
                    /* translators: 1: surface name, 2: the unusable value */
                    __('Surface "%1$s" is set to "%2$s", which is not a colour.', domain: 'elementor-mcp'),
                    (string) $name,
                    (string) $value,
                ),
                ['status' => 422],
...
function normalize_type(array $raw): array|WP_Error
{
    ...
    foreach ($raw as $role => $props) {
        ...
        if (!in_array($role, type_roles(), strict: true)) {
            return new WP_Error(
                'elementor_mcp_spec_type_role',
                sprintf(
                    /* translators: 1: the supplied role, 2: comma-separated valid roles */
                    __('"%1$s" is not a type role. Use one of: %2$s.', domain: 'elementor-mcp'),
                    $role,
                    implode(', ', type_roles()),
                ),
                ['status' => 422],
        ...
        if ($size <= 0) {
            return new WP_Error(
                'elementor_mcp_spec_type_size',
                sprintf(
                    /* translators: %s: the type role */
                    __('Type role "%s" has no size.', domain: 'elementor-mcp'),
                    $role,
                ),
                ['status' => 422],
            );
...
function normalize_sections(array $raw, array $surface_names): array|WP_Error
{
    ...
    foreach (array_values($raw) as $index => $section) {
        ...
        if (!in_array($surface, $surface_names, strict: true)) {
            return new WP_Error(
                'elementor_mcp_spec_section_surface',
                sprintf(
                    /* translators: 1: section index, 2: the surface named, 3: comma-separated declared surfaces */
                    __('Section %1$d names surface "%2$s", which the spec does not declare. Declared: %3$s.', domain: 'elementor-mcp'),
                    $index,
                    $surface,
        ...
        if (!array_key_exists($layout, $known) && !is_ratio_layout($layout)) {
            return new WP_Error(
                'elementor_mcp_spec_section_layout',
                sprintf(
                    /* translators: 1: section index, 2: the layout named, 3: comma-separated valid layouts */
                    __('Section %1$d names layout "%2$s". Use one of: %3$s.', domain: 'elementor-mcp'),
                    $index,
                    $layout,
...
function normalize_blocks(array $raw, int $section_index, int $depth = 0): array|WP_Error
{
    ...
    if ($depth > 6) {
        return new WP_Error(
            'elementor_mcp_spec_block_depth',
            sprintf(
                /* translators: %d: section index */
                __('Section %d nests blocks more than six deep.', domain: 'elementor-mcp'),
                $section_index,
            ),
            ['status' => 422],
        );
    ...
    foreach (array_values($raw) as $position => $block) {
        ...
        if (!in_array($type, $known, strict: true)) {
            return new WP_Error(
                'elementor_mcp_spec_block_type',
                sprintf(
                    /* translators: 1: section index, 2: block position, 3: the type named, 4: comma-separated valid types */
                    __('Section %1$d block %2$d is type "%3$s". Use one of: %4$s.', domain: 'elementor-mcp'),
                    $section_index,
                    $position,
                    $type,
                    implode(', ', $known),
        ...
        if ($type === 'widget' && sanitize_key((string) ($block['widget'] ?? '')) === '') {
            return new WP_Error(
                'elementor_mcp_spec_widget_name',
                sprintf(
                    /* translators: 1: section index, 2: block position */
                    __(
                        'Section %1$d block %2$d is a widget and names none. Set "widget" to an Elementor widget type; elementor-mcp/elementor-get-schema with action "list" enumerates them.',
                        domain: 'elementor-mcp',
                    ),
                    $section_index,
                    $position,
        ...
        if (!array_key_exists($layout, layouts()) && !is_ratio_layout($layout)) {
            return new WP_Error(
                'elementor_mcp_spec_block_layout',
                sprintf(
                    /* translators: 1: section index, 2: block position, 3: the layout named, 4: comma-separated valid layouts */
                    __('Section %1$d block %2$d names layout "%3$s". Use one of: %4$s.', domain: 'elementor-mcp'),
                    $section_index,
                    $position,
                    $layout,
        ...
        if ($tag !== '' && !in_array($tag, heading_tags(), strict: true)) {
            return new WP_Error(
                'elementor_mcp_spec_block_tag',
                sprintf(
                    /* translators: 1: section index, 2: block position, 3: the tag named, 4: comma-separated valid tags */
                    __('Section %1$d block %2$d names tag "%3$s". Use one of: %4$s.', domain: 'elementor-mcp'),
                    $section_index,
                    $position,
                    $tag,
        ...
        if (is_array($raw_items)) {
            foreach (array_values($raw_items) as $item) {
                ...
                if ($variant !== '' && !array_key_exists($variant, $variants)) {
                    return new WP_Error(
                        'elementor_mcp_spec_item_variant',
                        sprintf(
                            ...
                            $section_index,
                            $variant,
                            $variants === [] ? __('none', domain: 'elementor-mcp') : implode(', ', array_keys($variants)),
                        ),
                        ['status' => 422],
                    );
                }
                ...
                if ($item_blocks !== []) {
                    foreach (['icon', 'src', 'eyebrow', 'title', 'text', 'value'] as $shorthand) {
                        ...
                        return new WP_Error(
                            'elementor_mcp_spec_item_hybrid',
                            sprintf(
...
function save(string $slug, array $spec): array|WP_Error
{
    ...
    if ($post === null) {
        return new WP_Error(
            'elementor_mcp_spec_no_design',
            sprintf(
                /* translators: %s: the slug asked for */
                __('No design "%s" exists to attach a spec to. Save the design first.', domain: 'elementor-mcp'),
                $slug,
            ),
            ['status' => 404],
        );
