...
namespace ElementorMCP\Design\Contract;
...
function required_token_errors(array $tokens): array
{
    ...
    foreach ([
        '--elementor-mcp-bg' => __('A background color is required.', domain: 'elementor-mcp'),
        '--elementor-mcp-ink' => __('An ink/text color is required.', domain: 'elementor-mcp'),
        '--elementor-mcp-accent' => __('An accent color is required.', domain: 'elementor-mcp'),
        ...
        if (Preflight\normalize_hex($value) === '') {
            $errors[] = sprintf(
                ...
                __('The color for %s must be an exact hex value.', domain: 'elementor-mcp'),
                $property,
            );
        }
    }

    if (($vars['--elementor-mcp-font-heading'] ?? '') === '') {
        $errors[] = __('A heading/display font is required.', domain: 'elementor-mcp');
    }
    if (($vars['--elementor-mcp-font-body'] ?? '') === '') {
        $errors[] = __('A body font is required.', domain: 'elementor-mcp');
    }
    return $errors;
}
...
function readiness_warnings(array $sources): array
{
    ...
    if ($sources['spacing'] === 'missing') {
        $warnings[] = __('No spacing tokens are declared.', domain: 'elementor-mcp');
    }
    ...
    if ($sources['rounded'] === 'missing') {
        $warnings[] = __('No corner-radius tokens are declared.', domain: 'elementor-mcp');
    }
    if ($sources['components'] === 'missing') {
        $warnings[] = __('No component treatments are declared.', domain: 'elementor-mcp');
    }
    if ($sources['dials'] === 'missing') {
        $warnings[] = __('No compositional dials are declared; defaults will be used.', domain: 'elementor-mcp');
    }
...
function activation_error(array $inspection): string
{
    ...
    return $errors !== []
        ? implode(' ', $errors)
        : __('The design is incomplete and cannot be activated.', domain: 'elementor-mcp');
}
