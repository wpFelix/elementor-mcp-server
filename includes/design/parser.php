...
namespace ElementorMCP\Design\Parser;
...
function parse(string $raw): array
{
    ...
    $normalized = \ElementorMCP\Design\Tokens\normalize_md($raw);
    $body = $normalized;
    ...
    if (str_starts_with($normalized, "---\n")) {
        ...
        if ($closing === false) {
            $parse_error = __('Front matter opened with --- but had no closing ---', domain: 'elementor-mcp');
        }
        if ($closing !== false) {
