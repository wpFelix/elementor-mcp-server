...
use ElementorMCP\Design\Examples;
...
?>
<div class="elementor-mcp-starters">
    <div class="elementor-mcp-starters-head">
        <h2><?php esc_html_e('Starter kits', domain: 'elementor-mcp'); ?></h2>
        <p><?php esc_html_e(
    ...
    <div class="elementor-mcp-starters-grid">
        <?php foreach ($starters as $starter):
            $tokens = Tokens\extract($starter['content']);
            $vars = Tokens\css_vars($tokens);
            $bg = $vars['--elementor-mcp-bg'] ?? '#ffffff';
            $ink = $vars['--elementor-mcp-ink'] ?? '#000000';
            $accent = $vars['--elementor-mcp-accent'] ?? '#000000';
            $heading_font = $vars['--elementor-mcp-font-heading'] ?? '';
            $body_font = $vars['--elementor-mcp-font-body'] ?? '';
            ...
            foreach ($tokens['colors'] as $role => $value) {
                $hex = \ElementorMCP\Design\Preflight\normalize_hex((string) $value);
                if ($hex !== '') {
            ...
            ?>
            <article class="elementor-mcp-starter" style="--st-bg:<?php echo esc_attr($bg); ?>;--st-ink:<?php
                echo esc_attr($ink);
            ?>;--st-accent:<?php echo esc_attr($accent); ?>">

                <div class="elementor-mcp-starter-swatches" aria-hidden="true">
                    <?php foreach (array_slice($swatches, offset: 0, length: 6) as $role => $hex): ?>
                        <span
                            class="elementor-mcp-starter-swatch"
                ...
                <div class="elementor-mcp-starter-specimen">
                    <p class="elementor-mcp-starter-eyebrow"><?php echo esc_html(
                        sprintf(
                            /* translators: 1: heading typeface, 2: body typeface. */
                            __('%1$s / %2$s', domain: 'elementor-mcp'),
                            $heading_font !== '' ? $heading_font : __('unset', domain: 'elementor-mcp'),
                            $body_font !== '' ? $body_font : __('unset', domain: 'elementor-mcp'),
                        ),
                    ); ?></p>
                    <p class="elementor-mcp-starter-display" style="font-family:<?php echo
                        esc_attr($heading_font !== '' ? '"' . $heading_font . '", sans-serif' : 'inherit');
                    ?>"><?php echo esc_html($starter['name']); ?></p>
                    <p class="elementor-mcp-starter-body" style="font-family:<?php echo
                        esc_attr($body_font !== '' ? '"' . $body_font . '", sans-serif' : 'inherit');
                    ?>"><?php esc_html_e(
                        'Body text on this background, at the size a visitor actually reads.',
                        domain: 'elementor-mcp',
                    ); ?></p>
                    <span class="elementor-mcp-starter-button"><?php esc_html_e('Primary action', domain: 'elementor-mcp'); ?></span>
                </div>

                <div class="elementor-mcp-starter-body-col">
                    <h3><?php echo esc_html($starter['name']); ?></h3>
                    <p class="elementor-mcp-starter-desc"><?php echo esc_html($starter['description']); ?></p>
                    <p class="elementor-mcp-starter-contrast"><?php echo esc_html(sprintf(
                        /* translators: %s: contrast ratio, e.g. 12.1. */
                        __('Text contrast %s:1 — passes AAA', domain: 'elementor-mcp'),
                        number_format($ratio, decimals: 1),
                    )); ?></p>

                    <div class="elementor-mcp-starter-actions">
                        <form method="post" action="<?php echo esc_url($action_url); ?>">
                            <?php wp_nonce_field('elementor_mcp_design_use_starter'); ?>
                            <input type="hidden" name="action" value="elementor_mcp_design_use_starter" />
                            <input type="hidden" name="slug" value="<?php echo esc_attr($starter['slug']); ?>" />
                            <button type="submit" class="button"><?php
                                echo $taken
                                    ? esc_html__('Copy again', domain: 'elementor-mcp')
                                    : esc_html__('Start from this', domain: 'elementor-mcp');
                            ?></button>
                        </form>
                        <?php if ($taken): ?>
                            <span class="elementor-mcp-starter-taken"><?php esc_html_e(
                                'Already copied',
                                domain: 'elementor-mcp',
                            ); ?></span>
                        <?php endif; ?>
