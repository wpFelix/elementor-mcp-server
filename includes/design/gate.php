...
namespace ElementorMCP\Design\Gate;
 ...
 *
 * `elementor-mcp/check-design` has always been able to judge a candidate page, but an
 ...
 *
 * ELEMENTOR-COMPATIBLE BY CONSTRUCTION
 *
 * The rules read colour and font literals out of the JSON-encoded input rather
 * than duplicating Elementor's schema. That keeps the gate focused on values
 * that Elementor writes without coupling it to a particular widget version. It cannot see a
...
const OPTION = 'elementor_mcp_design_gate_mode';
...
const EXEMPT_PREFIXES = [
    'elementor-mcp/save-design',
    'elementor-mcp/activate-design',
    'elementor-mcp/delete-design',
    'elementor-mcp/check-design',
    'elementor-mcp/preview-ability',
    'elementor-mcp/apply-preview',
    'elementor-mcp/execute-php',
    'elementor-mcp/run-wp-cli',
    'elementor-mcp/write-file',
    'elementor-mcp/edit-file',
    'elementor-mcp/read-file',
    'elementor-mcp/delete-file',
    'elementor-mcp/rollback-change',
];
...
function check(WP_Ability $ability, mixed $input): ?WP_Error
{
    ...
    if (function_exists('elementor_mcp_ability_is_readonly') && \elementor_mcp_ability_is_readonly($ability)) {
        return null;
    }
    ...
    return new WP_Error(
        'elementor_mcp_design_off_direction',
        sprintf(
            /* translators: 1: design slug, 2: the violation list. */
            __(
                'This write does not match the site\'s active design "%1$s". %2$s Use the design\'s own tokens, or call elementor-mcp/get-active-design to read them. If the value is deliberate, add it to the design or record an allowance in its DESIGN.md rather than working around this check.',
 ...
 *
 * Keys are collected alongside values because Elementor can carry a colour in
 ...
 *
 * The palette rules compare hex because Elementor and hand-written style values
 * may use different notations. Without this, the same colour could be judged
 * inconsistently across an Elementor workflow.
 */
...
function register_setting(mixed $sections): mixed
{
    ...
    $sections[] = [
        'id' => 'elementor-mcp-design-gate',
        'title' => __('Design direction on writes', domain: 'elementor-mcp'),
        'description' => __(
        ...
        'fields' => [
            [
                'type' => 'select',
                'name' => OPTION,
                'label' => __('When a write uses a colour or font the design does not list', domain: 'elementor-mcp'),
                'help' => __(
                    'Warn records the drift in the change ledger and lets the write through. Enforce refuses it and tells the agent which token to use instead. Only the palette, the type stack and the design\'s own Don\'t rules are checked here; the copy and anti-slop rules stay in elementor-mcp/check-design, where an agent asks for a judgement rather than being blocked by one.',
                    domain: 'elementor-mcp',
                ),
                'value' => $current,
                'options' => [
                    MODE_OFF => __('Off', domain: 'elementor-mcp'),
                    MODE_WARN => __('Warn and record', domain: 'elementor-mcp'),
                    MODE_ENFORCE => __('Refuse the write', domain: 'elementor-mcp'),
                ],
