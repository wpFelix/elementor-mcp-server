...
(function () {
    ...
    function cssVars(t) {
        ...
        if (bg) { v['--elementor-mcp-bg'] = bg; }
        if (ink) { v['--elementor-mcp-ink'] = ink; }
        if (accent) { v['--elementor-mcp-accent'] = accent; }
        ...
        if (h) { v['--elementor-mcp-font-heading'] = h; }
        if (b) { v['--elementor-mcp-font-body'] = b; }
        if (hw) { v['--elementor-mcp-weight-heading'] = hw; }
        var r = t.rounded.md || firstVal(t.rounded);
        if (r) { v['--elementor-mcp-radius'] = cssLength(cssValue(r)); }
        var s = t.spacing.md || firstVal(t.spacing);
        if (s) { v['--elementor-mcp-space'] = cssLength(cssValue(s)); }
        return v;
    }

    var NAMES = ['--elementor-mcp-bg', '--elementor-mcp-ink', '--elementor-mcp-accent', '--elementor-mcp-font-heading', '--elementor-mcp-font-body', '--elementor-mcp-weight-heading', '--elementor-mcp-radius', '--elementor-mcp-space'];

    function apply(raw) {
        var el = document.querySelector('[data-elementor-mcp-preview]');
        if (!el) { return; }
    ...
    if (window.elementorMcpDesignEditor) {
        wire(window.elementorMcpDesignEditor);
    } else {
        window.addEventListener('elementor-mcp-design-editor-ready', function () {
            wire(window.elementorMcpDesignEditor);
        });
        // Fallback: plain textarea if CodeMirror failed to load.
        var ta = document.getElementById('elementor-mcp-design-content');
        if (ta) {
            apply(ta.value);
...
(function () {
    ...
    var l10n = window.elementorMcpDesignFontNote || {};
    ...
    function missingFamilies(preview) {
        var names = [];
        ['--elementor-mcp-font-heading', '--elementor-mcp-font-body'].forEach(function (prop) {
    ...
    function attachNote(preview, names) {
        ...
        box.className = 'elementor-mcp-preview-fontnote';
    ...
    function init() {
        ...
        document.fonts.ready.then(function () {
            Array.prototype.forEach.call(document.querySelectorAll('[data-elementor-mcp-preview]'), function (preview) {
...
(function () {
    ...
    function init() {
        var btn = document.querySelector('[data-elementor-mcp-copy-design]');
        var source = document.querySelector('.elementor-mcp-detail-md');
