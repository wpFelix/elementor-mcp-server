...
namespace ElementorMCP\Design\Rendered;
 ...
 * reader limited to inline styles reports a correctly styled Elementor page as
...
function inspect(string $url, int $timeout = 20): array|WP_Error
{
    ...
    if ($url === '' || !wp_http_validate_url($url)) {
        return new WP_Error(
            'elementor_mcp_rendered_bad_url',
            __('That is not a URL this site is allowed to fetch.', domain: 'elementor-mcp'),
        );
    }
    ...
    $response = wp_remote_get($url, [
        ...
        'user-agent' => 'Elementor MCP/' . (defined('ELEMENTOR_MCP_VERSION') ? ELEMENTOR_MCP_VERSION : 'dev') . ' (rendered-check)',
    ]);
    $elapsed = (int) round((microtime(true) - $started) * 1000);

    if (is_wp_error($response)) {
        return new WP_Error('elementor_mcp_rendered_unreachable', sprintf(
            /* translators: %s: transport error. */
            __('Could not fetch the page: %s', domain: 'elementor-mcp'),
        ...
        return [
            ...
            'findings' => [[
                ...
                'message' => sprintf(
                    ...
                    __('The page answered HTTP %d, so nothing was rendered to check.', domain: 'elementor-mcp'),
                    $status,
                ),
    ...
    if ($bytes > MAX_BYTES) {
        return new WP_Error('elementor_mcp_rendered_too_large', sprintf(
            /* translators: 1: page size, 2: the limit. */
            __('The page is %1$d bytes, past the %2$d byte limit for this check.', domain: 'elementor-mcp'),
            $bytes,
            MAX_BYTES,
        ));
    }
    ...
    if ($document === null) {
        $findings[] = [
            'check' => 'render-errors',
            'severity' => 'fail',
            'message' => __('The served HTML could not be parsed at all.', domain: 'elementor-mcp'),
            'evidence' => '',
        ];
    } else {
    ...
    // nothing on an Elementor site: Elementor puts every widget's colour and face
...
function headings(DOMXPath $xpath, array &$summary): array
{
    ...
    if ($h1 === 0 && $levels !== []) {
        $findings[] = [
            ...
            'severity' => 'warn',
            'message' => __('The page has headings but no h1.', domain: 'elementor-mcp'),
            'evidence' => '',
        ];
    }
    if ($h1 > 1) {
        $findings[] = [
            ...
            'message' => sprintf(
                ...
                __(
                    'The page has %d h1 elements; one is the convention. On an Elementor page this is usually the theme printing the post title above a hero that already carries one: switch the page to Elementor\'s canvas template, or demote the hero heading.',
    ...
    foreach ($levels as $level) {
        if ($previous !== 0 && $level > $previous + 1) {
            $findings[] = [
                'check' => 'heading-outline',
                'severity' => 'warn',
                'message' => __('The heading outline skips a level, which screen readers announce as a gap.', domain: 'elementor-mcp'),
    ...
    if ($missing_src > 0) {
        $findings[] = [
            ...
            'message' => sprintf(
                ...
                __('%d image elements render with no source, so they show as broken.', domain: 'elementor-mcp'),
                $missing_src,
            ),
...
/**
 * Containers Elementor wrote that ended up with nothing in them.
 *
...
function render_errors(string $html): array
{
    ...
    $patterns = [
        ...
        'unrendered-shortcode' => '/\[[a-z][a-z0-9_-]*(?:\s+[^\]]{0,120})?\]/i',
    ...
    foreach ($patterns as $check => $pattern) {
        ...
        $findings[] = [
            ...
            'message' => match ($check) {
                'php-error' => __('A PHP error is being printed into the served page.', domain: 'elementor-mcp'),
                'unrendered-shortcode' => __(
                    'A shortcode reached the visitor unrendered, which means the plugin or theme that owns it is not handling it here.',
                    domain: 'elementor-mcp',
                ),
                default => __('A template placeholder was served without being substituted.', domain: 'elementor-mcp'),
            },
...
function stylesheets(string $html, string $page_url, int $timeout): array
{
    ...
    foreach ($candidates as $resolved) {
        ...
        $response = wp_remote_get($resolved, [
            'timeout' => $timeout,
            'redirection' => 2,
            'user-agent' => 'Elementor MCP/rendered-check',
        ]);
