...
namespace ElementorMCP\Design\Generate;

use WP_Error;
use ElementorMCP\Design\Contrast;
use ElementorMCP\Design\Gate;
use ElementorMCP\Design\Grammars;
use ElementorMCP\Design\Seed;
use ElementorMCP\Design\Typefaces;
...
function resolve(array $brief): array|WP_Error
{
    ...
    if ($name === '') {
        return new WP_Error(
            'elementor_mcp_brief_name',
            __('A brief needs a name: it seeds every derived value, so two designs on one site diverge.', domain: 'elementor-mcp'),
            ['status' => 422],
        );
    }
    ...
    if (!array_key_exists($mood, $known)) {
        return new WP_Error(
            'elementor_mcp_brief_mood',
            sprintf(
                /* translators: 1: the mood asked for, 2: comma-separated valid moods */
                __('"%1$s" is not a mood this can resolve. Use one of: %2$s.', domain: 'elementor-mcp'),
                $mood,
...
function resolve_palette(string $seed, array $window, array $brief): array|WP_Error
{
    ...
    if ($ink === '') {
        return new WP_Error(
            'elementor_mcp_brief_contrast',
            __('No readable ink could be solved for the ground this brief resolved to.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
    ...
    if ($accent === '') {
        return new WP_Error(
            'elementor_mcp_brief_accent',
            __('No accent could be solved that carries ink at readable contrast.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
...
function resolve_type(string $seed, array $brief): array|WP_Error
{
    ...
    if ($display === '' || $body === '') {
        return new WP_Error(
            'elementor_mcp_brief_pairing',
            __('No sound type pairing could be derived from the available faces.', domain: 'elementor-mcp'),
            ['status' => 500],
        );
    }
