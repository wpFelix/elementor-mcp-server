 ...
 */
function elementor_mcp_resolve_path($path, $must_exist = false)
{
    ...
    $base_dir = apply_filters('elementor_mcp_filesystem_base_dir', ABSPATH);

    // Resolve path that may not exist yet via parent directory.
    $resolved = elementor_mcp_resolve_candidate_path($path);
    ...
    if ($must_exist) {
        ...
        if ($resolved === false) {
            ...
            return new WP_Error('path_not_found', sprintf(__('Path does not exist: %s', domain: 'elementor-mcp'), $path));
        }
    }
    ...
    if ($base_dir !== false) {
        ...
        if (!elementor_mcp_path_is_within_directory($resolved, $real_base)) {
            return new WP_Error('path_outside_base', sprintf(
                /* translators: 1: the resolved path, 2: the allowed base directory */
                __('Path "%1$s" is outside the allowed base directory "%2$s".', domain: 'elementor-mcp'),
                $resolved,
                $real_base,
            ));
        }
 ...
 */
function elementor_mcp_resolve_candidate_path(string $path): string
{
    $resolved_parent = realpath(dirname($path));
    if ($resolved_parent !== false) {
        return elementor_mcp_normalize_absolute_path($resolved_parent . DIRECTORY_SEPARATOR . basename($path));
    }

    return elementor_mcp_normalize_missing_path($path);
}

/**
 * Normalize a path with missing parents from the nearest existing ancestor.
 */
function elementor_mcp_normalize_missing_path(string $path): string
{
    ...
    return elementor_mcp_normalize_absolute_path(implode(DIRECTORY_SEPARATOR, array_reverse($tail)));
}

/**
 * Collapse "." and ".." path segments without requiring the path to exist.
 */
function elementor_mcp_normalize_absolute_path(string $path): string
{
 ...
 */
function elementor_mcp_reject_final_path_symlink(string $resolved): bool|WP_Error
{
    if (!is_link($resolved)) {
        return true;
    }
 ...
 */
function elementor_mcp_path_is_within_directory(string $path, string $directory): bool
{
    $normalized_path = elementor_mcp_normalize_boundary_path($path);
    $normalized_directory = elementor_mcp_normalize_boundary_path($directory);

    if ($normalized_path === $normalized_directory) {
        return true;
    }

    return elementor_mcp_path_is_child_of_normalized_directory($normalized_path, $normalized_directory);
}

/**
 * Check whether a path is contained by a directory boundary, excluding the directory itself.
 */
function elementor_mcp_path_is_child_of_directory(string $path, string $directory): bool
{
    return elementor_mcp_path_is_child_of_normalized_directory(
        elementor_mcp_normalize_boundary_path($path),
        elementor_mcp_normalize_boundary_path($directory),
    );
}
 ...
 */
function elementor_mcp_normalize_boundary_path(string $path): string
{
 ...
 */
function elementor_mcp_path_is_child_of_normalized_directory(string $normalized_path, string $normalized_directory): bool
{
 ...
 */
function elementor_mcp_ensure_parent_dir(string $parent_dir): array|WP_Error
{
 ...
 * @return bool
 */
function elementor_mcp_is_disabled_file($path)
{
