...
const ELEMENTOR_MCP_RATE_WINDOW_SECONDS = 60;

const ELEMENTOR_MCP_RATE_DEFAULT_LIMIT = 120;
 ...
 */
function elementor_mcp_rate_limit(): int
{
    /**
     ...
     */
    /** @var mixed $limit */
    $limit = apply_filters('elementor_mcp_tool_call_rate_limit', ELEMENTOR_MCP_RATE_DEFAULT_LIMIT);

    return is_int($limit) ? $limit : ELEMENTOR_MCP_RATE_DEFAULT_LIMIT;
}
 ...
 */
function elementor_mcp_rate_bucket_key(string $credential): string
{
    $window = (int) floor(time() / ELEMENTOR_MCP_RATE_WINDOW_SECONDS);

    return 'elementor_mcp_rate_' . md5($credential . '|' . $window);
}
 ...
 */
function elementor_mcp_rate_consume(string $credential): array
{
    $limit = elementor_mcp_rate_limit();
    if ($limit <= 0) {
        return ['allowed' => true, 'used' => 0, 'limit' => 0, 'retry_after' => 0];
    }

    $key = elementor_mcp_rate_bucket_key($credential);
    /** @var mixed $stored */
    ...
    $retry_after = ELEMENTOR_MCP_RATE_WINDOW_SECONDS - (time() % ELEMENTOR_MCP_RATE_WINDOW_SECONDS);
    ...
    set_transient($key, $used + 1, ELEMENTOR_MCP_RATE_WINDOW_SECONDS + 5);
 ...
 */
function elementor_mcp_rate_credential_id(): string
{
    ...
    if (function_exists('ElementorMCP\\OAuth\\Middleware\\request_oauth_identity')) {
        $identity = \ElementorMCP\OAuth\Middleware\request_oauth_identity();
 ...
 * @return mixed
 */
function elementor_mcp_rate_pre_mcp_tool_call(mixed $args, string $tool_name): mixed
{
    ...
    $result = elementor_mcp_rate_consume(elementor_mcp_rate_credential_id());
    if ($result['allowed']) {
        return $args;
    }

    return elementor_mcp_rate_limit_error($result);
}
 ...
 */
function elementor_mcp_rate_pre_ability_execute(mixed $input, WP_Ability $ability, string $transport): mixed
{
    if ($transport !== 'rest' || elementor_mcp_ability_is_readonly($ability)) {
        return $input;
    }
 ...
 */
function elementor_mcp_rate_limit_error(array $result): WP_Error
{
    return new WP_Error(
        'elementor_mcp_rate_limited',
        sprintf(
...
add_filter('mcp_adapter_pre_tool_call', callback: 'elementor_mcp_rate_pre_mcp_tool_call', priority: 6, accepted_args: 2);
add_filter('elementor_mcp_pre_ability_execute', callback: 'elementor_mcp_rate_pre_ability_execute', priority: 6, accepted_args: 3);
