...
namespace ElementorMCP\Context;
 ...
 * content is prepended to the Elementor MCP discover-abilities instructions, while
 * the read-only preview shows the system instructions without that user layer.
 */

const INSTRUCTIONS_ENABLED_OPTION = 'elementor_mcp_instructions_enabled';

const INSTRUCTIONS_CONTENT_OPTION = 'elementor_mcp_instructions_content';

const LEGACY_PRO_INSTRUCTIONS_ENABLED_OPTION = 'elementor_mcp_pro_instructions_enabled';

const LEGACY_PRO_INSTRUCTIONS_CONTENT_OPTION = 'elementor_mcp_pro_instructions_content';

const OPTION_MISSING = '__elementor_mcp_option_missing__';

function context_page_slug(): string
{
    return 'elementor-mcp-context';
}
...
{
    return (
        function_exists('\\ElementorMCP\\Pro\\instructions_get_content')
        && function_exists('\\ElementorMCP\\Pro\\instructions_is_enabled')
    );
}
...
{
    if (!defined('ELEMENTOR_MCP_VERSION')) {
        return;
    }
    ...
    add_submenu_page(
        ...
        page_title: \elementor_mcp_nav_label('elementor-mcp-context'),
        menu_title: \elementor_mcp_nav_label('elementor-mcp-context'),
...
function context_handle_post(): ?array
{
    ...
    $action = instructions_post_string('elementor_mcp_context_action');
    if (!in_array($action, ['toggle_context', 'save_context'], strict: true)) {
        return null;
    }

    check_admin_referer('elementor_mcp_context');

    if (!\elementor_mcp_current_user_can_manage()) {
        wp_die(esc_html__('You are not allowed to manage context.', domain: 'elementor-mcp'));
    }
...
function instructions_handle_toggle(): array
{
    ...
    return [
        'type' => 'success',
        'message' => $enable
            ? __('User context enabled for agents.', domain: 'elementor-mcp')
            : __('User context disabled for agents. Content is kept.', domain: 'elementor-mcp'),
    ];
}
...
function instructions_handle_save(): array
{
    ...
    return [
        'type' => 'success',
        'message' => __('User context saved.', domain: 'elementor-mcp'),
    ];
}
...
function context_system_instructions_preview(): string
{
    if (!function_exists('elementor_mcp_build_server_instructions')) {
        return __('System instructions are unavailable until the base Elementor MCP plugin has loaded.', domain: 'elementor-mcp');
    }

    $previous = instructions_custom_injection_suppression_state(action: 'suppress');

    try {
        return (string) apply_filters('elementor_mcp_discover_abilities_instructions', \elementor_mcp_build_server_instructions());
    } finally {
...
function render_context_page(): void
{
    ...
    ?>
    <div class="wrap">
        <h1 class="wp-heading-inline"><?php echo esc_html(\elementor_mcp_nav_label('elementor-mcp-context')); ?></h1>
        <hr class="wp-header-end">

        <p class="description elementor-mcp-context-intro"><?php esc_html_e(
        ...
        <?php do_action('elementor_mcp_context_page_sections'); ?>
    </div>
    <?php
}
...
function render_context_system_section(): void
{
    ...
    ?>
    <section class="elementor-mcp-context-section" aria-labelledby="elementor-mcp-system-context-heading">
        <h2 id="elementor-mcp-system-context-heading"><?php esc_html_e('System context', domain: 'elementor-mcp'); ?></h2>
        <p class="description"><?php esc_html_e(
            'Read-only instructions Elementor MCP generates for this site, adapting them to the plugins you have active. Shown here for visibility and not editable from this page.',
            domain: 'elementor-mcp',
        ); ?></p>

        <div class="elementor-mcp-context-panel elementor-mcp-system-box">
            <details class="elementor-mcp-system-details">
                <summary><?php esc_html_e('Show full system context', domain: 'elementor-mcp'); ?></summary>
                <div class="elementor-mcp-system-preview-wrap" aria-label="<?php echo
                    esc_attr__('Read-only system context preview', domain: 'elementor-mcp')
                ; ?>">
                    <pre class="elementor-mcp-system-preview is-full"><?php echo esc_html($preview); ?></pre>
                </div>
            </details>
            <pre class="elementor-mcp-system-preview is-excerpt"><?php echo esc_html($excerpt); ?></pre>
        </div>
    </section>
    <?php
}
...
function render_user_context_section(): void
{
    ...
    $toggle_label = $is_enabled
        ? __('Disable user context', domain: 'elementor-mcp')
        : __('Enable user context', domain: 'elementor-mcp');
    $toggle_class = $is_enabled ? 'button button-secondary' : 'button button-primary';
    $status_class = $is_enabled ? 'is-on' : 'is-off';
    $status_label = $is_enabled ? __('On', domain: 'elementor-mcp') : __('Off', domain: 'elementor-mcp');
    $hint = $is_enabled
        ? __('User-added context is prepended to the system context shown to agents.', domain: 'elementor-mcp')
        : __(
    ...
    ?>
    <section class="elementor-mcp-context-section" aria-labelledby="elementor-mcp-user-context-heading">
        <h2 id="elementor-mcp-user-context-heading"><?php esc_html_e('User context', domain: 'elementor-mcp'); ?></h2>
        <p class="description"><?php esc_html_e(
            'Additional instructions provided by this site owner for all connected agents.',
            domain: 'elementor-mcp',
        ); ?></p>

        <div class="elementor-mcp-context-panel elementor-mcp-context-status">
            <div>
                <div class="status-text">
                    <strong><?php esc_html_e('User context:', domain: 'elementor-mcp'); ?></strong>
            ...
            <form method="post" action="<?php echo esc_url($form_url); ?>">
                <?php wp_nonce_field('elementor_mcp_context'); ?>
                <input type="hidden" name="elementor_mcp_context_action" value="toggle_context">
        ...
        <div class="elementor-mcp-context-panel elementor-mcp-context-guidance">
        ...
        <form method="post" action="<?php echo esc_url($form_url); ?>" class="elementor-mcp-context-form">
            <?php wp_nonce_field('elementor_mcp_context'); ?>
            <input type="hidden" name="elementor_mcp_context_action" value="save_context">
