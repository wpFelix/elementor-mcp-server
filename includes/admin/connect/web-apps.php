 ...
 */
function elementor_mcp_web_apps(): array
{
    return [
 ...
 * @return list<string>
 */
function elementor_mcp_web_app_oauth_steps(string $which, string $mcp_name, string $mcp_url): array
{
    $steps = match ($which) {
        'chatgpt' => elementor_mcp_oauth_chatgpt_steps($mcp_name, $mcp_url),
        'manus' => elementor_mcp_oauth_manus_steps($mcp_name, $mcp_url),
        'mistral' => elementor_mcp_oauth_mistral_steps($mcp_name, $mcp_url),
        'perplexity' => elementor_mcp_oauth_perplexity_steps($mcp_name, $mcp_url),
        default => elementor_mcp_oauth_connector_steps('Claude', $mcp_name, $mcp_url),
    };
 ...
 */
function elementor_mcp_render_web_apps_step(): void
{
    $reachable = !elementor_mcp_host_unreachable_from_cloud();
    ?>
    ...
    <?php if (!$reachable): ?>
        <div class="notice notice-warning inline" style="margin:0;">
            <p style="margin:0;">
                <strong><?php esc_html_e('None of these can reach this site.', domain: 'elementor-mcp'); ?></strong>
                <?php esc_html_e(
 ...
 */
function elementor_mcp_render_web_apps_config_section(string $oauth_url, string $token_url, ?string $token): void
{
    $default_name = elementor_mcp_get_mcp_server_name_default();
    $display_token = $token ?? ELEMENTOR_MCP_TOKEN_PLACEHOLDER;
    $token_steps = elementor_mcp_build_token_web_ui_configs($token_url, $display_token);

    $apps = [];
    foreach (elementor_mcp_web_apps() as $key => $app) {
        $steps_key = $app['bearer_steps'];
        $has_bearer = $steps_key !== null && array_key_exists($steps_key, $token_steps);
        $apps[$key] = [
            'label' => $app['label'],
            'oauth' => elementor_mcp_web_app_oauth_steps($app['oauth'], $default_name, $oauth_url),
    ...
    ?>
    <h2 class="elementor-mcp-step-heading" id="elementor-mcp-webapps-steps">
    ...
    <div class="elementor-mcp-client-tabs" style="gap:8px; margin-top:16px; margin-bottom:0;">
        <?php foreach ($apps as $key => $app): ?>
            <button
                type="button"
                class="elementor-mcp-client-tab elementor-mcp-webapp-tab"
                data-app="<?php echo esc_attr((string) $key); ?>"
                onclick="elementorMcpSetWebApp('<?php echo esc_js((string) $key); ?>', this)"
            ><?php echo esc_html($app['label']); ?></button>
        <?php endforeach; ?>
    </div>

    <div id="elementor-mcp-webapps-content" style="display:none; margin-top:16px;">
        <?php if (!elementor_mcp_oauth_transport_allowed()): ?>
        ...
        <?php else: ?>
            <p class="elementor-mcp-legend" style="margin:0 0 6px;"><?php esc_html_e(
                'Sign in with OAuth',
                domain: 'elementor-mcp',
            ); ?></p>
            <ol id="elementor-mcp-webapp-oauth-steps" style="margin:0 0 18px 20px; padding:0;"></ol>
        <?php endif; ?>

        <div id="elementor-mcp-webapp-token-wrap">
            <p class="elementor-mcp-legend" style="margin:0 0 6px;"><?php esc_html_e(
                'Or use an access token instead',
                domain: 'elementor-mcp',
            ); ?></p>
            <ol id="elementor-mcp-webapp-token-steps" style="margin:0 0 8px 20px; padding:0;"></ol>
            <p class="description" id="elementor-mcp-webapp-token-hint" style="margin:0 0 8px;"></p>
        </div>

        <p class="description" id="elementor-mcp-webapp-token-none" style="display:none; margin:0;"></p>
    </div>

    <?php // phpcs:disable WordPress.Security.EscapeOutput.OutputNotEscaped -- Every value goes through elementor_mcp_script_json(), which hex-escapes for script context. ?>
    <script>
    (function () {
        var apps = <?php echo elementor_mcp_script_json($apps); ?>;
        ...
        window.elementorMcpSetWebApp = function (key, btn) {
            var app = apps[key];
            if (!app) { return; }
            document.querySelectorAll('.elementor-mcp-webapp-tab').forEach(function (t) { t.classList.remove('active'); });
            btn.classList.add('active');
            document.getElementById('elementor-mcp-webapps-content').style.display = '';

            fill('elementor-mcp-webapp-oauth-steps', app.oauth);

            var hasToken = app.token && app.token.length;
            document.getElementById('elementor-mcp-webapp-token-wrap').style.display = hasToken ? '' : 'none';
            document.getElementById('elementor-mcp-webapp-token-none').style.display = hasToken ? 'none' : '';
            if (hasToken) {
                fill('elementor-mcp-webapp-token-steps', app.token);
                document.getElementById('elementor-mcp-webapp-token-hint').textContent = app.tokenHint;
            } else {
                document.getElementById('elementor-mcp-webapp-token-none').textContent = noToken;
            }
        };

        function selectFirstWebApp() {
            var first = document.querySelector('.elementor-mcp-webapp-tab');
            if (first) { window.elementorMcpSetWebApp(first.getAttribute('data-app'), first); }
        }
