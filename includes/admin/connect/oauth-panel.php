 ...
 * connect instructions live in Step 3 (elementor_mcp_render_oauth_config_section), so this panel only
 * links to the connected-apps manager.
 */
function elementor_mcp_render_oauth_panel(): void
{
    if (!elementor_mcp_oauth_transport_allowed()) {
        return;
    }
    $connected_apps_url = admin_url('admin.php?page=elementor-mcp-connected-apps');
    ?>
    <p class="description" style="margin:0;">
        <a href="<?php echo esc_url($connected_apps_url); ?>">
            <?php esc_html_e('Manage connected apps', domain: 'elementor-mcp'); ?>
        </a>
    </p>
    <?php
}
 ...
 */
function elementor_mcp_render_local_https_notice(): void
{
    if (!elementor_mcp_likely_self_signed_https()) {
        return;
    }
    ?>
    <div class="notice notice-warning inline" style="margin:0 0 12px;">
        <p style="margin:0;">
            <strong><?php esc_html_e('Local HTTPS detected.', domain: 'elementor-mcp'); ?></strong>
            <?php printf(
 ...
 */
function elementor_mcp_render_oauth_config_section(string $rest_url): void
{
    if (!elementor_mcp_oauth_transport_allowed()) {
        return;
    }
    ...
    $configs = elementor_mcp_build_oauth_configs($rest_url, $name_placeholder);
    ...
    <?php foreach ($clients as $key => $label): ?>
        ...
        <button
            type="button"
            class="elementor-mcp-client-tab elementor-mcp-top-client-tab elementor-mcp-oauth-tab"
            data-client="<?php echo esc_attr($key); ?>"
            onclick="elementorMcpOauthSetClient('<?php echo esc_js($key); ?>', this)"
        ><?php echo esc_html($label); ?></button>
        <?php endif; ?>
    <?php endforeach; ?>
    </div>

    <div id="elementor-mcp-oauth-content" style="display:none; margin-top:16px;">
        <div id="elementor-mcp-oauth-connector-wrap" style="display:none; margin-bottom:12px;">
            <a
                id="elementor-mcp-oauth-connector-btn"
                class="button button-primary"
                style="padding:12px 24px; height:auto; font-size:15px;"
                href="#"
                target="_blank"
                rel="noopener"
            ><?php esc_html_e('Add the connector', domain: 'elementor-mcp'); ?></a>
        </div>

        <div id="elementor-mcp-oauth-deeplink-wrap" style="display:none; margin-bottom:12px;">
            <a
                id="elementor-mcp-oauth-deeplink-btn"
                class="button button-primary"
                style="padding:12px 24px; height:auto; font-size:15px;"
                href="#"
            ><?php esc_html_e('One-click install', domain: 'elementor-mcp'); ?></a>
        </div>

        <div
            id="elementor-mcp-oauth-notice"
        ...
        <div id="elementor-mcp-oauth-name-wrap">
        <p style="margin:8px 0 4px;">
            <button
                type="button"
                class="button-link"
                id="elementor-mcp-oauth-name-toggle"
                aria-expanded="false"
                aria-controls="elementor-mcp-oauth-name-field"
                onclick="elementorMcpOauthToggleName(this)"
            ><?php esc_html_e('Change server name (optional)', domain: 'elementor-mcp'); ?></button>
        </p>
        <div id="elementor-mcp-oauth-name-field" hidden style="display:none; margin:6px 0 14px;">
            <input
                type="text"
                id="elementor-mcp-oauth-name"
                value="<?php echo esc_attr($default_name); ?>"
                placeholder="<?php echo esc_attr($default_name); ?>"
                maxlength="25"
                style="width:220px;"
                oninput="elementorMcpOauthUpdateName(this.value)"
            >
            ...
            <div id="elementor-mcp-oauth-name-warning" class="notice notice-warning inline" style="display:none; margin:8px 0 0;">
                <p style="margin:0;">
                    <?php esc_html_e(
            ...
            <div
                id="elementor-mcp-oauth-name-suggestion"
        ...
        <div id="elementor-mcp-oauth-hint" style="font-size:13px; color:#666; padding:6px 0 0;"></div>

        <div id="elementor-mcp-oauth-manual-btn-wrap" style="display:none;">
            <hr style="border:none; border-top:1px solid #dcdcde; margin:12px 0 8px;">
            <button
                type="button"
                class="button button-secondary"
                id="elementor-mcp-oauth-manual-toggle"
                aria-expanded="false"
                aria-controls="elementor-mcp-oauth-manual"
                onclick="elementorMcpOauthToggleManual(this)"
            ><?php esc_html_e('Manual configuration', domain: 'elementor-mcp'); ?></button>
        </div>

        <div id="elementor-mcp-oauth-manual" style="display:none; margin-top:14px;">
            <ol
                id="elementor-mcp-oauth-steps"
                style="display:none; list-style-type:lower-alpha; margin:0 0 4px; padding-left:22px;"
            ></ol>
        </div>

        <?php elementor_mcp_render_agent_prompt_block('elementor-mcp-oauth', method: 'oauth'); ?>
    </div>
    ...
    (function () {
        ...
        echo elementor_mcp_script_json(elementor_mcp_agent_prompt_auth_line('oauth')); ?>;
        var oauthNotes = <?php

        echo elementor_mcp_script_json(elementor_mcp_agent_prompt_oauth_notes()); ?>;
        var manualLabelPrefix = <?php

        echo elementor_mcp_script_json(__('Manual configuration for', domain: 'elementor-mcp')); ?>;
        var connectorLabelPrefix = <?php

        echo elementor_mcp_script_json(__('Add the connector to', domain: 'elementor-mcp')); ?>;
        var deeplinkLabelPrefix = <?php

        echo elementor_mcp_script_json(__('One-click install in', domain: 'elementor-mcp')); ?>;
        var stepOpenLabel = <?php

        echo elementor_mcp_script_json(__('Open your config', domain: 'elementor-mcp')); ?>;
        var stepAddLabel = <?php

        echo elementor_mcp_script_json(__('Add this server', domain: 'elementor-mcp')); ?>;
        var stepAddNote = <?php echo
            elementor_mcp_script_json(__(
        ...
        echo elementor_mcp_script_json(__('Run this in your terminal', domain: 'elementor-mcp')); ?>;
        var copyLabel = <?php

        echo elementor_mcp_script_json(__('Copy', domain: 'elementor-mcp')); ?>;
        var copiedLabel = <?php

        echo elementor_mcp_script_json(__('Copied!', domain: 'elementor-mcp')); ?>;
        var stepSignInLabel = <?php

        echo elementor_mcp_script_json(__('Sign in', domain: 'elementor-mcp')); ?>;
        ...
        echo elementor_mcp_script_json(__('Restart and sign in', domain: 'elementor-mcp')); ?>;
        ...
        function renderSteps(steps) {
            ...
            steps.forEach(function (s) {
                ...
                if (s.copy) {
                    html +=
                        ...
                        'onclick="elementorMcpOauthCopyChip(this)">' +
                        esc(copyLabel) +
                        '</button></span></div>';
                }
                if (s.code) {
                    html +=
                        '<div class="elementor-mcp-config-block" style="margin-top:6px;">' +
                        '<pre>' + withName(s.code) + '</pre>' +
                        '<button type="button" class="button elementor-mcp-copy-btn" onclick="elementorMcpOauthCopyStep(this)">' +
                        esc(copyLabel) +
                        '</button></div>';
                }
                if (s.noteHtml) {
            ...
            document.getElementById('elementor-mcp-oauth-steps').innerHTML = html;
        }
        ...
        function render() {
            ...
            setDisplay('elementor-mcp-oauth-notice', isNotice);
            setDisplay('elementor-mcp-oauth-name-wrap', !isNotice);
            if (isNotice) {
                document.getElementById('elementor-mcp-oauth-notice').textContent = cfg.message || '';
                setDisplay('elementor-mcp-oauth-connector-wrap', false);
                setDisplay('elementor-mcp-oauth-deeplink-wrap', false);
                setDisplay('elementor-mcp-oauth-hint', false);
                setDisplay('elementor-mcp-oauth-steps', false);
                setDisplay('elementor-mcp-oauth-manual-btn-wrap', false);
                setDisplay('elementor-mcp-oauth-manual', false);
                setDisplay('elementor-mcp-oauth-agent-prompt-wrap', false);
                return;
            }
            ...
            setDisplay('elementor-mcp-oauth-connector-wrap', isConnector);
            if (isConnector) {
                var connBtn = document.getElementById('elementor-mcp-oauth-connector-btn');
                connBtn.setAttribute('href', cfg.connector);
                connBtn.textContent = connectorLabelPrefix + ' ' + label;
            }

            setDisplay('elementor-mcp-oauth-deeplink-wrap', hasDeeplink);
            if (hasDeeplink) {
                var dl = cfg.deeplink.split(namePlaceholder).join(encodeURIComponent(mcpName));
                var dlBtn = document.getElementById('elementor-mcp-oauth-deeplink-btn');
            ...
            var hintEl = document.getElementById('elementor-mcp-oauth-hint');
            hintEl.innerHTML = showHint ? cfg.hint : '';
            hintEl.style.display = showHint ? '' : 'none';

            setDisplay('elementor-mcp-oauth-steps', hasManual);
            if (hasSteps) {
            ...
            setDisplay('elementor-mcp-oauth-agent-prompt-wrap', true);
            window.elementorMcpAgentPrompt('elementor-mcp-oauth', {
            ...
            setDisplay('elementor-mcp-oauth-manual-btn-wrap', hasManual && hasPrimary);
            setDisplay('elementor-mcp-oauth-manual', hasManual && (!hasPrimary || manualOpen));
            document.getElementById('elementor-mcp-oauth-manual-toggle')
        ...
        // prompt — lives inside #elementor-mcp-oauth-content, which stayed display:none
        ...
        function selectFirstOauthClient() {
            var first = document.querySelector('.elementor-mcp-oauth-tab');
            if (first) {
                window.elementorMcpOauthSetClient(first.getAttribute('data-client'), first);
            }
        }

        window.elementorMcpOauthSetClient = function (key, btn) {
            client = key;
            manualOpen = false;
            document.querySelectorAll('.elementor-mcp-oauth-tab').forEach(function (t) { t.classList.remove('active'); });
            btn.classList.add('active');
            if (clientLabels[key]) {
                document.getElementById('elementor-mcp-oauth-manual-toggle').textContent =
                    manualLabelPrefix + ' ' + clientLabels[key];
            }
            document.getElementById('elementor-mcp-oauth-content').style.display = '';
            render();
        };

        window.elementorMcpOauthToggleManual = function (btn) {
            manualOpen = !manualOpen;
            btn.setAttribute('aria-expanded', manualOpen ? 'true' : 'false');
            setDisplay('elementor-mcp-oauth-manual', manualOpen);
        };

        function updateOauthNameWarning(value) {
            document.getElementById('elementor-mcp-oauth-name-warning').style.display = value.length >= 25 ? 'block' : 'none';
            var trimmed = value.trim();
            var missing = trimmed.length > 0 && trimmed.toLowerCase().indexOf('elementor-mcp') === -1;
            document.getElementById('elementor-mcp-oauth-name-suggestion').style.display = missing ? 'block' : 'none';
        }

        window.elementorMcpOauthUpdateName = function (value) {
            mcpName = value.trim() || defaultName;
            updateOauthNameWarning(value);
            render();
        };

        window.elementorMcpOauthToggleName = function (btn) {
            var field = document.getElementById('elementor-mcp-oauth-name-field');
        ...
        window.elementorMcpOauthCopyStep = function (btn) {
            var pre = btn.previousElementSibling;
            if (!pre) { return; }
            window.elementorMcpClipboardCopy(pre.textContent).then(function () {
        ...
        window.elementorMcpOauthCopyChip = function (btn) {
            var value = btn.previousElementSibling;
            if (!value) { return; }
            window.elementorMcpClipboardCopy(value.textContent).then(function () {
                var orig = btn.textContent;
                btn.textContent = copiedLabel;
                setTimeout(function () { btn.textContent = orig; }, 1500);
            });
