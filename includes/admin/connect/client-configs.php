 ...
 *
 * Capped at 25 characters total ("elementor-mcp-" prefix + up to 16 chars of host slug)
 ...
 */
function elementor_mcp_get_mcp_server_name_default(): string
{
    ...
    return 'elementor-mcp-' . $site_slug;
}
 ...
 */
function elementor_mcp_build_paste_to_agent_paragraph(
    string $rest_url,
    string $username,
    string $display_password,
    string $name_placeholder = '__ELEMENTOR_MCP_MCP_NAME__',
    ...
    $lines = [
        ...
        '- args array must be exactly ["-y", "@automattic/mcp-wordpress-remote@latest"].'
            . (
                elementor_mcp_likely_self_signed_https()
                    ? "\n"
        ...
        '',
        'If you cannot modify the config of this AI client from here, tell me to expand "Manual configuration for your AI client" on the Elementor MCP Connect page and copy the snippet manually.',
    ];
 ...
 */
function elementor_mcp_build_npx_server(string $rest_url, string $username, string $display_password): array
{
    $env = [
        'WP_API_URL' => $rest_url,
 ...
 */
function elementor_mcp_build_mcpb_manifest(
    string $rest_url,
    string $username,
    string $display_password,
    string $mcp_name,
    ...
    $display_name = $site_name !== '' ? 'Elementor MCP — ' . $site_name : 'Elementor MCP';
    ...
    return [
        ...
        'description' => __(
            'Elementor-focused tools for your WordPress site. Build and update Elementor pages with your AI agent; advanced developer actions remain available only in explicitly enabled profiles.',
            domain: 'elementor-mcp',
        ),
        'author' => ['name' => 'Elementor MCP'],
        'server' => [
...
function elementor_mcp_build_zed_json(string $mcp_name, array $npx_server, int $opts): string
{
...
function elementor_mcp_build_opencode_json(
    string $mcp_name,
    string $rest_url,
    string $username,
    string $display_password,
...
function elementor_mcp_build_codex_toml(
    string $mcp_name,
    string $rest_url,
    string $username,
    string $display_password,
 ...
 * variables all have to be right before anything works. Elementor MCP already serves
 ...
 * Code is handed.
 */
function elementor_mcp_build_claude_code_http_cmd(
    string $mcp_name,
    string $rest_url,
    string $username,
    string $display_password,
...
function elementor_mcp_build_claude_code_cmd(
    string $mcp_name,
    string $rest_url,
    string $username,
    string $display_password,
 ...
 */
function elementor_mcp_build_configs(string $rest_url, string $username, string $display_password, string $mcp_name): array
{
    $opts = JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES;
    $npx_server = elementor_mcp_build_npx_server($rest_url, $username, $display_password);
    ...
    $add_to = __('Add to %s.', domain: 'elementor-mcp');

    $special = [
        // The direct-HTTP form first: fewest moving parts, and the one that
        // cannot half-succeed. See elementor_mcp_build_claude_code_http_cmd().
        'claude-code' => [
            'code' => elementor_mcp_build_claude_code_http_cmd($mcp_name, $rest_url, $username, $display_password),
            'hint' => __('Run in your terminal, then restart Claude Code. No Node or npx needed.', domain: 'elementor-mcp'),
            'paths' => [],
            'isShell' => true,
        ],
        'codex' => [
            'code' => elementor_mcp_build_codex_toml($mcp_name, $rest_url, $username, $display_password),
        ...
        'zed' => [
            'code' => elementor_mcp_build_zed_json($mcp_name, $npx_server, $opts),
            'hint' => sprintf($add_to, '<code>settings.json</code>'),
            'paths' => ['macOS / Linux' => '~/.config/zed/settings.json'],
            'isShell' => false,
        ],
        'opencode' => [
            'code' => elementor_mcp_build_opencode_json($mcp_name, $rest_url, $username, $display_password, $opts),
    ...
    return array_merge(
        elementor_mcp_build_standard_configs($mcp_servers_json, $vscode_servers_json),
        elementor_mcp_build_cli_agent_configs($mcp_servers_json),
        $special,
    );
}
 ...
 * @return array<string, array{code: string, hint: string, paths: array<string, string>, isShell: bool}>
 */
function elementor_mcp_build_standard_configs(string $mcp_servers_json, string $vscode_servers_json): array
{
    /* translators: %s: config file name wrapped in <code> tags */
    $add_to = __('Add to %s.', domain: 'elementor-mcp');
    ...
    return [
        ...
        'factory-droid' => [
            ...
            'hint' => sprintf($add_to, '<code>mcp.json</code>'),
            'paths' => [
                __('Global', domain: 'elementor-mcp') => '~/.factory/mcp.json',
                __('Project', domain: 'elementor-mcp') => '.factory/mcp.json',
            ],
            'isShell' => false,
        ],
        'vscode' => [
 ...
 * @return array<string, array{code: string, hint: string, paths: array<string, string>, isShell: bool}>
 */
function elementor_mcp_build_cli_agent_configs(string $mcp_servers_json): array
{
    /* translators: %s: config file name wrapped in <code> tags */
    $add_to = __('Add to %s.', domain: 'elementor-mcp');
    ...
    return [
        'kimi-cli' => [
            'code' => $mcp_servers_json,
            'hint' => sprintf($add_to, '<code>mcp.json</code>'),
            'paths' => [__('Global', domain: 'elementor-mcp') => '~/.kimi/mcp.json'],
            'isShell' => false,
        ],
        ...
        'zcode' => [
            'code' => $mcp_servers_json,
            'hint' => __('Add through ZCode\'s MCP server manager, choosing the stdio transport.', domain: 'elementor-mcp'),
            'paths' => [],
            'isShell' => false,
        ],
    ];
 ...
 */
function elementor_mcp_handle_download_mcpb(): void
{
    if (!elementor_mcp_current_user_can_manage()) {
        wp_die(esc_html__('You are not allowed to download this bundle.', domain: 'elementor-mcp'));
    }

    check_admin_referer('elementor_mcp_download_mcpb');
    ...
    $raw_password = $_POST['elementor_mcp_mcpb_password'] ?? '';
    $password = is_string($raw_password) ? (string) preg_replace('/\s+/', replacement: '', subject: $raw_password) : '';
    if ($password === '') {
        wp_die(esc_html__('Missing application password for the bundle.', domain: 'elementor-mcp'));
    }

    $username = wp_get_current_user()->user_login;
    $rest_url = rest_url('mcp/elementor-mcp');

    $raw_name = $_POST['elementor_mcp_mcpb_name'] ?? '';
    ...
    if ($mcp_name === '' || strlen($mcp_name) > 25) {
        $mcp_name = elementor_mcp_get_mcp_server_name_default();
    }

    $manifest = elementor_mcp_build_mcpb_manifest($rest_url, $username, $password, $mcp_name);
    ...
    $tmp = wp_tempnam('elementor-mcp-mcpb');
    $zip = new ZipArchive();
    if ($tmp === '' || $zip->open($tmp, ZipArchive::OVERWRITE) !== true) {
        wp_die(esc_html__('Could not create the bundle archive.', domain: 'elementor-mcp'));
    }
    ...
    $filename = 'elementor-mcp-' . sanitize_file_name($host !== '' ? $host : 'site') . '.mcpb';
 ...
 */
function elementor_mcp_render_mcpb_download(string $display_password, string $mcp_name): void
{
    ...
    ?>
    <div id="elementor-mcp-mcpb-download" style="display:none; margin-top:20px; margin-bottom:4px;">
        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:0;">
            <input type="hidden" name="action" value="elementor_mcp_download_mcpb">
            <?php wp_nonce_field('elementor_mcp_download_mcpb'); ?>
            <input type="hidden" name="elementor_mcp_mcpb_password" value="<?php echo esc_attr($display_password); ?>">
            <input type="hidden" name="elementor_mcp_mcpb_name" id="elementor-mcp-mcpb-name" value="<?php echo
                esc_attr($mcp_name)
            ; ?>">
            <button
                type="submit"
        ...
        <p style="margin:8px 0 4px;">
            <button
                type="button"
                class="button-link"
                onclick="elementorMcpShowPromptForDesktop(this)"
            ><?php esc_html_e('Use the prompt for Claude Desktop instead', domain: 'elementor-mcp'); ?></button>
        </p>
    </div>
    <?php
}
...
function elementor_mcp_render_json_config_block(): void
{ ?>
    <div class="elementor-mcp-tab-content" style="border-radius:4px;">
        <div class="elementor-mcp-config-block">
            <pre id="elementor-mcp-config-code"></pre>
            <button type="button" class="button elementor-mcp-copy-btn" onclick="elementorMcpCopyConfig(this)"><?php esc_html_e(
                'Copy',
                domain: 'elementor-mcp',
            ); ?></button>
        </div>
        <div id="elementor-mcp-config-footer" style="font-size:13px; color:#5e6c7d; border-top: 1px solid #d7e0ea;">
            ...
            <div id="elementor-mcp-config-hint" style="padding: 10px 16px;"></div>
            <div id="elementor-mcp-config-paths" style="padding: 0 16px 10px;"></div>
        </div>
    </div>
    <?php }
