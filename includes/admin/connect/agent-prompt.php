 ...
 * expects to exist.
 */
function elementor_mcp_agent_prompt_auth_line(string $method): string
{
    return match ($method) {
        'oauth' => __(
        ...
        'token' => __(
            'A Elementor MCP access token, sent as an Authorization: Bearer header. The value is already in the configuration below. Treat it as a secret: never commit it, never echo it back to me in full, and never send it anywhere except this site.',
            domain: 'elementor-mcp',
        ),
        default => __(
 ...
 * @return list<string>
 */
function elementor_mcp_agent_prompt_token_notes(): array
{
    return [
        __(
 ...
 * @return list<string>
 */
function elementor_mcp_agent_prompt_password_notes(): array
{
    return [
        __(
 ...
 * @return list<string>
 */
function elementor_mcp_agent_prompt_oauth_notes(): array
{
    return [
        __(
 ...
 */
function elementor_mcp_render_agent_prompt_block(string $prefix, string $method): void
{
    elementor_mcp_render_agent_prompt_assets_once();
    $is_secret = $method !== 'oauth';
    ?>
    <div class="elementor-mcp-agent-prompt" id="<?php echo esc_attr($prefix); ?>-agent-prompt-wrap" style="margin-top:14px;">
        <p style="margin:0 0 6px;">
            <strong><?php esc_html_e('Easier: let your AI coder set this up', domain: 'elementor-mcp'); ?></strong>
            <button
                type="button"
                class="button-link"
                aria-expanded="true"
                aria-controls="<?php echo esc_attr($prefix); ?>-agent-prompt-body"
                onclick="elementorMcpToggleAgentPrompt('<?php echo esc_js($prefix); ?>', this)"
                style="margin-left:8px;"
            ><?php esc_html_e('Hide', domain: 'elementor-mcp'); ?></button>
        </p>
        <?php /*
        ...
        <div id="<?php echo esc_attr($prefix); ?>-agent-prompt-body">
            ...
            <div class="elementor-mcp-config-block">
                ...
                <button
                    ...
                    onclick="elementorMcpCopyAgentPrompt('<?php echo esc_js($prefix); ?>', this)"
 ...
 */
function elementor_mcp_render_agent_prompt_assets_once(): void
{
    ...
    $strings = [
        'intro' => __('Set up this WordPress site as an MCP server in %s.', domain: 'elementor-mcp'),
        'serverName' => __('Server name to use:', domain: 'elementor-mcp'),
        'serverUrl' => __('Server URL:', domain: 'elementor-mcp'),
        'auth' => __('Authentication:', domain: 'elementor-mcp'),
        'runThis' => __('Run exactly this command:', domain: 'elementor-mcp'),
        'writeThis' => __('Use exactly this configuration:', domain: 'elementor-mcp'),
        'writeTo' => __('Write it to the configuration file for this client:', domain: 'elementor-mcp'),
        'uiSteps' => __(
            'This client has no configuration file for MCP servers — it is added through its own interface. Walk me through these steps:',
            domain: 'elementor-mcp',
        ),
        'rules' => __('Rules:', domain: 'elementor-mcp'),
        'ruleExact' => __(
        ...
        'ruleNoAsk' => __('6. Do not ask me to confirm choices already specified above.', domain: 'elementor-mcp'),
        'verify' => __('Then verify, and tell me what you found:', domain: 'elementor-mcp'),
        'verifyTools' => __(
            '- List this server\'s tools. A working connection returns a long list of names beginning with elementor_mcp_. An empty list means the connection is up but the site is exposing nothing, which is a different problem and not a configuration error.',
            domain: 'elementor-mcp',
        ),
        'verifyCall' => __(
        ...
        'notesTitle' => __('Known quirks, so you do not chase them:', domain: 'elementor-mcp'),
        'copied' => __('Copied!', domain: 'elementor-mcp'),
        'show' => __('Show', domain: 'elementor-mcp'),
        'hide' => __('Hide', domain: 'elementor-mcp'),
    ];
    ?>
    ...
    <script>
    (function () {
        var S = <?php echo elementor_mcp_script_json($strings); ?>;
        ...
        window.elementorMcpAgentPrompt = function (prefix, ctx) {
        ...
        window.elementorMcpToggleAgentPrompt = function (prefix, btn) {
        ...
        window.elementorMcpCopyAgentPrompt = function (prefix, btn) {
            var el = document.getElementById(prefix + '-agent-prompt');
            if (!el) { return; }
            window.elementorMcpClipboardCopy(el.textContent).then(function () {
