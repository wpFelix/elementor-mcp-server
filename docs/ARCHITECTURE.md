...
Elementor MCP is a dual-era MCP server.
...
`server/discover` advertises only what is actually registered. Subscriptions, logging, and the tasks extension are never advertised — Elementor MCP has no change-notification producer, so `subscriptions/listen` is not implemented. Cacheable results carry `cacheScope: "private"`, because the ability list is filtered per user, per safety profile, and per site.
...
4. Elementor MCP applies the manual ability policy and active safety profile before execution.
...
- `includes/abilities/elementor/`: the 41-ability Elementor Core surface and its shared runtime — document inspection, page construction, element editing, templates, global styles, Atomic Elements, Global Classes, and Variables. Pro consumes this runtime but does not duplicate these registrations.
...
- `includes/oauth/`: OAuth authorization server, token repositories, discovery, and connected-app management. `middleware.php` also routes the Bearer path: a credential carrying the `emcp_` prefix is validated as an access token, and anything else is treated as an OAuth JWT.
...
Elementor MCP Pro registers specializations only when their third-party dependency is available. Every manifest gate, bootstrap, category registration, and loader is isolated with per-integration health reporting. An error in one integration must not prevent unrelated abilities from registering.
