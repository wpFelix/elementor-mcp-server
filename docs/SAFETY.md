...
```json
{
  "ability_name": "elementor-mcp/woocommerce-create-refund",
  "parameters": {
    "order_id": 123,
    "amount": "10.00",
...
Elementor MCP removes the control-only `confirm` field before target schema validation unless the target ability explicitly declares its own field with that name.
...
- **Direct filesystem access is required.** Elementor MCP runs headless, and `request_filesystem_credentials()` has no screen to render an FTP form on. Anything other than the `direct` method is refused with the method WordPress selected, instead of stalling or half-writing.
- **Elementor MCP cannot target itself.** `deactivate-plugin` and `delete-plugin` refuse Elementor MCP and Elementor MCP Pro: severing the connection mid-call would leave the agent unable to undo what it just did.
