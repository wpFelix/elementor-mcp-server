...
use function ElementorMCP\OAuth\Middleware\bearer_secret;
use function ElementorMCP\OAuth\Middleware\oauth_identity_may_use_route;
use function ElementorMCP\OAuth\Middleware\record_oauth_identity;
use function ElementorMCP\OAuth\Middleware\request_identity_via;
use function ElementorMCP\OAuth\Middleware\reset_request_context;
use function ElementorMCP\OAuth\Middleware\route_required_scope;
...
final class AccessTokenTest extends TestCase
{
    ...
    {
        return [
            'plain' => ['Bearer emcp_abc123', 'emcp_abc123'],
            'lowercase scheme' => ['bearer emcp_abc123', 'emcp_abc123'],
            'surrounding whitespace' => ["  Bearer   emcp_abc123\t", 'emcp_abc123'],
            ...
            'two values' => ['Bearer emcp_abc other', ''],
        ];
    }
    ...
    {
        $this->assertTrue(elementor_mcp_token_looks_like('emcp_abc123'));
        $this->assertFalse(elementor_mcp_token_looks_like('legacy_abc123'));
        $this->assertFalse(elementor_mcp_token_looks_like('eyJhbGciOiJSUzI1NiJ9.body.sig'));
        $this->assertFalse(elementor_mcp_token_looks_like(''));
        ...
        $this->assertFalse(elementor_mcp_token_looks_like('xemcp_abc123'));
    }

    public function test_a_basic_credential_is_never_taken_for_an_access_token(): void
    {
        $this->assertFalse(elementor_mcp_token_looks_like(bearer_secret('Basic ' . base64_encode('emcp_x:secret'))));
    }

    public function test_digest_is_stable_and_distinguishes_secrets(): void
    {
        $digest = elementor_mcp_token_hash('emcp_abc123');

        $this->assertSame($digest, elementor_mcp_token_hash('emcp_abc123'));
        $this->assertSame(64, strlen($digest));
        $this->assertMatchesRegularExpression('/^[0-9a-f]{64}$/', $digest);
        $this->assertNotSame($digest, elementor_mcp_token_hash('emcp_abc124'));
    }
    ...
    public function test_token_identity_reaches_the_canonical_mcp_endpoint(): void
    {
        ...
        $this->assertTrue(oauth_identity_may_use_route('/mcp/elementor-mcp', 'POST'));
        $this->assertTrue(oauth_identity_may_use_route('/mcp/elementor-mcp-oauth', 'POST'));
    ...
    public function test_oauth_identity_keeps_its_narrower_route_boundary(): void
    {
        ...
        $this->assertFalse(oauth_identity_may_use_route('/mcp/elementor-mcp', 'POST'));
    }
    ...
    public function test_an_unauthenticated_request_to_the_password_endpoint_gets_no_oauth_challenge(): void
    {
        ...
        $this->assertNull(route_required_scope('/mcp/elementor-mcp', 'POST'));
        $this->assertSame('mcp', route_required_scope('/mcp/elementor-mcp-oauth', 'POST'));
    }
    ...
    public function test_token_identity_reaches_the_ability_run_route(): void
    {
        ...
        $this->assertTrue(oauth_identity_may_use_route('/elementor-mcp/v1/abilities/elementor-mcp/diagnostics/run', 'POST'));
