#!/usr/bin/env bash
#
# Build the distributable Elementor MCP ZIP.
#
# One archive, one name: `elementor-mcp.zip`, attached to every GitHub release.
#
...
# that URL — behind elementormcp.com/download/latest — is what the website, the
# purchase emails and Elementor MCP Pro's dependency notice all point at. A pinned
# download is still available per release, because every release keeps its own
# copy at /releases/download/<tag>/elementor-mcp.zip, which is what the update
# endpoint hands to WordPress.
#
# There is no WordPress.org variant. Elementor MCP is not listed in the directory and
...
#
# The archive uses `elementor-mcp/` as the root folder. WordPress names the installed
# directory after the archive root, and while Pro now finds the free plugin by
# its `elementor-mcp.php` file name rather than its directory, a predictable directory
# is what every support answer and path in the docs assumes. GitHub's
# auto-generated "Source code (zip)" uses `elementor-mcp-<tag>/` and is therefore NOT
# installable.
#
...
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
VERSION="$(grep -m1 '^ \* Version:' elementor-mcp.php | sed 's/.*Version:[[:space:]]*//' | tr -d '[:space:]')"

if [ -z "$VERSION" ]; then
  echo "Could not read Version from elementor-mcp.php header" >&2
  exit 1
fi

echo "Packaging Elementor MCP $VERSION"
...
for entry in * .[!.]*; do
  case "$entry" in
    ...
    package.json|package-lock.json|bun.lock|bun.lockb|tsconfig.json) continue ;;
  ...
  cp -R "$entry" "$BUILD/elementor-mcp/"
done

verify_tree "$BUILD/elementor-mcp" "release build"
make_zip "$ROOT/build/elementor-mcp.zip" "$BUILD" elementor-mcp
echo "  build/elementor-mcp.zip"

rm -rf "$BUILD/elementor-mcp"
